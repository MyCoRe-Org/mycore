    ...
    <div>
      ...
      <script type="text/javascript">
        ...
        function update(elementID,categID, f) {
          <xsl:if test="string-length($loadingImage) &gt; 0">
            var loaderImage = '&lt;img class="loading" src="<xsl:value-of select="concat($WebApplicationBaseURL,$loadingImage)"/>" /&gt;';
          ...
          jQuery(document.getElementById(elementID)).load('<xsl:value-of select="concat($ServletsBaseURL,$MCR.classbrowser.Servlet,$addParam)" />', requestParam, f );
        }
