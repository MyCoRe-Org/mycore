...
namespace mycore.viewer.widgets.canvas {
    ...
    export class TileImagePageIIIF implements model.AbstractPage {
        ...
        public draw(ctx: CanvasRenderingContext2D, rect: Rect, scale, overview: boolean): void {
            ...
            let tileSizeInZoomLevel = TileImagePageIIIF.TILE_SIZE / zoomLevelScale;
        ...
        private _abortLoadingTiles() {
            this._loadingTiles.forEach((k, v) => {
                v.onerror = TileImagePageIIIF.EMPTY_FUNCTION;
                v.onload = TileImagePageIIIF.EMPTY_FUNCTION;
                v.src = "#";
            });
            this._loadingTiles.clear();
        }
        ...
        private loadTile(tilePos: Position3D) {
            ...
            if (this._tiles.has(tilePos)) {
                ...
                if (!this._loadingTiles.has(tilePos)) {
                    this._loadTile(tilePos, iiifPos, (img: HTMLImageElement) => {
        ...
        private tilePosToIIIFPos(tilePos: Position3D) {
            let iiifPos:any;
            iiifPos = tilePos;
            // iiifPos.z = Math.ceil(this._width / Math.pow(2, this.maxZoomLevel())) * Math.pow(2, this._backBufferAreaZoom);
            // iiifPos.x = iiifPos.x * (256 * Math.pow(2, this._backBufferAreaZoom));
            // iiifPos.w = (256 * Math.pow(2, this._backBufferAreaZoom));
            // iiifPos.y = iiifPos.y * (256 * Math.pow(2, this._backBufferAreaZoom));
            // iiifPos.h = (256 * Math.pow(2, this._backBufferAreaZoom));
            // iiifPos.z = 256;
        ...
        private maxZoomLevel(): number {
            return Math.max(Math.ceil(Math.log(Math.max(this._width, this._height) / TileImagePageIIIF.TILE_SIZE) / Math.LN2), 0);
        }
        ...
        private _loadTile(tilePos: Position3D, iiifPos: any, okCallback: (image: HTMLImageElement) => void, errorCallback: () => void): void {
