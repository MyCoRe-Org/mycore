...
export function viewerClearTextSelection() {
...
export function addIviewComponent(component: any) {
...
export class MoveVector extends Position2D {
...
export function ViewerFormatString(pattern: string, args: any) {
  let replaceArg = (pattern: string, i: any, arg: string) => pattern.replace("{" + i + "}", arg);
  let resultPattern: string = pattern;
...
export interface ViewerPropertyObserver<T> {
  propertyChanged(_old: ViewerProperty<T>, _new: ViewerProperty<T>);
}

export interface ContainerObserver<T1, T2> {
 ...
 */
export class ViewerUserSettingStore {
...
export function viewerCrossBrowserWheel(element: HTMLElement, handler: (e: {
  deltaX: number; deltaY: number; orig: any; pos: Position2D; altKey?: boolean, ctrlKey?: boolean
}) => void) {
  let internHandler = (e: any) => {
    ...
    if ("axis" in e) {
      ...
      if ("detail" in e) {
        ...
        let obj = { deltaX: 0, deltaY: 0, orig: e, pos: pos, altKey: e.altKey, ctrlKey: e.ctrlKey };
    ...
    if ("wheelDelta" in e) {
      let val = <number>-e.wheelDelta;
      let deltaObj = <any>((e.shiftKey) ? { "deltaX": val, "deltaY": 0 } : { "deltaX": 0, "deltaY": val });
      deltaObj.orig = e;
      deltaObj.pos = pos;
...
export interface GivenViewerPromise<T1, T2> {
  then(handler: (result: T1) => void): void;

  onreject(handler: (reason: T2) => void): void;
}

export class ViewerPromise<T1, T2> implements GivenViewerPromise<T1, T2> {
...
export class ViewerParameterMap extends MyCoReMap<string, string> {
  constructor() {
    super();
  }
...
export function singleSelectShim(xml: Document, xpath: string, nsMap: MyCoReMap<string, string>): Node {
  /**
...
export function getNodesShim(xml: Document, xpathExpression: string, contextNode: any,
...
export class XMLUtil {
  ...
  public static NS_MAP = (() => {
    const nsMap = new MyCoReMap<string, string>();
