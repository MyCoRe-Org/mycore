...
import {MyCoReMap, Position2D, Rect, Size2D} from "../../Utils";
import {Scrollbar} from "./Scrollbar";
import {PageAreaInformation} from "./model/PageArea";
import {AbstractPage} from "../../components/model/AbstractPage";
import {StructureImage} from "../../components/model/StructureImage";

export class PageLayout {

    public init(model: OrderedListModel,
                pageController: PageController,
                pageDimension: Size2D,
                horizontalScrollbar: Scrollbar,
                verticalScrollbar: Scrollbar,
                pageLoader: (number) => void) {
        this._model = model;
    ...
    public _insertedPages: Array<number> = [];
    ...
    public getPageHitInfo(windowPosition: Position2D): PageHitInfo {
        ...
        for (let page of pageArea.getPagesInViewport(viewport)) {
            const structureImage = this._model.hrefImageMap.get(page.id);
            if (structureImage == null) {
                continue;
            }
            const info: PageAreaInformation = pageArea.getPageInformation(page);
            const realPageDimension = page.size.getRotated(info.rotation).scale(info.scale);
            const pageRect = new Rect(new Position2D(
                    info.position.x - (realPageDimension.width / 2),
                    info.position.y - (realPageDimension.height / 2)),
                realPageDimension);
            if (pageRect.intersects(positionInArea)) {
                const r = pageRect.flip(info.rotation);
                const p1 = r.pos.rotate(info.rotation);
                const p2 = positionInArea.rotate(info.rotation);
                const hit = new Position2D(Math.abs(p2.x - p1.x), Math.abs(p2.y - p1.y));
                return {
                    id: page.id,
...
export interface OrderedListModel {
    children: MyCoReMap<number, AbstractPage>;
    hrefImageMap: MyCoReMap<string, StructureImage>;
    pageCount: number;
}
