...
import { PageHitInfo, PageLayout } from "../../base/widgets/canvas/PageLayout";
import { MetsStructureModel } from "../widgets/MetsStructureModel";
...
import { HighlightAltoChapterCanvasPageLayer } from "../widgets/canvas/HighlightAltoChapterCanvasPageLayer";
import { AddCanvasPageLayerEvent } from "../../base/components/events/AddCanvasPageLayerEvent";
import { PageController } from "../../base/widgets/canvas/PageController";
...
import { InterpolationAnimation } from "../../base/widgets/canvas/InterpolationAnimation";
...
import { MyCoReMap, Position2D, Rect } from "../../base/Utils";
import { AltoFile } from "../widgets/alto/AltoFile";
import { AltoElement } from "../widgets/alto/AltoElement";

export class MyCoReHighlightAltoComponent extends ViewerComponent {
  private pageLayout: PageLayout = null;
  private highlightLayer: HighlightAltoChapterCanvasPageLayer =
    new HighlightAltoChapterCanvasPageLayer();
  private _model: MetsStructureModel;
  ...
  public init(): any {
    if (this._settings.doctype === 'mets') {
      ...
      this.trigger(new AddCanvasPageLayerEvent(this, 0, this.highlightLayer));
    }
  }

  public get handlesEvents(): string[] {
    if (this._settings.doctype === 'mets') {
      return [ChapterChangedEvent.TYPE,
      PageLayoutChangedEvent.TYPE,
      RequestPageEvent.TYPE,
      MetsLoadedEvent.TYPE,
      TextEditEvent.TYPE
      ];
    } else {
      return [];
  ...
  public getPageLayout(): PageLayout {
  ...
  public setChapter(chapterId: string, triggerChapterChangeEvent: boolean = false, forceChange: boolean = false) {
    ...
    if (triggerChapterChangeEvent) {
      const chapter: StructureChapter = this._altoChapterContainer.getChapter(chapterId);
      this.trigger(new ChapterChangedEvent(this, chapter));
    }
  }
  ...
  public handleDarkenPageAnimation() {
    ...
    if (!selected && !highlighted) {
      ...
      this.highlightLayer.fadeAnimation = new InterpolationAnimation(1000, oldValue, 0);
      this.getPageController().addAnimation(this.highlightLayer.fadeAnimation);
      return;
    }
    // fade in
    const alpha: number = selected ? 0.4 : 0.15;
    this.highlightLayer.fadeAnimation = new InterpolationAnimation(1000, oldValue, alpha);
    this.getPageController().addAnimation(this.highlightLayer.fadeAnimation);
  }

  public handle(e: ViewerEvent) {
    if (e.type === MetsLoadedEvent.TYPE) {
      const mle = e as MetsLoadedEvent;
      this._model = mle.mets.model as MetsStructureModel;
      if (!this.isEnabled) {
        return;
      }
      ...
      this.trigger(new RequestDesktopInputEvent(this, new HighlightAltoInputListener(this)));
    }
    ...
    if (e.type === PageLayoutChangedEvent.TYPE) {
      this.pageLayout = (e as PageLayoutChangedEvent).pageLayout;
    }
...
export class HighlightAltoInputListener extends DesktopInputAdapter {
  ...
  private getChapterId(position: Position2D): string {
    ...
    const pageHitInfo: PageHitInfo = pageLayout.getPageHitInfo(position);
...
export class AltoChapterContainer {
  ...
  constructor(private _model: MetsStructureModel) {
  ...
  getAreaListOfChapter(chapter: StructureChapter): MetsArea[] {
  ...
  getChapter(chapterId: string): StructureChapter {
    return this.findChapter(this._model.rootChapter, chapterId);
  }

  findChapter(from: StructureChapter, chapterId: string): StructureChapter {
  ...
  getBlocklistOfChapterAndAltoHref(chapterId: string, altoHref: string): MetsArea[] {
    const chapter: StructureChapter = this.getChapter(chapterId);
    if (chapter == null) {
      return [];
    }
  ...
  getAllBlocklistChapters(from: StructureChapter): StructureChapter[] {
    let chapters: StructureChapter[] = [];
  ...
  addPage(pageId: string, altoHref: string, alto: AltoFile) {
...
export class AltoChapter {
  ...
  public addPage(pageId: string, altoFile: AltoFile, metsAreas: MetsArea[]) {
    const altoBlocks: AltoElement[] = this.getAltoBlocks(altoFile, metsAreas);
   ...
   */
  public fixEmptyAreas(pageId: string, alto: AltoFile) {
    let thisAreas = this.boundingBoxMap.get(pageId);
  ...
  private getAltoBlocks(altoFile: AltoFile,
    metsAreas: MetsArea[]): AltoElement[] {
    const allBlocks = altoFile.allElements;
    const ids = allBlocks.map(block => block.getId());
    const blocks: AltoElement[] = [];
  ...
  private getAreaRects(altoFile: AltoFile, blocks: AltoElement[]): Rect[] {
