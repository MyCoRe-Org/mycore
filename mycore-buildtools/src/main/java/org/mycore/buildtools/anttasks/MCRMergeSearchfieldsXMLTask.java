...
public class MCRMergeSearchfieldsXMLTask extends Task {
    ...
    public void execute() throws BuildException {
        try {
            ...
            Document docDelta = builder.parse(new InputSource(new InputStreamReader(new FileInputStream(new File(delta)), "UTF-8")));
            docDelta.normalize();
    ...
    private void merge(Document docBase, Document docDelta) {
        NodeList nlDeltaElemIndex = docDelta.getDocumentElement().getElementsByTagName("index");
        for (int i = 0; i < nlDeltaElemIndex.getLength(); i++) {
            ...
            Element eBaseIndex = findIndexElement(eDeltaIndex.getAttribute("id"), docBase);
            mergeIndexElements(eBaseIndex, eDeltaIndex);
        }
    }

    private Element findIndexElement(String id, Document doc) {
        NodeList nlElemIndex = doc.getDocumentElement().getElementsByTagName("index");
    ...
    private void mergeIndexElements(Element eBaseIndex, Element eDeltaIndex) {
        ...
        for (int i = 0; i < nlDeltaElemField.getLength(); i++) {
            ...
            Element eBaseField = findFieldElement(eDeltaField.getAttribute("name"), eBaseIndex);
            if (overwrite) {
    ...
    private void mergeField(Element eBaseField, Element eDeltaField) {
        ...
        if (!eBaseField.getAttribute("source").equals("") && !eDeltaField.getAttribute("source").equals("")
            && !eBaseField.getAttribute("source").equals(eDeltaField.getAttribute("source"))) {
            throw new BuildException("Attributes source of field " + eBaseField.getAttribute("name") + " must be equal to be merged!" + "\n   base file : "
                + base + "\n   delta file: " + delta);
        }
        if (!eBaseField.getAttribute("type").equals("") && !eDeltaField.getAttribute("type").equals("")
            && !eBaseField.getAttribute("type").equals(eDeltaField.getAttribute("type"))) {
            throw new BuildException("Attributes type of field " + eBaseField.getAttribute("name") + " must be equal to be merged!" + "\n   base file : "
                + base + "\n   delta file: " + delta);
        }
        ...
        if (!eDeltaField.getAttribute("objects").equals("")) {
            if (eBaseField.getAttribute("objects").trim().length() == 0) {
                eBaseField.setAttribute("objects", eDeltaField.getAttribute("objects"));
            } else {
        ...
        if (!eDeltaField.getAttribute("value").equals("")) {
            if (eBaseField.getAttribute("value").trim().length() == 0) {
                eBaseField.setAttribute("value", eDeltaField.getAttribute("value"));
            } else {
        ...
        if (!eDeltaField.getAttribute("xpath").equals("")) {
            if (eBaseField.getAttribute("xpath").trim().length() == 0) {
                eBaseField.setAttribute("xpath", eDeltaField.getAttribute("xpath"));
            } else {
        ...
        if (!eDeltaField.getAttribute("sortable").equals("")) {
            eBaseField.setAttribute("sortable", eDeltaField.getAttribute("sortable"));
        }
        ...
        if (!eDeltaField.getAttribute("classification").equals("")) {
            eBaseField.setAttribute("classification", eDeltaField.getAttribute("classification"));
        }
    }

    private void overwriteField(Element eBaseField, Element eDeltaField) {
        Node newField = eBaseField.getOwnerDocument().importNode(eDeltaField, true);
