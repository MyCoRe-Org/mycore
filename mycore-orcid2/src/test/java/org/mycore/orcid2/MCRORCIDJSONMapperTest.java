...
public class MCRORCIDJSONMapperTest {

    @Test
    public void testSerialization() {
        final MCRORCIDCredential credential = new MCRORCIDCredential("accessToken");
        credential.setTokenType("bearer");
        credential.setRefreshToken("refreshToken");
        credential.setScope("/read-limited");
        credential.setExpiration(LocalDate.now());
        final String credentialString = MCRORCIDJSONMapper.credentialToJSON(credential);
        LogManager.getLogger().info(credentialString);
        final MCRORCIDCredential result = MCRORCIDJSONMapper.jsonToCredential(credentialString);
