...
/**
 * This servlet create a MyCoRe classification JDOM tree based of a content 
 * in a directory selected by the <em>type</em> string in the file name and 
 * push them to the MCRLayoutServlet.
 * To select the directory it needs a property value named
 * <b>MCR.editor_<em>type</em>_directory</b>. The type is a part
 * of the MCRObjectID of the datasets.<br />
 * The categories are linear. Each category ID is the file name and the
 * text attribute is the file name without the extention <em>.xml</em>.<br />
 * Only users with the privileg <b>update object in datastore</b> get the file 
 * list. For others the classification is empty.<br /><br />
 * Call this servlet with <b>.../servlets/MCRFileListWorkflowServlet/XSL.Style=xml&type=...</b>
 *
  ...
  if ((dirname != null) && haspriv) {
    ...
    if (dirl != null) {
      for (int i=0; i< dirl.length;i++) {
        LOGGER.debug(dirl[i]);
        if (dirl[i].indexOf(type) != -1) { workfiles.add(dirl[i]); }
        }
      }
    }
