...
public class MCRStartEditorServlet extends MCRServlet {
	...
	public void doGetPost(MCRServletJob job) throws Exception {
		...
		if (!mytodo.equals("wnewobj") && !mytodo.equals("wnewder") && !mytodo.equals("waddfile") && !mytodo.equals("wdelfile") && !mytodo.equals("weditobj")
				&& !mytodo.equals("weditder") && !mytodo.equals("wdelobj") && !mytodo.equals("wdelder") && !mytodo.equals("wsetfile")
				&& !mytodo.equals("wsetlabel") && !mytodo.equals("wcommit") && !mytodo.equals("seditobj") && !mytodo.equals("seditder")
				&& !mytodo.equals("sdelobj") && !mytodo.equals("sdelder") && !mytodo.equals("snewder") && !mytodo.equals("scommitder")
				&& !mytodo.equals("saddfile") && !mytodo.equals("snewfile") && !mytodo.equals("sdelfile") && !mytodo.equals("ssetlabel")
		...
		if (mytodo.equals("seditobj")) {
			...
			sb = new StringBuffer();
			sb.append(getBaseURL()).append("servlets/MCRQueryServlet").append("?XSL.Style=editor").append("&mode=ObjectMetadata").append("&type=").append(
					mytype).append("&hosts=").append(CONFIG.getString("MCR.editor_baseurl", "local")).append("&query=/mycoreobject%5b@ID=%22")
					.append(mytfmcrid).append("%22%5d");
	 ...
	 */
	private final boolean checkAccess(String mcrid, String user, ArrayList privs, String action, boolean source) {
		...
		// the author.
		for (int i = 0; i < ar.size(); i++) {
			String primaryGroupID = MCRUserMgr.instance().getPrimaryGroupIDOfUser((String) ar.get(i));

			if (primaryGroupID != null) {
				MCRGroup primaryGroup = MCRUserMgr.instance().retrieveGroup(primaryGroupID);
				...
				for (int j = 0; j < allCurrentUserGroupIDs.size(); j++) {
					if (primaryGroup.hasGroupMember((String) allCurrentUserGroupIDs.get(j))) {
						return true;
					}
