...
public class MCRStartEditorServlet extends MCRServlet {
	...
	public void doGetPost(MCRServletJob job) throws Exception {
		...
		String cancelpage = pagedir
				+ CONFIG.getString("MCR.editor_page_cancel",
						"editor_cancel.xml");
		String commitpage = pagedir
				+ CONFIG.getString("MCR.editor_page_commit",
						"editor_loaded.xml");
		String deletepage = pagedir
				+ CONFIG.getString("MCR.editor_page_delete",
						"editor_delete.xml");
		String usererrorpage = pagedir
		...
		if (mytodo.equals("waddfile")) {
			if (!checkAccess(myremcrid, userid, privs, "create-" + mytype,
					false)) {
		...
		if (mytodo.equals("weditobj")) {
			if (!checkAccess(mysemcrid, userid, privs, "modify-" + mytype,
					false)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
		...
		if (mytodo.equals("wdelobj")) {
			if (!checkAccess(mysemcrid, userid, privs, "delete-" + mytype,
					false)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
		...
		if (mytodo.equals("wdelder")) {
			if (!checkAccess(myremcrid, userid, privs, "delete-" + mytype,
					false)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
		...
		if (mytodo.equals("wcommit")) {
			if (!checkAccess(mysemcrid, userid, privs, "commit-" + mytype,
					false)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
			...
			try {
				if (WFM.commitMetadataObject(mytype, mysemcrid)) {
					...
					sb = (new StringBuffer("MCR.type_")).append(mytype).append(
							"_in");
					...
					sb.append(searchtype).append(
							"&hosts=local&query=%2Fmycoreobject[%40ID%3D\'")
							.append(mysemcrid).append("\']");
					myfile = sb.toString();
				} else {
			...
			job.getResponse().sendRedirect(
					job.getResponse().encodeRedirectURL(getBaseURL() + myfile));
			return;
		}

		// action WEDITDER in the database
		if (mytodo.equals("weditder")) {
			if (!checkAccess(myremcrid, userid, privs, "modify-" + mytype,
					false)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
		...
		if (mytodo.equals("wsetfile")) {
			...
			sb = new StringBuffer();
			sb.append(pagedir).append("editor_").append(mytype).append(
					"_editor.xml");
			job.getResponse().sendRedirect(
					job.getResponse().encodeRedirectURL(
							getBaseURL() + sb.toString()));
		...
		if (mytodo.equals("wdelfile")) {
			...
			sb = new StringBuffer();
			sb.append(getBaseURL()).append(pagedir).append("editor_").append(
					mytype).append("_editor.xml");
			job.getResponse().sendRedirect(
					job.getResponse().encodeRedirectURL(sb.toString()));
			return;
		}

		// action SEDITOBJ in the database
		if (mytodo.equals("seditobj")) {
			if (!checkAccess(mytfmcrid, userid, privs, "commit-" + mytype, true)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
				return;
			...
			sb = new StringBuffer();
			sb.append(getBaseURL()).append("servlets/MCRQueryServlet").append(
					"?XSL.Style=editor").append("&mode=ObjectMetadata").append(
					"&type=").append(mytype).append("&hosts=").append(
					CONFIG.getString("MCR.editor_baseurl", "local")).append(
					"&query=/mycoreobject%5b@ID=%22").append(mytfmcrid).append(
					"%22%5d");
			String base = getBaseURL() + myfile;
			Properties params = new Properties();
			params.put("XSL.editor.source.url", sb.toString());
		...
		if (mytodo.equals("sdelobj")) {
			if (!checkAccess(mytfmcrid, userid, privs, "remove-" + mytype, true)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
				return;
		...
		if (mytodo.equals("sdelder")) {
			if (!checkAccess(myremcrid, userid, privs, "remove-" + mytype, true)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
				return;
			...
			try {
				...
				sb = (new StringBuffer("MCR.type_")).append(ID.getTypeId())
						.append("_in");
				String searchtype = CONFIG.getString(sb.toString(), ID
						.getTypeId());
				sb = new StringBuffer();
				sb.append("servlets/MCRQueryServlet?mode=ObjectMetadata&type=");
				sb.append(searchtype).append(
						"&hosts=local&query=%2Fmycoreobject[%40ID%3D\'")
						.append(myremcrid).append("\']");
				myfile = sb.toString();
			} catch (Exception e) {
				myfile = deleteerrorpage;
			}
		...
		if (mytodo.equals("snewder")) {
			if (!checkAccess(myremcrid, userid, privs, "commit-" + mytype, true)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
				return;
		...
		if (mytodo.equals("seditder")) {
			if (!checkAccess(mysemcrid, userid, privs, "commit-" + mytype, true)) {
				job.getResponse().sendRedirect(
						job.getResponse().encodeRedirectURL(
								getBaseURL() + usererrorpage));
				return;
		...
		if (mytodo.equals("ssetlabel")) {
			...
			MCRObjectID ID = new MCRObjectID(myremcrid);
			sb = new StringBuffer();
			sb = (new StringBuffer("MCR.type_")).append(ID.getTypeId()).append(
					"_in");
	 ...
	 * <ul>
	 * <li>the user is the owner of the object and his group has the work
	 * privileges</li>
	 * <li>the group of the user is member in the group of the author and the
	 * users grou has the privileg 'editor'</li>
	 * </ul>
	 * 
	 * @param mcrid
	 *            the MCRObjectID
	 ...
	 *            the user name
	 * @param privs
	 *            an ArrayList of the privileges of the user
	 * @param action
	 *            the action privileg that is expected
	 * @param source
	 *            true if the data come from the database, false ift it come
	 *            form the workflow
	 * @return true if the access is successful, else return false
	 */
	private final boolean checkAccess(String mcrid, String user,
			ArrayList privs, String action, boolean source) {
		// is the user the administrator
		if (user.equals("administrator")) {
			return true;
		}
		// has the user the privilege
		if (!privs.contains(action)) {
			return false;
		}
		...
		if (source) {
			...
			try {
				...
				LOGGER.debug("Parse error in file " + sb.toString());
				return false;
			}
		}
		if (jdom == null) {
		...
		// For all authors (users in the object servflags) we now check if the
		// current
		// user directly is a member of the primary group of the author or
		// implicitly
		// is a member of a group which itself is a member of the primary group
		// of
		// the author.
		...
		for (int i = 0; i < ar.size(); i++) {
			...
			if (primaryGroupID != null) {
				...
				for (int j = 0; j < allCurrentUserGroupIDs.size(); j++) {
					if (primaryGroup
							.hasGroupMember((String) allCurrentUserGroupIDs
									.get(j))) {
						return true;
					}
		...
		if ((ar.size() == 0) && (privs.contains("editor"))) {
			return true;
		}
		return false;
	}
