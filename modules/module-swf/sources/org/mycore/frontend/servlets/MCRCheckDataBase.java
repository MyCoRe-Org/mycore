...
abstract public class MCRCheckDataBase extends MCRCheckBase {
	...
	private final void errorHandlerValid(MCRServletJob job, ArrayList logtext,
		...
		String myfile = pagedir
				+ CONFIG.getString("MCR.editor_page_error_formular",
		...
		try {
			InputStream in = (new URL(getBaseURL() + myfile + "?XSL.Style=xml"))
					.openStream();
			if (in == null)
				throw new MCRConfigurationException("Can't read editor file "
						+ myfile);
			...
			for (int i = 0; i < sectionlist.size(); i++) {
				org.jdom.Element section = (org.jdom.Element) sectionlist
						.get(i);
				if (!section.getAttributeValue("lang",
						org.jdom.Namespace.XML_NAMESPACE).equals(
						lang.toLowerCase())) {
					continue;
				}
				...
				form.setAttribute("action", (new URL(getBaseURL()))
						+ "servlets/MCRStartEditorServlet");
			...
			throw new MCRException("Can't read editor file " + myfile
