...
public class MCRUserEditorServlet extends MCRUserAdminGUICommons {
	private static final Logger LOGGER = Logger
			.getLogger(MCRUserEditorServlet.class);
	...
	public void doGetPost(MCRServletJob job) throws IOException,
		...
		ArrayList currentPrivs = MCRUserMgr.instance()
				.retrieveAllPrivsOfTheUser(currentUserID);
	 ...
	 *             for errors from the servlet engine.
	 */
	private void getAssignableGroupsForUser(MCRServletJob job,
			ArrayList currentPrivs) throws IOException, ServletException {
		...
		try {
			...
			// current user.
			MCRUser currentUser = MCRUserMgr.instance().retrieveUser(
					currentUserID);
			if (currentPrivs.contains("user administrator")) {
				...
				LOGGER
						.warn("MCRUserEditorServlet: not enough privileges! "
		...
		for (int i = 0; i < groupIDs.size(); i++) {
			org.jdom.Element item = new org.jdom.Element("item").setAttribute(
					"value", (String) groupIDs.get(i)).setAttribute("label",
	...
	private void getEditorSubmission(MCRServletJob job) throws IOException,
		...
		if ((useCase.equals("create-user") || useCase.equals("modify-user"))
			...
			String numID = Integer.toString(MCRUserMgr.instance()
					.getMaxUserNumID() + 1);
			jdomDoc.getRootElement().getChild("user").setAttribute("numID",
					numID);
			if (jdomDoc.getRootElement().getChild("user").getAttributeValue(
					"id_enabled") == null) {
				jdomDoc.getRootElement().getChild("user").setAttribute(
						"id_enabled", "false");
			}
			if (jdomDoc.getRootElement().getChild("user").getAttributeValue(
					"update_allowed") == null) {
			...
			org.jdom.Element userElement = jdomDoc.getRootElement().getChild(
					"user");
			...
			try {
				if (useCase.equals("create-user")) {
					...
					LOGGER.info("User " + currentUserID
							+ " has successfully created the new user: "
							+ newUser.getID());
				} else {
