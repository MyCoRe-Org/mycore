...
public class MCRUserAdminServlet extends MCRUserAdminGUICommons {
	...
	private static final Logger LOGGER = Logger
			.getLogger(MCRUserAdminServlet.class);
	...
	private void listallUser(MCRServletJob job, ArrayList currentPrivs)
		...
		String UserServlet = getBaseURL()
				+ "servlets/MCRUserEditorServlet?mode=retrievealluserxml";
		job.getResponse().sendRedirect(
				job.getResponse().encodeRedirectURL(UserServlet));
		return;
	}
	...
	private void modifyUser(MCRServletJob job, ArrayList currentPrivs)
		...
		Properties params = new Properties();
		params
				.put("XSL.editor.source.url",
						"servlets/MCRUserEditorServlet?mode=retrieveuserxml&uid="
								+ uid);
		params.put("XSL.editor.cancel.url", getBaseURL() + cancelPage);
