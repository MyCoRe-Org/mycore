...
public class MCRUserAdminGUICommons extends MCRServlet {
	...
	public void init() throws ServletException {
		...
		noPrivsPage = pageDir
				+ CONFIG.getString("MCR.useradmin_page_error_privileges",
						"useradmin_error_privileges.xml");
		cancelPage = pageDir
				+ CONFIG.getString("MCR.useradmin_page_cancel",
						"useradmin_cancel.xml");
		okPage = pageDir
				+ CONFIG.getString("MCR.useradmin_page_ok", "useradmin_ok.xml");
	}
	...
	protected void showOkPage(MCRServletJob job) throws IOException,
		...
		job.getResponse().sendRedirect(
				job.getResponse().encodeRedirectURL(getBaseURL() + okPage));
		return;
	}
	 ...
	 *             for java I/O errors.
	 */
	protected void doLayout(MCRServletJob job, String styleSheet,
			Document jdomDoc, boolean useStrict) throws ServletException,
			IOException {
