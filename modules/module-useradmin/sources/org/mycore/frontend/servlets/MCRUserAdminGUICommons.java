...
/**
 * This servlet provides some common methods for the editors of the user management 
...
public class MCRUserAdminGUICommons extends MCRServlet
{
    ...
    { 
        ...
        noPrivsPage = pageDir+CONFIG.getString("MCR.useradmin_page_error_privileges", 
                      "useradmin_error_privileges.xml");
        cancelPage  = pageDir+CONFIG.getString("MCR.useradmin_page_cancel", 
                      "useradmin_cancel.xml");
    ...
    /**
     * This method simply redirects to a page providing information that the privileges
     * for a use case are not sufficient.
     * 
    ...
    throws IOException, ServletException
    {
        job.getResponse().sendRedirect(job.getResponse()
           .encodeRedirectURL(getBaseURL()+noPrivsPage));
        return;      
    }
    
    /**
     * This method simply redirects to a page providing information that the current
     * use case was fulfilled successfully.
     * 
     * @param  job  The MCRServletJob instance
     */
    ...
    {
        job.getResponse().sendRedirect(job.getResponse()
           .encodeRedirectURL(getBaseURL()+okPage));
        return;      
    }
    
    /**
     * This methods forwards a jdom document with XSL.Style=xml using the MCRLayoutServlet
     * 
     * @param  jdomDoc
     ...
     *             for java I/O errors.
     */
    protected void forwardXML(MCRServletJob job, Document jdomDoc)
    ...
    /**
     * Gather information about the XML document to be shown and the corresponding 
     * XSLT stylesheet and redirect the request to the LayoutServlet
     * 
     * @param job
     ...
     * @param useStrict
     *            If true, the parameter styleSheet must be used directly as name of a
     *            stylesheet when forwarding to the MCRLayoutServlet. If false, styleSheet 
     *            will be appended by the signature of the current language. useStrict=true
     *            is used when not using a stylesheet at all because one simply needs
     *            the raw XML output.
     *  
     * @throws ServletException
     *             for errors from the servlet engine.
    ...
    throws ServletException, IOException
    {
        ...
        if (!useStrict) styleSheet = styleSheet + "-" + language;
