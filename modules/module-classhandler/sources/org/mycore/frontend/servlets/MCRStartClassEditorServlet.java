...
public class MCRStartClassEditorServlet extends MCRServlet  {
  ...
  public void doGetPost(MCRServletJob job) throws Exception    {
    ...
    // read the parameter
    todo = parms.getParameter("todo"); //getProperty(job.getRequest(), "todo");
    todo2 = parms.getParameter("todo2"); //getProperty(job.getRequest(), "todo2");
    path = parms.getParameter("path");  //getProperty(job.getRequest(), "path");
    
    
    // get the Classification 
    clid = parms.getParameter("clid"); //getProperty(job.getRequest(), "clid");
    categid = parms.getParameter("categid"); //getProperty(job.getRequest(), "categid");
    ...
    if ("commit-classification".equals(todo)) {
    	...
    	if ( "create-category".equals(todo2) || "modify-category".equals(todo2) ) {
        	...
        	if ( path.indexOf("&clid") > 0 ) {
    			// Classification abschneiden um wieder auf der Classifikationsstartseite zu landen
    ...
    // beim ersten Aufruf, direkt ohnen editor
		...
		if ( cnt == 0) { //ok - gelöscht , da keine referenzen auf Category vorhanden
			job.getResponse().sendRedirect(job.getResponse().encodeRedirectURL(path+"&clid="+clid));
		} else {		    // nicht gelöscht, da noch Referenzen auf Category vorhanden
