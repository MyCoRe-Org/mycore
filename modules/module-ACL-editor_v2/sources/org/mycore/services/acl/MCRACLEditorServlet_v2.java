...
public class MCRACLEditorServlet_v2 extends MCRServlet {
    ...
    public void doGetPost(MCRServletJob job) throws Exception {
        ...
        if (mode == null) {
            ...
            answer.addContent(getFilterElem(request));
        ...
        } else if (mode.equals("deleteFilter")) {
            // rules in item form for dropdown list in permission editor
            String redirectURL = getServletBaseURL() + "MCRACLEditorServlet_v2?mode=getACLPermissions";
    ...
    private void setFilter(HttpServletRequest request, HttpServletResponse response) {
        ...
        redirect(response, buildRedirectURL(redirectURL, parameters));
    }

    private Element getFilterElem(HttpServletRequest request) {
        MCRACLXMLProcessing XMLProcessing = new MCRACLXMLProcessing();
        String objidFilter = request.getParameter("objid");
        String acpoolFilter = request.getParameter("acpool");
        Element answer = XMLProcessing.accessFilter2XML(objidFilter, acpoolFilter);
        return answer;
    }
    ...
    private Element createNewPermission(HttpServletRequest request, HttpServletResponse response) {
        ...
        String xmlUrl = "webapp:modules/module-ACL-editor/web/editor/editor-ACL_Permissions.xml";
        Element permissionEdit = MCRURIResolver.instance().resolve(xmlUrl);
        permissionEdit.removeChildren("source");

        Element source = new Element("source");
        StringBuffer uri = new StringBuffer("request:servlets/MCRACLEditorServlet_v2?mode=getNewPermission");
        ...
        permissionEdit.addContent(0, source);

        String aclDebug = request.getParameter("aclDebug");
        if (aclDebug == null)
            aclDebug = "null";

        if (aclDebug.equals("elem"))
            try {
                doLayout(request, response, permissionEdit, "xml", false);
            } catch (IOException e) {
        ...
        editor.addContent((Element) permissionEdit.clone());
        webPage.addContent(editor);

        Document doc = new Document();
        doc.setRootElement(webPage);
        doc.setDocType(new DocType("MyCoReWebPage"));
        if (aclDebug.equals("docBef"))
            try {
                doLayout(request, response, doc, "xml");
            } catch (IOException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }

        MCREditorServlet.replaceEditorElements(request, xmlUrl, doc);

        if (aclDebug.equals("docAfter"))
            try {
                doLayout(request, response, doc, "xml");
            } catch (IOException e) {
                // TODO Auto-generated catch block
        ...
        // editor.addContent((Element)doc.getRootElement().clone());
