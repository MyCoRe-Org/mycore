...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/mcr-PermissionsEditor">
		    <xsl:call-template name="include.editor">
		        <xsl:with-param name="uri" select="'webapp:modules/module-ACL-editor/web/editor/editor-ACL_start.xml'" />
		        <xsl:with-param name="ref" select="'ACL-PermissionEditor'" />
		    </xsl:call-template>
	</xsl:template>
</xsl:stylesheet>