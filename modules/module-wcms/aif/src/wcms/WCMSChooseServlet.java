...
public class WCMSChooseServlet extends WCMSServlet {
	...
	private String label = null; //representing the label(name) of the selected
								 // link
	...
	private String addAtPosition = null; //position, where a link can be added:
	...
	protected void processRequest(HttpServletRequest request,
		...
		File[] contentTemplates = new File(super.CONFIG
				...
				+ "content/".replace('/', File.separatorChar)).listFiles();
		File[] masterTemplates = new File(super.CONFIG
				.getString("MCR.WCMS.templatePath")
				+ "master/".replace('/', File.separatorChar)).listFiles();
		template = request.getParameter("template");
		File conTemp = new File(super.CONFIG.getString("MCR.WCMS.templatePath")
				+ "content/".replace('/', File.separatorChar));
		...
		if (mcrSession != null) {
			defaultLang = MCRConfiguration.instance().getString(
					"MCR.metadata_default_lang", "en").toLowerCase();
		...
		try {
			...
			Document doc = builder
					.build(super.CONFIG.getString("MCR.WCMS.navigationFile")
		 ...
		 */
		if ((action.equals("edit") && c == '1') || action.equals("add")
				|| (action.equals("translate") && c == '1')) {
			...
			try {
				...
				if (!action.equals("add") && mode.equals("intern"))
					ed = sax.build(getServletContext().getRealPath("") + fs
							+ href);
				else {
					//if (mode.equals("intern")) {
					ed = sax.build((super.CONFIG
				...
				while (contentElementsIterator.hasNext()) {
					...
					if (content.getAttributeValue("lang", ns) != null) {
						...
						validXHTML = true;
						if (content.getAttributeValue("lang", ns).equals(
								defaultLang)) {
				...
				try {
					if (!mode.equals("extern")) {
						error = "Error: " + getServletContext().getRealPath("")
								+ href.replace('/', File.separatorChar)
								+ " is no valid XHTML file.";
						File hrefFile = new File(getServletContext()
								.getRealPath("")
								+ href);
						...
						String wow = stri.substring(begin, stri
								.indexOf("</section>"));
						String strin = stri.substring(begin + wow.indexOf('>')
								+ 2, stri.indexOf("</section>"));
		...
		if (c != '1' && c != '2' || contentError != null) {
			...
			if (c != '1' && c != '2') {
				rootOut.addContent(new Element("error").setText(String
						.valueOf(c)));
			} else
				rootOut.addContent(new Element("contentError").setAttribute(
						"msg", contentError).setText("yes"));
		...
		if (action.equals("translate") && mode != null) {
			rootOut.addContent(new Element("action").setAttribute("mode", mode)
					.setText(action));
			rootOut.addContent(new Element("label_currentLang")
					.setText(label_currentLang));
		}
		...
		if (defaultLangContentOutput != null) {
			try {
				if (validXHTML) {
					...
					XMLOutputter contentOut = new XMLOutputter(Format
							.getRawFormat().setTextMode(
									Format.TextMode.PRESERVE).setEncoding(
									OUTPUT_ENCODING));
					...
					rootOut.addContent(new Element("content").setText(sw
							.toString()));
					sw.flush();
					sw.close();
					...
					if (action.equals("translate")
						...
						rootOut.addContent(new Element("content_currentLang")
								.setText(sw.toString()));
						sw.flush();
						sw.close();
					}
				} else {
					rootOut.addContent(new Element("content")
							.setText((String) defaultLangContentOutput.get(0)));
					rootOut.addContent(new Element("error")
							.setText("invalidXHTML"));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		File imagePath = new File((super.CONFIG.getString("MCR.WCMS.imagePath")
				.replace('/', File.separatorChar)));
		if (!imagePath.exists())
			imagePath.mkdirs();
		...
		for (int i = 0; i < imageList.length; i++) {
			if (!imageList[i].isDirectory()) {
				images.addContent(new Element("image").setText(imageList[i]
						.getName()));
			}
		}

		rootOut.addContent(new Element("imagePath").setText(super.CONFIG
				.getString("MCR.WCMS.imagePath").substring(
						super.CONFIG.getString("MCR.WCMS.imagePath")
								.lastIndexOf("webapps"))));

		Element documents = new Element("documents");
		rootOut.addContent(documents);
		File documentPath = new File((super.CONFIG
				.getString("MCR.WCMS.documentPath").replace('/',
				File.separatorChar)));
		...
		for (int i = 0; i < documentList.length; i++) {
			if (!documentList[i].isDirectory()) {
				documents.addContent(new Element("document")
						.setText(documentList[i].getName()));
			}
		}
		rootOut.addContent(new Element("documentPath").setText(super.CONFIG
				.getString("MCR.WCMS.documentPath").substring(
						super.CONFIG.getString("MCR.WCMS.documentPath")
								.lastIndexOf("webapps"))));
		...
		while (elementIterator.hasNext()) {
			...
			if (child.getAttribute("href") != null) {
				if (child.getAttributeValue("href").equals(href)) {
					...
					if (getLabel(child, defaultLang) != null
							&& !getLabel(child, defaultLang).equals(""))
						label = getLabel(child, defaultLang);
					if (getLabel(child, currentLang) != null
							&& !getLabel(child, currentLang).equals(""))
