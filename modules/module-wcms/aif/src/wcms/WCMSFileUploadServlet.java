...
public class WCMSFileUploadServlet extends WCMSServlet {
	...
	private String fileName, filePath, contentType, fileContentType, error,
			status, action = null;

	private String documentPath = super.CONFIG.getString(
			"MCR.WCMS.documentPath").replace('/', File.separatorChar);

	private String imagePath = super.CONFIG.getString("MCR.WCMS.imagePath")
		...
		if (action.equals("upload")) {
			...
			if (fileSize == 0) {
				...
				throw new ServletException(
						"No legal request object. Please select a valid File for uploading.");
			}
						...
						if (newLine.indexOf("filename=\"") != -1) {
							...
							while (i != -1 && !newLine.startsWith(boundary)) {
								...
								if ((i == boundaryLength + 2 || i == boundaryLength + 4) // + 4
										...
										&& (new String(line, 0, i)
												.startsWith(boundary)))
									pw.print(newLine.substring(0, newLine
											.length() - 2));
								else
									pw.print(newLine);
								newLine = new String(line, 0, i);
			...
			rootOut.addContent(new Element("userID")
					.setText((String) mcrSession.get("userID")));
			rootOut.addContent(new Element("userClass")
					.setText((String) mcrSession.get("userClass")));
			...
			rootOut
					.addContent(new Element("path")
							.setText(savePath + fileName));
