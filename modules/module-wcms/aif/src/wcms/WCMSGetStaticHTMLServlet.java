...
public class WCMSGetStaticHTMLServlet extends WCMSServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		try {
			...
			Document ed = sax.build(getServletContext().getRealPath("") + fs
					+ href);
			...
			if (contentOutput != null && validXHTML) {
				...
				doc
						.setDocType(new DocType("html",
								"-//W3C//DTD XHTML 1.0 Transitional//EN",
								"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"));
				Element root = new Element("html");
				Element meta1 = new Element("meta").setAttribute("http-equiv",
						"Pragma").setAttribute("content", "no-cache");
				Element meta2 = new Element("meta").setAttribute("http-equiv",
						"Cache-Control").setAttribute("content",
						"no-cache, must-revalidate");
				root
						.addContent(
								new Element("head").addContent(meta1)
										.addContent(meta2)).addContent(
								new Element("body").addContent(contentOutput));
				doc.setRootElement(root);
				XMLOutputter xmlout = new XMLOutputter(Format.getRawFormat()
						.setEncoding(OUTPUT_ENCODING));
				ServletOutputStream sos = response.getOutputStream();

				response
						.setContentType("text/html; charset=" + OUTPUT_ENCODING);
