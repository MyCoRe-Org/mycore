...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xalan"
    ...
    xmlns:layoutUtils="xalan:///org.mycore.frontend.MCRLayoutUtilities" xmlns:acl="xalan://org.mycore.access.MCRAccessManager">

    <xsl:variable name="manage-read" select="wcmsUtils:getPermRightsManagementReadAccess()" />
    <xsl:variable name="manage-wcms" select="wcmsUtils:getPermRightsManagementWCMSAccess()" />
    <xsl:variable name="read" select="layoutUtils:getPermission2ReadWebpage()" />
    <xsl:variable name="write" select="wcmsUtils:getWritePermissionWebpage()" />
    <!-- =================================================================================================== -->
    <!--
    ...
    <xsl:template name="wcms.getBrowserAddress">
        ...
        <xsl:variable name="browserAddress_href">
            ...
            <xsl:for-each select="$loaded_navigation_xml//item[@href]">
                ...
                <xsl:if
                    test="( $RequestURL.langDel = $href.langDel )
                              or
                    ...
                    <ul class="menu">
                        ...
                        <li>
                            <script type="text/javascript">
                                function OpenWindow (address) { MyWindow = window.open(address, "Multimedia", "width=400,height=350,scrollbars=yes");
                                MyWindow.window.moveTo(screen.width*0.5,screen.height*0.2); MyWindow.focus(); }
                            </script>
                            <a href="{$WebApplicationBaseURL}modules/module-wcms/aif/web/multimedia.xml{$JSessionID}"
                        ...
                        <xsl:if test="acl:checkPermission($manage-read)">
                            <li>
                                <a href="{$ServletsBaseURL}MCRWCMSAdminServlet{$JSessionID}?action={$manage-read}">

                                    <xsl:if test="$menupunkt=i18n:translate('wcms.rightsManag_read')">
                                        <xsl:attribute name="class">
                                    <xsl:value-of select="'current'" />
                                    </xsl:attribute>
                                    </xsl:if>
                                    <xsl:value-of select="i18n:translate('wcms.rightsManag_read')" />
                                </a>
                            </li>
                        </xsl:if>
                        <!-- management of write access -->
                        <xsl:if test="acl:checkPermission($manage-wcms)">
                            <li>
                                <a href="{$ServletsBaseURL}MCRWCMSAdminServlet{$JSessionID}?action={$manage-wcms}">
                                    <xsl:if test="$menupunkt=i18n:translate('wcms.rightsManag_write')">
                                        <xsl:attribute name="class">
                                    <xsl:value-of select="'current'" />
                                    </xsl:attribute>
                                    </xsl:if>
                                    <xsl:value-of select="i18n:translate('wcms.rightsManag_write')" />
                                </a>
                            </li>
                        </xsl:if>
                    </ul>
    ...
    <xsl:template name="zeigeSeitenname">
        ...
        <div id="seitenname">
            <h3>
                <xsl:value-of select="$seitenname" />
            </h3>
        </div>
    </xsl:template>
