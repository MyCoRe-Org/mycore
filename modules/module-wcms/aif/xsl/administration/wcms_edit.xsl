...
<form name="editContent" action="{$ServletsBaseURL}WCMSActionServlet" method="post" onSubmit="return chkFormular()" > 
 ...
 <table width="90%" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr>
      <!-- label path -->      
      <td width="70%"><b>Seite:  </b> <xsl:value-of select="concat('  ',$labelPath)"/>
    ...
    <xsl:when test=" /cms/error = 'emptyFormField' " >
	  <tr>
	 	  <th colspan="2" class="red" align="left">
			Fehler aufgetreten: Formular nicht vollständig ausgefüllt. <br/><br/>
			Mindestens ein Formularfeld wurde nicht ausgefüllt. 
		  </th>	  
	  </tr>
	  <tr>
	     ...
	     <table cellspacing="0" cellpadding="0" align="left">
		   <tr>
		     <th class="red" width="100">
			   <a href="{$ServletsBaseURL}WCMSActionServlet?delete=true&amp;labelPath={$labelPath}">löschen</a>
			 </th>
		     <td width="50"></td>
  ...
  <tr>
    <td colspan="2">
        <table width="100%" class="table_noGrid" border="0" cellspacing="0" cellpadding="0">
          ...
          <!-- head line -->
		  <tr><td align="center" class="green" colspan="2"><b>Metadaten</b></td></tr>
...
<xsl:template name="buildInterface.general.metaData.abstract">
  ...
  <span style="text-decoration:line-through;">    
	  <xsl:choose>
	      ...
	      <xsl:when test="/cms/action = 'translate'">
	        Kurzbeschreibung (<xsl:value-of select="$DefaultLang"/>):<br/>
	            "<xsl:value-of select="/cms/abstract"/>"
	        <br/><br/>
	        übersetzte Kurzbeschreibung (<xsl:value-of select="$CurrentLang"/>):<br/>
	          <textarea name="abstract_currentLang" rows="5" cols="60" wrap="off" />         
	      </xsl:when>
	      <xsl:otherwise>
	        Kurzbeschreibung:<br/><textarea name="abstract" rows="4" cols="60" wrap="off" />         
	      </xsl:otherwise>
	  </xsl:choose>
   </span>           
...
<xsl:choose>
    <xsl:when test="/cms/action = 'edit' or /cms/action = 'add' ">        
	  ...
	  <br/>
	  	  ...
	  	  <span style="text-decoration:line-through;">
			  Zugriff auf Gruppe beschränken:
	       </span>        		  
...
<xsl:choose>
    <xsl:when test="/cms/action = 'edit' ">        
        ...
        <input type="checkbox" name="" value="" />        
	  <span style="text-decoration:line-through;">
	        automatisch auf Unterseite weiterleiten:
	   </span>
	   <input type="text" size="10" maxlength="40" name="" /> 				
...
<xsl:template name="buildInterface.general.metaData.label">
    ...
    <xsl:choose>
        <xsl:when test="/cms/action='translate'">
                Titel (<xsl:value-of select="$DefaultLang"/>):<br/>    
           		    "<xsl:value-of select="/cms/label" />"
                <br/><br/>
                übersetzter Titel (<xsl:value-of select="$CurrentLang"/>):<br/>    
	   ...
	   <select name="style" size="1">
		<xsl:choose>
		  ...
		  <xsl:otherwise>
				...
				<option selected="selected" value="bold">
				  fett
				</option>					
		  </xsl:otherwise>
		</xsl:choose>			
        ...
        Template:    
                  <!-- set template -->
        				...
        				<xsl:variable name="template_temp" >
        				  <!-- point to rigth item -->
<xsl:for-each select="document($navigationBase) /navigation//item[@href = $href]" > <!-- collect @template !='' entries along the choosen axis -->	 
	<xsl:for-each select="ancestor-or-self::*[  @template != '' ]"> <xsl:if test="position()=last()" > <xsl:value-of select="@template" 
	/> </xsl:if> </xsl:for-each> <!-- END OF: collect @template !='' entries along the choosen axis --> </xsl:for-each>
...
<xsl:otherwise> <xsl:value-of select="document($navigationBase) /navigation/@template" 	/> 
	</xsl:otherwise>
        			  			</xsl:choose>								
        				</xsl:variable>				
...
<xsl:for-each select="document($navigationBase) /navigation//item[@href = $href]" > 	<xsl:choose> 
	<xsl:when test=" @template != '' " > <xsl:value-of select="'yes'" /> </xsl:when> <xsl:otherwise> <xsl:value-of select="'no'" /> 
        				...
        				<xsl:variable name="templateParent" >
        				  <!-- point to rigth item -->
<xsl:for-each select="document($navigationBase) /navigation//item[@href = $href]" > <!-- collect @template !='' entries along the choosen axis -->	 
	<xsl:for-each select="ancestor-or-self::*[  @template != '' ]"> <xsl:if test="position()=last()-1" > <xsl:value-of select="@template" 
	/> </xsl:if> </xsl:for-each> <!-- END OF: collect @template !='' entries along the choosen axis --> </xsl:for-each>
        				  <!-- END OF: point to rigth item -->       
        				</xsl:variable>
        																...
        																<xsl:choose>
        																	    ...
        																	    <xsl:when test=" node() = $template " >
        																				<option selected="selected">						    
        																					...
        																					<xsl:value-of select="node()" /> (Vorlage) 
        																				</option>					
        																		</xsl:when>
        													    ...
        													    <xsl:when test=" /cms/action = 'edit' " >
        																<xsl:choose>
        																	    ...
        																	    <xsl:when test=" node() = $template " >
        																				<option selected="selected">						    
        																					<xsl:attribute name="value"><xsl:value-of select="'noAction'" /></xsl:attribute>
        																					<xsl:value-of select="node()" /> (hier gesetzt) 
        																				</option>					
        																		</xsl:when>
        																		<!-- END OF: current template is the valid template -->																
        																	    <xsl:when test=" node() = $templateParent " >
    ...
    <tr>
	  <td colspan="2"> 
        ...
        <table width="100%" class="table_nogrid" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td colspan="2" align="center" class="green" valign="bottom"><b>Editor zum Inhalt der  Seite:</b></td>
          </tr>
...
<xsl:if test="/cms/action = 'edit'">
    ...
    <br/><br/><br/>Was wurde verändert:<br/><textarea name="changeInfo" rows="2" cols="40" wrap="off" /></xsl:if>
        ...
        Dynamischer Inhalt:<br/><br/>
            ...
            <input type="checkbox" name="dcbActionAdd" value="dcbActionAdd" />XML binden:<input type="text" size="30" maxlength="40" name="dcbValueAdd" /> 				
        	  ...
        	  <xsl:if test=" /cms/action = 'edit' and $dcb = 'yes' " >
        	      <br/>
          		  <input type="checkbox" name="dcbActionDelete" value="dcbActionDelete" />XML lösen:
...
<xsl:for-each select="document($navigationBase) /navigation//item[@href = $href]/dynamicContentBinding/rootTag" 
	> <option> <xsl:value-of select="." /> </option> </xsl:for-each>
        		  </select>											  
        	  </xsl:if>
...
<xsl:template name="buildInterface.general.content.validateXHTML">
	...
	<xsl:if test=" /cms/error = 'invalidXHTML' " >
		<table>
			<tr>
				<td align="left" class="red"> <b> Achtung: <br/> </b> Der Inhalt der Seite ist nicht XHTML konform! Die Seite wird 
					dadurch zwar gespeichert, kann aber nicht angezeigt werden. <br/> <b>Tip:</b> Nutzen sie <a target="_blank" 
					href="http://validator.w3.org/" >XHTML-Syntaxprüfer</a>, um die Seite XHTL konform zu machen. </td>
			</tr>
		</table>
	</xsl:if>
...
<xsl:if test="/cms/action[@mode = 'extern'] and /cms/action = 'add'">
    Linkadresse:<br/><input class="inputfield" type="text" size="120" maxlength="256" name="href"> 
	      <xsl:attribute name="value" />
	  </input>	
</xsl:if>    
...
<xsl:if test="/cms/action[@mode='intern'] = 'add' or /cms/action[@mode='intern'] = 'edit'">    
        ...
        <table align="right" class="table" border="1" cellspacing="0" cellpadding="0">
		  <tr>
		    <td align="center" class="green" colspan="2"><b>Multimedia:</b></td>
		  </tr>	
			  ...
			  <tr>
				  ...
				  Bilder:<br/>
    		        ...
    		        <select size="1" name="selectPicturePreview" onchange="previewPicture(document.editContent.JavaScriptImagePath_hidden.value)">
        				...
        				<xsl:for-each select="/cms/images/image" >
        					<option>
        			          <xsl:attribute name="value"><xsl:value-of select="node()" /></xsl:attribute>					 						  <xsl:value-of select="node()" />
        					</option>		
        				</xsl:for-each>				
    		        </select>
	            ...
	            <!-- END OF: list of available documents -->			        			      
			       ...
			       <!-- upload --> 
			        <br/><br/>
			       <a target="blank" href="{$ServletsBaseURL}WCMSFileUploadServlet?action=select">Bild oder sonst. Dokument hochladen</a>
				...
				<th align="right" valign="top">
				  <img name="image" width="120" height="120" border="0" alt="" title="">				
			        <xsl:attribute name="src">
			            <xsl:value-of select="concat($WebApplicationBaseURL,substring-after(/cms/imagePath,'/'),/cms/images/image)" /></xsl:attribute>							 
				  </img>			  
				</th>
	          </tr>
...
<xsl:template name="buildInterface.general.content.showExistingHREF">
 <xsl:param name="href"/>
    ...
    <xsl:if test="/cms/action[@mode = 'extern'] and /cms/action = 'edit'">
    	  Linkadresse:<br/><input class="inputfield" type="text" size="120" maxlength="256" name="href"> 
    	      <xsl:attribute name="value">
