          ...
          <table class="table_noGrid" width="100%" cellspacing="0" cellpadding="0" border="0" align="center">    
            ...
            <tr>
        	  <td class="green_noBorder" align="center" colspan="2">
        	   <b>
                Bitte legen sie zunächst fest, was sie machen wollen und auf welche Seite sich diese Aktion beziehen soll
        	   </b>
              ...
              <tr>
                ...
                <th align="left" valign="top">
                    ...
                    <xsl:call-template name="addAtPosition" />
                    <xsl:call-template name="template" />
                    <br/><br/>
            	  	&lt;m&gt; ... neues Menü<br/>
            	  	&lt;t&gt; ... neues Template<br/>
            	  	&lt;d&gt; ... dynamischer Inhalt zugewiesen<br/>  
                    <xsl:if test="$CurrentLang != $DefaultLang">
                        <br/><br/>                        
        		  	    &lt;!&gt; ... Seite bisher nicht übersetzt<br/>                                                                      </xsl:if>
                </th>
                <!-- right column -->
                <th align="left" valign="top">
                    Inhalt:<br/>
...
<xsl:template name="siteStructureInfo">

&lt;<xsl:if test="@replaceMenu='true' " >m</xsl:if><xsl:if test="@template != '' " >t</xsl:if><xsl:if test=" count(child::dynamicContentBinding) &gt; 0 " >d</xsl:if>&gt;
       ...
       <b>
		<xsl:choose>
		  <xsl:when test=" /cms/error = '0' or /cms/error = '9' " >
			Fehler aufgetreten: Keine Seite ausgewählt. <br/><br/>
			Bitte wählen sie in der 2. Box die Seite aus, die sie bearbeiten wollen.		
		  </xsl:when>
		  <xsl:when test=" /cms/error = '5' " >
			Fehler aufgetreten: Anlegen nicht möglich. <br/><br/>
			Auf dieser Ebene können keine weiteren Menüpunkte angelegt werden. Wählen sie beim Erstellen die Position "darunter" aus.		
		  </xsl:when>
...
<xsl:template name="chooseAction">
    ...
    Aktion:<br/>    
   <select name="action" size="5" onchange="setHelpText()" >
     <xsl:if test="(/cms/userClass = 'admin' or /cms/userClass = 'editor') and $CurrentLang=$DefaultLang" > 
		<option selected="selected" value="edit" >- vorhandenen Inhalt bearbeiten</option>		
	  </xsl:if>
	  <xsl:if test="(/cms/userClass = 'autor'  or /cms/userClass = 'editor' or /cms/userClass = 'admin') and $CurrentLang=$DefaultLang" >
		<option value="add_intern">- neue Web-Seite einpflegen</option>		
	  </xsl:if>
	  <xsl:if test="(/cms/userClass = 'autor' or /cms/userClass = 'editor' or /cms/userClass = 'admin') and $CurrentLang=$DefaultLang" >
		<option value="add_extern">- neuen Link einpflegen</option>		
	  </xsl:if>			  
      <xsl:if test="/cms/userClass = 'admin' and $CurrentLang=$DefaultLang" > 
		<option value="delete">- Inhalt in Archiv verschieben</option>		
      </xsl:if>			  			  
      <xsl:if test="$DefaultLang != $CurrentLang" > 
		<option value="translate">- Inhalt übersetzen</option>		
	  </xsl:if>			  			         
   </select>		              
					...
					<xsl:if test="position() = 2 " >
							...
							<xsl:choose>
							  ...
							  <xsl:otherwise>
									...
									<xsl:for-each 
										select="document($navigationBase)//item">
										<xsl:if test="$myRootNode = @href">
                                          ...
                                          <xsl:value-of select="concat(@label,$labelPath)" />	                                        														    </option>								
							  </xsl:otherwise>
							</xsl:choose>	
							<!-- test if user is allowed to select this menu point -->	
						<option value="9">
					...
					<xsl:if test="@href" >
						<option value="1{@href}">
							...
							<xsl:choose>
								...
								<xsl:when test="($CurrentLang != $DefaultLang) and $label_curLang != ''"> <xsl:value-of 
									select="./label[lang($CurrentLang)]" /> (<xsl:value-of select="./label[lang($DefaultLang)]" 
									/>) </xsl:when>
								<!-- not default lang and label NOT translated -->
								<xsl:when test="($CurrentLang != $DefaultLang) and $label_curLang = ''"> &lt;!&gt; (<xsl:value-of 
									select="./label[lang($DefaultLang)]" />) </xsl:when>
								<xsl:otherwise>
...
<xsl:choose>    
    <xsl:when test="$CurrentLang=$DefaultLang">
        <br/><br/><br/>
        An welcher Position: (nur bei neuem Inhalt)<br/>    
        <select name="addAtPosition" size="3" >
...
-->    
            <br/><br/><br/>
            Content-Template: (nur bei neuem Inhalt)<br/>    
