    ...
    <xsl:if test="(contains($User-Agent, 'Trident/') or contains($User-Agent,'MSIE ')) and $msie-banner = 'enabled'">
      <div class="alert alert-light p-2 unsupported-browser" role="alert">
        <div class="container clearfix d-flex justify-content-center py-2">
          <svg xmlns="http://www.w3.org/2000/svg" class="flex-self-start flex-shrink-0 pt-1 me-2" aria-hidden="true"
          ...
          <div class="d-flex flex-auto flex-row">
            <div class="flex-auto min-width-0 me-2">
              <h5>
                <xsl:value-of select="i18n:translate('component.webtools.msie.headline')"/>
              </h5>
              <p class="m-0">
                <xsl:value-of select="concat(i18n:translate('component.webtools.msie.recommendation'),' ')"/>
                <a href="https://www.microsoftedge.com" class="alert-link text-nowrap">
                  <i class="fab fa-edge"></i>
                  <xsl:value-of select="' Microsoft Edge'" />
                </a>
                <xsl:value-of select="concat(i18n:translate('component.webtools.msie.enum'),' ')"/>
                <a href="https://mozilla.org/firefox/" class="alert-link text-nowrap">
                  <i class="fab fa-firefox"></i>
                  <xsl:value-of select="' Firefox'" />
                </a>
                <xsl:value-of select="concat(i18n:translate('component.webtools.msie.enumLast'),' ')"/>
            ...
            <div class="d-flex justify-content-center align-items-center flex-shrink-0 ">
              <form accept-charset="UTF-8">
                ...
                <button class="btn btn-warning" type="submit">
                  <xsl:value-of select="i18n:translate('component.webtools.msie.ignore')"/>
                </button>
              </form>
            </div>
          </div>
