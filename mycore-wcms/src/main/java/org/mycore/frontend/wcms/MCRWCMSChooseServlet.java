...
public class MCRWCMSChooseServlet extends MCRWCMSServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException, TransformerException, SAXException {
        ...
        /*
         * System.err.println("defaultLang"+defaultLang+"..."); System.err.println("currentLang"+currentLang+"...");
         */

        /**
         * If action parameter contains a character '_' (e.g.: add_intern, add_extern) then split action in action (e.g.: add) and mode (e.g.: intern)
         */
        ...
        /**
         * Split href parameter (e.g.: 1/content/top/index.xml) in character c and href for identifying the selected element in the navigation.xml and checking
         * the selected action Transfered characters are: --- valid values ------------------------------------------------------------------------ '1' - if the
         * selected element has a valid href attribute. '2' - if the selected element was a main element (e.g.: "Men? oberhalb", "Men? links" and "Zusatzmen?
         * links unten") and has a valid dir attribute instead of a valid href attribute. --- invalid values
         * ---------------------------------------------------------------------- '5' - if the selected element is a main navigation element like "Men? links"
         * and addAtPosition is "predecessor" or "successor". '6' - if the selected element has an external target and action was add. This is to avoid the
         * creation of a child under a parent with external target because it can not reached via navigation tree. '7' - if the selected element has child(s)
         * and action was delete. Thus, an element with active childs can not be deleted. Deleting elements with active childs is only possible when deleting
         * all childs first. '8' - if the selected element has childs but is not nested within a rootNode. This occurs only on restricted users trying to edit
         * such an element. '9' - if a place holder was selected. '0' - if nothing was selected.
         */
        ...
        /**
         * Try to build a jdom Object from the content (this has to be valid XHTML content) of the file to be edited. The path of this file is build from the
         * webapp dir and the href parameter of the selected element found in the navigation.xml.
         */
        ...
        /*
         * catch (JDOMException je) { if (output != null) { output.clear(); output.add("File konnte nicht geparst werden."); } }
         */
        ...
        /**
         * If character c consist an invalid value (5, 6, 7, 8, 9, 0) then print out the corresponding error and restart the WCMSChooseServlet (both things will
         * be done by the belonging stylesheet). Output: <cms> <session>choose </session> <error>5|6|7|8|9|0 </error> <rootNode href="no"|"yes">rootNode
         * </rootNode>{0,*} <templates><content> <template>$template </template>{0,*} </content> </templates> </cms>
         */
        ...
        /**
         * Else output: <cms><session>action </session> <userName>uid </userName> <userClass>admin|editor|autor </userClass> <action
         * mode="intern"|"extern">add|edit|delete </action> <addAtPosition>predecessor|successor|child </addAtPosition> <-- only at action add <href>href
         * </href> <label>label </label> <target>_blank|_self </target> <style>normal|bold </style> <content>output </content> <-- only at action edit and add
         * <images>$imagePath </images>{0,*} <documents>$documentPath </documents>{0,*} <templates><master><template>$template </template>{0,*} </master>
         * <content><template>conTemp </template> </content> </templates> </cms>
         */
        else {
        ...
        if (defaultLangContentOutput != null) {
            try {
                if (validXHTML) {
                    /*
                     * String test = ""; Element elem = null; for ( int i=0; i <output.size(); i++ ) { elem = (Element)output.get(i); test += elem.getText(); }
        ...
        /**
         * and add some further attributes to the session object: --- allready existing --------------------------------- <session ... uid=uid
         * userClass="admin"|"editor"|"autor" rootNodes=rootNodes ... --- new attributes ------------------------------------ ... href=href label=label
         * action="add"|"edit"|"delete" mode="intern"|"extern" ... ... target="intern"|"extern" style="normal"|"bold" />
         */
        if (href != null) {
