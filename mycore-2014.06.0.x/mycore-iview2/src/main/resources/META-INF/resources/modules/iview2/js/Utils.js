 ...
 */
jQuery.fn.jquibutton = jQuery.fn.button;
 ...
 * @name SimpleIterator
 * @description Allows it to get java like the next object within a given
...
(function() {
	 ...
	 * @name hasNext
	 * @memberOf iview.utils.SimpleIterator#
	 * @description proofs if there are any items remaining within the
	 *              collection (in forward direction) note once the end is
	 *              reached this function can return true as soon as a
	 *              getPrevious() was called
	 ...
	 * @name next
	 * @memberOf iview.utils.SimpleIterator#
	 * @description gives the user the next object while moving forward. This
	 *              function will return null objects if the end is reached
	 ...
	 * @memberOf iview.utils.SimpleIterator#
	 * @description proofs if there are any items remaining within the
	 *              collection (in backward direction) note once the beginning
	 *              is reached this function can return true as soon as a
	 *              getNext() was called
	 ...
	 * @name previous
	 * @memberOf iview.utils.SimpleIterator#
	 * @description gives the user the next object while moving backward. This
	 *              function will return null objects if the beginning is
	 *              reached
 ...
 * @name alertError
 * @memberOf iview.Utils
 * @description generates from the given Information an Error Alert so that the
 *              user is able to supply better Error Traceing
 * @param {string}
 *            msg own the description so that a better location where and when
 *            the error occured is possible
 * @para, {} nr number of the error to display
 * @param {}
 *            e the error event from which the more detailed information will
 *            gained
 */
 ...
 * @function
 * @name callBack
 * @description execute the given function, if it's no function, do nothing or
 *              alert a comment
 * @param {function}
 *            func is the function whis is to be executed
 */
 ...
 * @name getEvent
 * @memberOf iview.Utils
 * @description proofs if e is set, if it isn't were on IE which doesn't give
 *              the event with it
 * @param {}
 *            e var which will be proofed if event is supplied or not
 ...
 * @name toInt
 * @memberOf iview.Utils
 * @description converts a given String into an integer, but further doesn't
 *              give NAN for String was undefined and other cases
 * @param {string}
 *            value which shall be converted into an integer
 ...
 * @name toFloat
 * @memberOf iview.Utils
 * @description converts a given String into an float, but further doesn't give
 *              NAN for String was undefined and other cases
 * @param {string}
 *            value which shall be converted into a float
 ...
 * @param {string}
 *            browser Array with the Names of the Browsers or some parts of the
 *            useragent which shall be looked up
 * @return {boolean} true if within the supplied array the current browser was
 *         named or false if it wasn't
 */
function getBrowser(browser) {
	...
	if (window.addEventListener && !window.attachEvent && navigator.userAgent.indexOf("Firefox") != -1) {
		theBrowser = "Firefox";
		return;
	}
	// if (navigator.userAgent.indexOf("Chrome") != -1) { theBrowser = "Chrome";
	// return;}
...
function isBrowser(browsers) {
	...
	if ((browsers.sort && browsers.join(";").match(new RegExp(theBrowser + "(;|$)", "gi")))
			|| (!browsers.sort && browsers.indexOf(theBrowser) != -1)) {
		return true;
	} else {
		return false;
	}
...
function hideProperty(o, name, writeable) {
	if (typeof Object.defineProperty === 'function' && false) {
		try {
			...
			log({
				"msg:" : "Error while hiding property",
				"object:" : o,
				"property:" : name,
 ...
 * 
 * @param {Object}
 *            i18nKey the i18n key for the message that should be shown
 */
 ...
 * @name getParam
 * @memberOf URL
 * @description additional function, look for a parameter into search string an
 *              returns it
 * @param {string}
 *            param parameter whose value you want to have
 ...
 * @param {Object}
 *            url the url that should be cleaned (if null then the current URL
 *            will be used)
