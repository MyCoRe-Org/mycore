...
public abstract class MCRSolrBasicAuthenticatorBase implements MCRSolrAuthenticator {
    ...
    @Override
    public final void applyAuthentication(SolrRequest<?> request, MCRSolrAuthenticationLevel level) {
        request.setBasicAuthCredentials(getUsername(), getPassword());
    }

    @Override
    public final void applyAuthentication(HttpRequest.Builder request, MCRSolrAuthenticationLevel level) {
