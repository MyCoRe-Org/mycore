    ...
    public void index(ContentStreamBase stream) {
        try {
            ...
            try {
                ...
                LOGGER.log(MCRSolrLogLevels.SOLR_ERROR, "Could not commit changes", e);
            }
            LOGGER.trace("Indexing data of\"" + stream.getName() + "\" (" + (System.currentTimeMillis() - tStart) + "ms)");
        } catch (Exception ex) {
            LOGGER.log(MCRSolrLogLevels.SOLR_ERROR, "Error sending content to solr through content stream " + getClass().getSimpleName(),
                    ex);
        }
    }
}
