  ...
  <xsl:template match="/response">
    <xsl:copy>
      ...
      <xsl:if test="(result/doc and not(result/doc[101])) or (response[@subresult='groupOwner']/result/doc and not(response[@subresult='groupOwner']/result/doc[101]))">
        ...
        <xsl:apply-templates select="document(concat('solr:fl=id,returnId,derivateMaindoc,iviewFile&amp;sort=derivateOrder%20asc&amp;rows=',(count(result/doc|response[@subresult='groupOwner']/result/doc)*10),'&amp;q=', encoder:encode($query)))" mode="derivate"/>
      </xsl:if>
    </xsl:copy>
  </xsl:template>
