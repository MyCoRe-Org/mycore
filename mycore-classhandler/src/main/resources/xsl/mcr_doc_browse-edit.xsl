<?xml version="1.0" encoding="UTF-8"?>
  ...
  <!-- ============================================== -->
  <!--
    + | This stylesheet controls the Web-Layout of the MCRClassificationBrowser Servlet. | | This Template is embedded as a Part in the
    XML-Site, configurated in the Classification | section of the mycore.properties. | The complete XML stream is sent to the Layout Servlet
    and finally handled by this stylesheet. | | Authors: A.Schaar | Last changes: 2005-30-10 +
  -->
    ...
    <table cellspacing="0" cellpadding="0" border="0">
      <xsl:for-each select="classification">
        ...
        <xsl:variable name="categpath"
          select="concat($WebApplicationBaseURL, 'browse/',$browserClass, $HttpSession, '?mode=edit&amp;clid=',$classifID )" />
                ...
                <xsl:if test="($canEdit = 'true') and ($userEdited = 'false')">
                  ...
                  <table cellpadding="0" cellspacing="0" height="16" valign="right">
                    <tr valign="right" height="16">
                      ...
                      <xsl:if test="$use-aclEditor = true()">
                        <td width="25" valign="top">
                          ...
                          <a href="{$aclEditorAddress_edit}">
                            <img width="18" height="13" src="{$classacl}" title="{i18n:translate('wcms.rightsManag.acl.edit')}"
                              alt="{i18n:translate('component.classhandler.browse.classACL')}" />
                          </a>
                        </td>
                        <td width="25" valign="top">
                          <xsl:variable name="aclEditorAddress_delete">
                            <xsl:choose>
                              <xsl:when test="acl:hasRule($classifID, 'deletedb')">
                                <xsl:call-template name="aclEditor.embMapping.getAddress">
                                  <xsl:with-param name="objId" select="$classifID" />
                                  <xsl:with-param name="action" select="'edit'" />
                                </xsl:call-template>
                              ...
                              <xsl:otherwise>
                                <xsl:call-template name="aclEditor.embMapping.getAddress">
                                  <xsl:with-param name="objId" select="$classifID" />
                                  <xsl:with-param name="action" select="'add'" />
                                  <xsl:with-param name="permission" select="'deletedb'" />
                                </xsl:call-template>
                              </xsl:otherwise>
                            </xsl:choose>
                          </xsl:variable>
                          <a href="{$aclEditorAddress_delete}">
                            <img width="18" height="13" src="{$classdelete}" title="{i18n:translate('wcms.rightsManag.acl.edit')}"
                              alt="{i18n:translate('component.classhandler.browse.classACL')}" />
                          </a>
                        </td>
                      </xsl:if>
                      <xsl:if test="$edited='false'">
                        <td width="25" valign="top">
                          <a target="new" alt="$classifID"
                            onclick="fensterCodice('{$WebApplicationBaseURL}servlets/MCRClassExportServlet?id={$classifID}');return false;"
                            href="{$WebApplicationBaseURL}servlets/MCRClassExportServlet?id={$classifID}{$HttpSession}">
                            <input onclick="fensterCodice('{$WebApplicationBaseURL}servlets/MCRClassExportServlet?id={$classifID}');return false;"
                              type="image" src='{$classexport}' title="{i18n:translate('component.classhandler.browse.classExport')}" />
                          </a>
                        </td>
                      </xsl:if>
                      <td width="25" valign="top">
                      ...
                      <xsl:if test="$edited='false'">
                        <td width="25" valign="top">
                          <a target="new" alt="$classifID"
                            ...
                            <input
                              onclick="fensterCodice('{$WebApplicationBaseURL}servlets/MCRClassExportServlet?id={$classifID}{$HttpSession}');return false;"
                              type="image" src='{$classexport}' title="{i18n:translate('component.classhandler.browse.classExport')}" />
                          </a>
                        </td>
                      </xsl:if>
    ...
    <td class="{$cellStyle}">
      <xsl:choose>
        <xsl:when test="col[2]/@numDocs > 0 and col[2]/@searchbase != 'default' ">
          <a
            href="{concat($ServletsBaseURL, 'MCRSearchServlet',$HttpSession,'?query=(',../@searchField,'+=+', col[2]/@lineID, ')+and+',../@doctype,'&amp;numPerPage=10','&amp;mask=browse/',../startPath)}">
    ...
    <td class="{$cellStyle}">
      <xsl:choose>
        ...
        <xsl:otherwise>
          ...
          <xsl:if
            test="(../../userCanEdit = 'true') and ((../../userEdited='false') or ((../../session='') or (../../session=../../currentSession))) ">
            <table cellpadding="0" cellspacing="0" height="16">
              <tr valign="middle" height="16">
