...
public class MCRMetsCommands extends MCRAbstractCommands {
	...
	private static final Logger LOGGER = Logger
			.getLogger(MCRMetsCommands.class);
	...
	public static void validateSelectedMets() {
		List<String> selectedObjectIDs = MCRObjectCommands
		...
		for (String objectID : selectedObjectIDs) {
			...
			if (Files.exists(metsFile)) {
				try {
					...
					List<ValidationException> validationExceptionList = mv
							.validate();
					for (ValidationException validationException : validationExceptionList) {
						LOGGER.error(validationException.getMessage());
					}
				} catch (IOException e) {
					LOGGER.error("Error while reading mets.xml of " + objectID,
							e);
				} catch (JDOMException e) {
					LOGGER.error("Error while parsing mets.xml of " + objectID,
							e);
				}
			}
