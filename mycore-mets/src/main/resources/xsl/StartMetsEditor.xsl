<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" 
  ...
  xmlns:metslock="xalan://org.mycore.mets.tools.MCRMetsLock" 
  exclude-result-prefixes="xsl i18n acl metslock">
  ...
  <xsl:template match="/StartMetsEditor">
    ...
    <xsl:choose>
      <xsl:when test="$createMetsAllowed">
      	<xsl:variable name="metsLock" select="not(metslock:doLock($derivateId))" />
	      	<xsl:choose>
	      	<xsl:when test="$metsLock">
				<xsl:variable name="lockText">
				<xsl:value-of select="i18n:translate('component.mets.editor.lock')" />
				</xsl:variable>
	      		<html>
		          <head>
		            <title>
		            	<xsl:value-of select="$lockText" />
		            </title>
		            </head>
		            <body>
		            	<h1><xsl:value-of select="$lockText" /></h1>
		            </body>
		        </html>
	      	</xsl:when>
			<xsl:otherwise>
				<html>
		          <head>
		            ...
		            <script type="text/javascript">
		              ...
		              function metsUnlock(){
		              	var url = webApplicationBaseURL + "servlets/MCRMETSUnlockServlet/?derivate=" + derivateId;
		              	var xhrArgs = {
						    url: url,
						    handleAs: "text",
						    load: function(data){
