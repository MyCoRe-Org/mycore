...
public final class MCRObjectID {
	...
	static {
		...
		number_distance = CONFIG.getInt(
				"MCR.metadata_objectid_number_distance", 1);
		number_pattern = CONFIG.getString(
				"MCR.metadata_objectid_number_pattern", "0000000000");
	...
	public synchronized void setNextFreeId(String base_id) throws MCRException {
		...
		if (!setID(mcrid.toString()))
			throw new MCRException(
					"Error in project base string for the new ID:" + mcrid);
	...
	public String getId() {
		...
		if (mcr_id == null) {
			mcr_id = new StringBuffer(MAX_LENGTH).append(mcr_project_id)
					.append('_').append(mcr_type_id).append('_').append(
							number_format.format((long) mcr_number)).toString();
		}
		return mcr_id;
	}
	...
	private final boolean setID(String id) {
		...
		String mcr_id;
		try {
			mcr_id = URLEncoder.encode(id, CONFIG.getString(
	...
	public boolean equals(MCRObjectID in) {
		return (mcr_project_id.equals(in.mcr_project_id)
				&& (mcr_type_id.equals(in.mcr_type_id)) && (mcr_number == in.mcr_number));
	}
