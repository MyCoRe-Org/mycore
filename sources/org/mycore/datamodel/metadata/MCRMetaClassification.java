 ...
 */
public class MCRMetaClassification extends MCRMetaDefault implements
		MCRMetaInterface {
	...
	public MCRMetaClassification(String set_datapart, String set_subtag,
			int set_inherted, String set_type, String set_classid,
	...
	public final void setValue(String set_classid, String set_categid)
			throws MCRException {
		if ((set_classid == null)
				|| ((set_classid = set_classid.trim()).length() == 0)) {
			throw new MCRException("The classid is empty.");
		}
		if ((set_categid == null)
				|| ((set_categid = set_categid.trim()).length() == 0)) {
	...
	public org.jdom.Element createXML() throws MCRException {
		if (!isValid()) {
			throw new MCRException(
					"The content of MCRMetaClassification in subtag " + subtag
							+ " is not valid.");
		}
	...
	public boolean isValid() {
		...
		try {
			MCRClassificationItem cl = MCRClassificationItem
					.getClassificationItem(classid);
			if (cl == null) {
				return false;
			}
