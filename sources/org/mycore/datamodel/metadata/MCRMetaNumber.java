 ...
 */
final public class MCRMetaNumber extends MCRMetaDefault implements
		MCRMetaInterface {
	...
	public final void setFromDOM(org.jdom.Element element) {
		...
		if (attr != null) {
			...
			if ((temp_meas != null)
					&& ((temp_meas = temp_meas.trim()).length() != 0)) {
		...
		if (attr != null) {
			...
			if ((temp_dim != null)
					&& ((temp_dim = temp_dim.trim()).length() != 0)) {
				dimension = temp_dim;
			}
			if (dimension.length() >= MAX_DIMENSION_LENGTH) {
				dimension = dimension.substring(0, MAX_DIMENSION_LENGTH);
	...
	public final org.jdom.Element createXML() throws MCRException {
		...
		if ((dimension != null)
				&& ((dimension = dimension.trim()).length() != 0)) {
	...
	public final MCRTypedContent createTypedContent(boolean parasearch)
		...
		if ((dimension = dimension.trim()).length() != 0) {
			tc.addStringElement(MCRTypedContent.TYPE_ATTRIBUTE, "dimension",
					dimension);
		}
		if ((measurement = measurement.trim()).length() != 0) {
			tc.addStringElement(MCRTypedContent.TYPE_ATTRIBUTE, "measurement",
					measurement);
		}
		return tc;
	}
	...
	public final Object clone() {
		MCRMetaNumber out = new MCRMetaNumber(datapart, subtag, lang,
				inherited, dimension, measurement, number);
		return (Object) out;
	}
