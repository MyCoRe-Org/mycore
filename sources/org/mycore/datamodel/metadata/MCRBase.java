...
public abstract class MCRBase {
	...
	// logger
	static Logger logger = Logger.getLogger(MCRBase.class.getPackage()
			.getName());
	...
	static {
		...
		try {
			...
			mcr_persist = (MCRObjectSearchStoreInterface) Class.forName(
					persist_name).newInstance();
		} catch (Exception e) {
	 ...
	 */
	public abstract void createDataBase(String mcr_type,
			org.jdom.Document confdoc);
	...
	public boolean isValid() {
		...
		if ((mcr_label == null)
				|| ((mcr_label = mcr_label.trim()).length() == 0)) {
			return false;
		}
		if ((mcr_schema == null)
				|| ((mcr_schema = mcr_schema.trim()).length() == 0)) {
			return false;
		}
		return true;
	}
