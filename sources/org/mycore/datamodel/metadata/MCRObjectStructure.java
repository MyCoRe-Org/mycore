...
public class MCRObjectStructure {
	...
	public final boolean addChild(MCRMetaLinkID child) {
		...
		for (i = 0; i < n; ++i) {
			if (((MCRMetaLinkID) children.get(i)).getXLinkHref().equals(
					child.getXLinkHref())) {
				return false;
			}
		}
	...
	public final void setFromDOM(org.jdom.Element element) {
		...
		if (struct_element != null) {
			...
			for (int i = 0; i < struct_links_list.size(); i++) {
				org.jdom.Element link_element = (org.jdom.Element) struct_links_list
						.get(i);
		...
		if (struct_element != null) {
			List struct_links_list = struct_element.getChildren();
			for (int i = 0; i < struct_links_list.size(); i++) {
				org.jdom.Element der_element = (org.jdom.Element) struct_links_list
						.get(i);
	...
	public final boolean isValid() {
		...
		for (int i = 0; i < derivates.size(); ++i) {
			...
			if (!((MCRMetaLinkID) derivates.get(i)).getXLinkType().equals(
					"locator")) {
				return false;
			}
