 ...
 */
final public class MCRMetaHistoryDate extends MCRMetaDefault implements
		MCRMetaInterface {
	 ...
	 */
	public static final GregorianCalendar getAntikDate(String datestr,
			boolean last) {
			...
			if (secdot != -1) {
				...
				if (firstdot != -1) {
					if (iso) {
						...
						mon = Integer.parseInt(datestr.substring(start,
								firstdot));
					...
					year = Integer.parseInt(datestr.substring(start, datestr
							.length()));
					if (last) {
						mon = 11;
						day = 31;
	...
	public final org.jdom.Element createXML() throws MCRException {
		if (!isValid()) {
			...
			throw new MCRException(
					"The content of MCRMetaHistoryDate is not valid.");
		}
		...
		if ((text = text.trim()).length() != 0) {
			elm.addContent((org.jdom.Content) new org.jdom.Element("text")
					.addContent(text));
		}
		if (von != null) {
			elm.addContent((org.jdom.Content) new org.jdom.Element("von")
					.addContent(getVonToString()));
			elm.addContent((org.jdom.Content) new org.jdom.Element("ivon")
					.addContent((new Integer(ivon)).toString()));
		}
		if (bis != null) {
			elm.addContent((org.jdom.Content) new org.jdom.Element("bis")
					.addContent(getBisToString()));
			elm.addContent((org.jdom.Content) new org.jdom.Element("ibis")
					.addContent((new Integer(ibis)).toString()));
		}
		return elm;
	}
	...
	public final boolean isValid() {
		if (((text = text.trim()).length() == 0) && (von == null)
				&& (bis == null)) {
			return false;
		}
		return true;
	...
	public final Object clone() {
		MCRMetaHistoryDate out = new MCRMetaHistoryDate(datapart, subtag, lang,
				type, inherited);
		out.setText(text);
		out.setVonDate(von);
		out.setBisDate(bis);
