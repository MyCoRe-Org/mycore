...
/**
 * This class implements all methods for handling with the MCRMetaHistoryDate part
 * of a metadata object. The MCRMetaHistoryDate class is a special class for the
 * datamodel of Papyrus-Project Jena-Halle-Leipzig.
 *
...
public MCRMetaHistoryDate()
  ...
  try { default_von = getAntikDate(min,false); }
 ...
 * The syntax for the input is [-|AD|BC][[[t]t.][m]m.][yyy]y in a Gregorian 
 * calendar date.
 *
 * @param datestr the date as string.
 * @param last the value is true if the defaults should be high values like
...
public static final GregorianCalendar getAntikDate(String datestr, boolean last)
  ...
  if (datestr.length()>18) { return null; }
  if (datestr.length()==0) { return null; }
  try {
  // suche nach v. Chr.
  boolean bc = false; 
  int start = 0;
  if (datestr.substring(0,1).equals("-")) { bc = true; start = 1; }
  if (datestr.length() > 2) {
    if (datestr.substring(0,2).equals("AD")) { bc = false; start = 2; }
    if (datestr.substring(0,2).equals("BC")) { bc = true; start = 2; }
    }
  if (datestr.length() > 7) {
    if (datestr.substring(0,7).equals("n. Chr.")) { bc = false; start = 7; }
    if (datestr.substring(0,7).equals("v. Chr.")) { bc = true; start = 7; }
    }
  // deutsch oder ISO?
  boolean iso = false;
  String token = ".";
  if (datestr.indexOf("-",start+1) != -1) { iso = true; token = "-"; }
  // Punkte/Striche ermitteln
  int firstdot = datestr.indexOf(token,start+1);
  int secdot = -1;
  if (firstdot != -1) { secdot = datestr.indexOf(token,firstdot+1); }
  ...
  if (secdot != -1) {
    ...
    if (firstdot != -1) {
      ...
      if (last) { mon = 11; day = 31; } else { mon = 0; day = 1; }
      }
    }
  // setzten AD/BC
...
public final void setText(String set)
  {
  if (set == null) { text = ""; return; }
  if (set.length() <= PAPANTIKDATE_MAX_TEXT) { text = set.trim(); }
  else { text = set.substring(0,PAPANTIKDATE_MAX_TEXT); }
  }
...
public final void setDefaultVonBis()
  { 
  von = default_von; ivon = getIntDate(von); 
  bis = default_bis; ibis = getIntDate(bis); 
  }
...
public final void setDateVonBis(String numdat)
  ...
  if (i == -1) {
     setVonDate(numdat); setBisDate(numdat); }
  else {
...
public final void setVonDate(GregorianCalendar set_date)
  { 
  von = default_von; if (set_date != null) { von = set_date; } 
  ivon = getIntDate(von);
  }
...
public final void setVonDate(String set_date)
  { 
  try { von = getAntikDate(set_date,false);  } 
  catch (Exception e) { von = default_von; }
  ivon = getIntDate(von);
  }
...
public final void setBisDate(GregorianCalendar set_date)
  { 
  bis = default_bis; if (set_date != null) { bis = set_date; }
  ibis = getIntDate(bis);
  }
...
public final void setBisDate(String set_date) throws MCRException
  { 
  try { bis = getAntikDate(set_date,true);  } 
  catch (Exception e) { bis = default_bis; }
  ibis = getIntDate(bis);
  }
 ...
 * by the MyCoRe XML MCRMetaHistoryDate definition for the given subtag.
 *
...
public final MCRTypedContent createTypedContent(boolean parasearch)
  ...
  if ((text = text.trim()).length() !=0) {
    tc.addStringElement(MCRTypedContent.TYPE_ATTRIBUTE,"text",text); }
  if (von != null) {
    tc.addDateElement(MCRTypedContent.TYPE_ATTRIBUTE,"von",von); }
  if (bis != null) {
    tc.addDateElement(MCRTypedContent.TYPE_ATTRIBUTE,"bis",bis); }
  return tc;
  }
