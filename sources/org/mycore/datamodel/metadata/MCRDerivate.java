...
final public class MCRDerivate extends MCRBase {
    ...
    private final void set() throws MCRException {
        ...
        mcr_id = new MCRObjectID((String) jdom_element_root.getAttribute("ID")
                .getValue());
        mcr_label = (String) jdom_element_root.getAttribute("label").getValue()
                .trim();
        if (mcr_label.length() > MAX_LABEL_LENGTH) {
            mcr_label = mcr_label.substring(0, MAX_LABEL_LENGTH);
        }
        mcr_schema = (String) jdom_element_root.getAttribute(
    ...
    public final void createInDatastore() throws MCRPersistenceException {
        ...
        if (getDerivate().getInternals() != null) {
            ...
            if ((!f.isDirectory()) && (!f.isFile())) {
                ...
                throw new MCRPersistenceException("The File or Directory on "
                        + getDerivate().getInternals().getSourcePath()
                        + " was not found.");
            }
            try {
                MCRDirectory difs = MCRFileImportExport.importFiles(f, mcr_id
                        .getId());
        ...
        try {
            ...
            throw new MCRPersistenceException(
                    "Error while creating derivate in" + " datastore.", e);
        }
        ...
        for (int i = 0; i < getDerivate().getLinkMetaSize(); i++) {
            ...
            try {
                ...
                // delete from IFS
                MCRDirectory difs = MCRDirectory.getRootDirectory(mcr_id
                        .getId());
                difs.delete();
                // delete search data
                ...
                // throw final exception
                throw new MCRPersistenceException(
                        "Error while creatlink to MCRObject "
                                + meta.getXLinkHref() + ".", e);
            }
        }
    }
    ...
    public final MCRDirectory receiveDirectoryFromIFS(String id)
        ...
        if (difs == null) {
            throw new MCRPersistenceException(
                    "Error while receiving derivate with " + "ID "
                            + mcr_id.getId() + " from IFS.");
        }
        return difs;
    }
    ...
    public final void updateInDatastore() throws MCRPersistenceException {
        ...
        for (int i = 0; i < getDerivate().getLinkMetaSize(); i++) {
            ...
            try {
                ...
                throw new MCRPersistenceException("The MCRObject "
                        + meta.getXLinkHref() + " was not found.");
            }
        }
    }
