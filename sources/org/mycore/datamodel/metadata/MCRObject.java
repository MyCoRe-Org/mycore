...
final public class MCRObject extends MCRBase {
    ...
    private final void set() throws MCRException {
        ...
        mcr_schema = jdom_element_root.getAttribute(
    ...
    public final MCRTypedContent createTypedContent() throws MCRException {
        ...
        tc.addStringElement(MCRTypedContent.TYPE_ATTRIBUTE, "ID", mcr_id
                .getId());
    ...
    public final void createInDatastore() throws MCRPersistenceException {
        ...
        if (parent_id != null) {
            try {
                parent.getStructure().addChild(mcr_id,
                        mcr_struct.getParent().getXLinkLabel(), mcr_label);
    ...
    public final void removeDerivateInDatastore(String id, MCRMetaLinkID link)
        ...
        if (j != -1) {
            ...
            throw new MCRPersistenceException("The derivate link "
                    + link.getXLinkHref() + " was not found.");
        }
    }
    ...
    private final void deleteFromDatastore() throws MCRPersistenceException {
        ...
        for (int i = 0; i < mcr_struct.getDerivateSize(); i++) {
            der = new MCRDerivate();
            try {
                der.deleteFromDatastore(getStructure().getDerivate(i)
                        .getXLinkHref());
        ...
        for (int i = 0; i < mcr_struct.getChildSize(); i++) {
            child = new MCRObject();
            try {
                child.deleteFromDatastore(getStructure().getChild(i)
                        .getXLinkHref());
            } catch (MCRException e) {
                logger.debug(MCRException.getStackTraceAsString(e));
                logger.error(e.getMessage());
        ...
        if (parent_id != null) {
            ...
            try {
                ...
                logger.warn("Attention, the parent " + parent_id
                        + "is now inconsist.");
            }
        }
        // remove him self
    ...
    public final void updateInDatastore() throws MCRPersistenceException {
        ...
        if ((old.mcr_struct.getParent() != null)
            ...
            if (!newparent.equals(oldparent)) {
                ...
                try {
                    ...
                    logger.debug(MCRException.getStackTraceAsString(e));
                    logger
                            .error("Error while delete child ID in parent object.");
                    logger.warn("Attention, the parent " + oldparent
                            + "is now inconsist.");
                }
            }
        }
        ...
        // set the date
        mcr_service.setDate("createdate", old.getService()
                .getDate("createdate"));
        // update this dataset
        ...
        for (int i = 0; i < mcr_struct.getChildSize(); i++) {
            ...
            child.updateMetadataInDatastore(mcr_struct.getChild(i)
                    .getXLinkHrefID());
        }
    }
    ...
    private void addLinksToTable() {
        ...
        while ((i < mcr_tc.getSize())
                && (!mcr_tc.getNameElement(i).equals("metadata"))) {
            i++;
        }
        // add metadata links
        i++;
        while ((i < mcr_tc.getSize())
                && (!mcr_tc.getNameElement(i).equals("service"))) {
            if ((mcr_tc.getNameElement(i).equals("type"))
                ...
                try {
                    MCRObjectID mcr_link = new MCRObjectID((String) mcr_tc
                            .getValueElement(i + 1));
            ...
            if ((mcr_tc.getNameElement(i).equals("classid"))
                ...
                try {
                    MCRObjectID mcr_class = new MCRObjectID((String) mcr_tc
                            .getValueElement(i));
