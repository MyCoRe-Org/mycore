...
public class MCRClassificationBrowserData {
	...
	private static Logger logger = Logger
			.getLogger(MCRClassificationBrowserData.class);
	...
	public MCRClassificationBrowserData(String u) throws Exception {
		...
		if (uriParts.length > 1) {
			logger.debug(this.getClass() + " PathParts - classification "
					+ uriParts[1]);
			logger.debug(this.getClass() + " Number of PathParts ="
					+ uriParts.length);
			classifID = config.getString("MCR.ClassificationBrowser."
					+ uriParts[1] + ".Classification");
			pageName = config.getString("MCR.ClassificationBrowser."
					+ uriParts[1] + ".EmbeddingPage");
			xslStyle = config.getString("MCR.ClassificationBrowser."
					+ uriParts[1] + ".Style");
			try {
				emptyLeafs = config.getString("MCR.ClassificationBrowser."
						+ uriParts[1] + ".EmptyLeafs");
				view = config.getString("MCR.ClassificationBrowser."
						+ uriParts[1] + ".View");
				comments = config.getString("MCR.ClassificationBrowser."
						+ uriParts[1] + ".Comments");
				doctype = config.getString("MCR.ClassificationBrowser."
						+ uriParts[1] + ".Doctype");
				restriction = config.getString("MCR.ClassificationBrowser."
						+ uriParts[1] + ".Restriction");
			} catch (org.mycore.common.MCRConfigurationException ig) {
				// ignore for this parameters
				;
			}
		...
		if (emptyLeafs == null)
			emptyLeafs = "yes";
		if (view == null || !view.endsWith("tree"))
			view = "flat";
		if (comments == null)
			comments = "false";
		...
		if (doctype != null) {
			try {
				...
				logger.info(this.getClass()
						+ "No search type was set - it seams to be ok");
			}
		}
	...
	private void clearPath(String[] uriParts) throws Exception {
		...
		for (int k = 2; k < uriParts.length; k++) {
			logger.debug(this.getClass() + " uriParts[k]=" + uriParts[k]
					+ " k=" + k);
			...
			logger.debug(this.getClass() + " cati[len]=" + cati[len] + " len="
					+ len);
		}
	...
	public org.jdom.Document loadTreeIntoSite(org.jdom.Document cover,
		...
		Element placeholder = cover.getRootElement().getChild(
	...
	public org.jdom.Document createXmlTree(String lang) throws Exception {
		...
		xDocuments.addContent(String.valueOf(cl.countDocLinks(doctypeArray,
				restriction)));
		...
		while ((line = getLine(i++)) != null) {
			int numDocs = line.cat.countDocLinks(doctypeArray, restriction);
			logger.info(this.getClass() + " NumDocs - " + numDocs);

			if (emptyLeafs.endsWith("no") && numDocs == 0) {
				logger.debug(this.getClass() + " empty Leaf continue - "
						+ emptyLeafs);
				continue;
			}
			...
			if (line.status.equals("T")) {
				...
				xCol.setAttribute("folder2",
						numDocs > 0 ? "folder_closed_in_use"
								: "folder_closed_empty");
			} else if (line.status.equals("F")) {
				xCol.setAttribute("plusminusbase", line.cat.getID());
				xCol.setAttribute("folder1", "folder_minus");
				xCol.setAttribute("folder2", numDocs > 0 ? "folder_open_in_use"
						: "folder_open_empty");
			} else {
	...
	public void update(String categID) throws Exception {
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			logger.debug(this.getClass() + " compare CategoryTree on " + i
					+ "_" + line.cat.getID() + " to " + categID);
			...
			if (view.endsWith("tree")) {
				if (hideLevel) {
														...
														// children
					{
						...
						for (int j = 0, k = children.length; j < k; j++) {
							...
							lines.insertElementAt(new MCRNavigTreeLine(cat,
									line.level + 1), ++i);
						}
					}
				}
				...
				if (categID.equalsIgnoreCase(line.cat.getID())) {
					...
					logger.debug(this.getClass() + " remove lines " + i + "_"
							+ line.cat.getID());
					lines.removeElementAt(i--);
				}
			}
