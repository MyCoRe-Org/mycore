...
/**
 * For MCROldFiles that contain streaming audio/video, instances of this class 
 * provide methods to get technical metadata like bitrate, framerate, 
 * duration, size etc. and to start a player to stream the asset to a browser.
 *
 * @author Frank Lützenkirchen 
 * @deprecated USED BY MILESS 1.3 ONLY!
 * @version $Revision$ $Date$
 */
public abstract class MCROldAudioVideoExtender
{
  ...
  protected MCROldFile file;

  /** 
   * Creates a new MCROldAudioVideoExtender. The instance has to be initialized 
   * by invoking init() before it can be used.
   **/
  public MCROldAudioVideoExtender()
  {}
   ...
   *
   * @param file the MCROldFile that this extender belongs to
   **/
  public abstract void init( MCROldFile file )
    throws MCRException;
  ...
  /**
   * Returns the streaming bitrate formatted as a String, e. g.
   * "1.3 MBit" or "300 kBit".
   *
   ...
   *
   * @return the maximum number of frames per second for a streaming video asset
   **/
   ...
   * For example, <tt>getDurationFormatted( "Std.", "Min.", "Sek." )</tt>
   * will return the String "1 Std. 15 Min." for an asset that is
   * one hour and 15 minutes long. If duration is less than one hour,
   * only minutes and seconds go into the output string, otherwise
   * hours and minutes are used.
   *
   * @param hourLabel the label for the hours part of the duration
   * @param minutesLabel the label for the minutes part of the duration
   * @param secondsLabel the label for the seconds part of the duration
  ...
  { return MCROldFile.getSizeFormatted( size ); }
   ...
   * asset. If a queryString is provided, it is forwarded to the server, for
   * example to determine the start and stop position for streaming.
   *
   * @param out the OutputStream to write the player starter to
   * @param a HTTP request query string to forward to the player starter
   **/
  public abstract void getPlayerStarterTo( OutputStream out, String queryString )
  ...
  public String toString()
  {
    ...
    sb.append( "Duration        : " )
      .append( getDurationFormatted( "h", "min", "sec" ) );
    sb.append( " (" )
      .append( getDurationTimecode() )
      .append( ")\n" );
    sb.append( "Size            : " )
      .append( getSizeFormatted() );
    sb.append( " (" )
      .append( getSize() )
      .append( ")\n" );
    sb.append( "Content Type    : " )
      ...
      .append( "\n" );
    sb.append( "Player Download : " )
      .append( playerDownloadURL );
    
    return sb.toString();
  }
  
  /**
   * Helper method to get a substring that lies between a prefix and
   * a suffix string. If either prefix or suffix are not found in the
   * string, the defaultValue is returned. This helper method is used
   * by subclasses to parse metadata that is read from the server.
   *
   * @param prefix the string before the substring
   * @param suffix the string after the substring
   * @param data the string to search through
   * @param defaultValue the default to return when no match is found
   * @return the substring between prefix and suffix
   **/
  protected String getBetween( String prefix, String suffix, 
                               String data, String defaultValue )
  {
    int from = data.indexOf( prefix );
    if( from == -1 ) return defaultValue;
    from += prefix.length();
    
    int to = data.indexOf( suffix, from );
    if( to == -1 ) return defaultValue;

    return data.substring( from, to ).trim();
  }

  /**
   * Helper method that reads all data from an URLConnection input stream
   * and forwards it to the given output stream.
   *
   * @param connection the URLConnection to get the InputStream from
  ...
  /**
   * Helper method that creates a URLConnection to a given URL and
   * wraps possible IOException or MalformedURLExceptions
   *
  ...
  /**
   * Helper method that connects to the given URL and returns the 
