 ...
 * classes to create, retrieve, update or delete the metadata
 * of all MCRFilesystemNode objects in a persistent datastore.
 * The manager holds a cache of all MCRFilesystemNodes most
 * recently used. It creates the MCRFileMetadataStore instance
 * as configured in the MyCoRe properties and hides its usage
 * from the other classes.
 *
 * Configuration properties:
 * <ul>
 *   <li><b>MCR.IFS.FileMetadataStore.Class:</b> 
 *   The class that implements the MCRFileMetadataStore interface
 *   and that should be used to store the persistent metadata.
 *   </li>
 *   <li><b>MCR.IFS.FileMetadataStore.CacheSize:</b> The size of
 *   the cache holding the most recently used MCRFilesystemNodes 
 *   expressed as maximum number of node objects in the cache.
 *   </li>
 * </ul>
 *
...
public class MCRFileMetadataManager
{
  ...
  /** Last number that was used for creating a unique ID for each MCRFilesystemNode **/
  private long last_number = System.currentTimeMillis();

  /** 
   * Prefix for all generated IDs that is derived from the server's
   * IP address to ensure that different MyCoRe instances will
   * create different unique IDs for each MCRFilesystemNode
   **/   
  private String prefix;
  
  /**
   * Creates a prefix for all generated IDs. This ID is derived from the server's
   * IP address to ensure that different MyCoRe instances will
   * create different unique IDs for each MCRFilesystemNode
   **/
  ...
  /**
   * Creates or updates the data of the MCRFilesystemNode 
   * in the persistent MCRFileMetadataStore.
   *
   * @param node the MCRFilesystemNode to store
   **/
  ...
  /**
   * Retrieves the MCRFilesystemNode with the given ID
   * from the persistent MCRFileMetadataStore.
   *
   * @param ID the unique ID of the MCRFilesystemNode
   * @return the MCRFilesystemNode with that ID, or null if no such node exists.
   **/  
  ...
  /**
   * Retrieves the first MCRFilesystemNode found in the 
   * persistent MCRFileMetadataStore, that has the 
   * given owner ID. This is assumed to be a root node.
   *
   * @param ownerID the ID of the owner of the MCRFilesystemNode to be retrieved
   * @return the MCRFilesystemNode with that owner, or null if no such node exists.
   **/  
   ...
   * @param name the file name of the child in that directory
  ...
  /**
   * Callback method for internal use by any 
   * MCRFileMetadataStore implementation, do not use.
   * Builds a MCRFilesystemNode object from the raw data
   * that is retrieved from the persistent store, or uses
   * the existing copy in the MCRCache instance.
   **/
  ...
  /**
   * Retrieves a list of all IDs of the child MCRFilesystemNodes
   * of a given MCRDirectory.
   *
   * @param ID the ID of the parent MCRDirectory
   * @return a Vector of String objects containing the IDs of all children of that MCRDirectory
   **/  
   ...
   * 
   * @param ID the ID of the node to delete from the store
   **/  
