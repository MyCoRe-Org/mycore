...
/**
 * Represents a "National Bibliographic Name" URN that is used as a persistent
 * identifier for digital documents like electronic dissertations, as proposed
 * by "Deutsche Bibliothek" in the project "Carmen - AP 4". A typical NBN URN
...
public class MCRNBN {
	 ...
	 * This implementation creates a NISS of eight digits length that is derived
	 * from the current time measured in seconds. The method guarantees
	 * uniqueness and therefore will block to produce only one NISS per second.
	 * 
	 * @return a new NISS
	 */
	...
	/**
	 * Creates a new local NBN and calculates a unique NISS and checksum for it.
	 * This implementation creates a NISS of eight digits length that is derived
	 * from the current time measured in seconds. The constructor guarantees
	 * uniqueness and therefore will block to produce only one NBN per second.
	...
	public static boolean exists(String urn) {
		...
		if (urn.startsWith(prefix)) {
			logger.info("Looking for NBN: "
					+ urn.substring(urn.lastIndexOf("-") + 1));
			HashMap map = (HashMap) manager.listURNs(urn.substring(urn
					.lastIndexOf("-") + 1));
			if (!map.isEmpty()) {
				return true;
			}
	...
	/**
	 * Method getURL. Returns the URL which has been locally attached with the
	 * URN
	 * 
	...
	/**
	 * Returns the namespace of this URN, that is the part between "urn:" and
	...
	/**
	 * Returns the NISS and checksum part of this NBN URN. Returns null if this
	 * NBN is not valid.
	 */
	...
	/**
	 * Returns true if this NBN is valid and starts with the local NBN prefix.
	 */
	...
	/**
	 * Returns true if this NBN has a valid structure and the checksum is
	 * correct.
	 */
