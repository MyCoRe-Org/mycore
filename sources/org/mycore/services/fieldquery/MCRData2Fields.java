...
/**
 * Provides methods to automatically extract field values for indexing 
 * from MCRObject or MCRFile using the definition in searchfields.xml.
 * The buildFields method return a list of MCRSearchField objects with values
 * extracted from the object for the given search index. This class supports
 * extracting values from MCRObject metadata, MCRFile metadata, 
 * MCRFile xml content and MCRFile text content using text filter plug-in.
 *  
...
public class MCRData2Fields
{
   ...
   * 
   * @param obj the MCRObject thats metadata should be indexed
   * @param index the ID of the index as defined in searchfields.xml
   * @return a List of MCRSearchField objects that contain name, type and value
   */
  ...
  /**
   * Extracts field values for indexing from the given MCRFile's metadata,
   * xml content or text content.
   * 
   * @param obj the MCRFile thats data should be indexed
   * @param index the ID of the index as defined in searchfields.xml
   * @return a List of MCRSearchField objects that contain name, type and value
   */
   ...
   * its MCRAudioVideoExtender, if present. That xml can be used for indexing this
   * data.
   */
  ...
  /**
   * For MCRObject, returns the object's metadata as XML, for MCRFile, returns the
   * file's content as XML, or null. 
   **/
  private static Document getXML( Object obj )
  {
    if( obj instanceof MCRFile )
    {
      ...
      try{ return file.getContentAsJDOM(); }
      catch( Exception ex )
      {
    ...
    else return ( (MCRObject)obj ).createXML();
  }
  ...
  private static Document buildStylesheet( String index, List fields )
  {
    ...
    if( xsl == null )
    {
      ...
      Namespace mcrns = Namespace
          .getNamespace( "mcr", "http://www.mycore.org/" );
      ...
      Namespace extns = Namespace.getNamespace( "ext",
      ...
      for( int i = 0; i < fields.size(); i++ )
      {
        ...
        if( ( xpath == null ) || ( xpath.trim().length() == 0 ) ) continue;
        ...
        for( int j = 0; j < attributes.size(); j++ )
        {
          ...
          attribute.setAttribute( "value", "{"
              + attribute.getAttributeValue( "value" ) + "}" );
        }
      }
  ...
  /**
   * Xalan XSL extension to convert MyCoRe date values to standard format. To be
   * used in a stylesheet or searchfields.xml configuration. Usage example:
   * 
