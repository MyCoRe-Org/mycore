...
abstract public class MCRQueryBase implements MCRQueryInterface {
	...
	protected static String NL = new String((System.getProperties())
	...
	public MCRQueryBase() {
		...
		logger
				.info("The maximum of the results is "
						+ Integer.toString(maxres));
			...
			for (int p = 0; p < subqueries.size(); p++) {
				...
				if (q.startsWith("@ID")) {
					...
					if (a <= 7) {
						if (b == q.length() - 1) {
							try {
								testid[p] = new MCRObjectID(q.substring(a + 1,
										b));
								isid++;
							} catch (Exception e) {
								logger.error(
										"Error while extracting id from query "
												+ q, e);
							}
						}
					}
	...
	public MCRObjectID getObjectID(String DerivateID) {
		...
		if (derivate.size() > 0) {
			...
			String objectID = derivateE.getChild("derivate").getChild(
					"linkmetas").getChild("linkmeta").getAttributeValue("href",
	...
	/**
	 * This method take a n ArrayList of MCRObjectID's and put the coresponding
	 * XML in a MCRResultContainer. If the method can not find a coresponding
	 ...
	 * MCRXMLContainer. This implementation must be overwrite with them form the
	 * persitence layer.
	 * 
	 * @param type
	 *            the MCRObject type
