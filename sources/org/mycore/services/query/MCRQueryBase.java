...
abstract public class MCRQueryBase implements MCRQueryInterface {
     ...
     */
    public MCRXMLContainer getResultList(String query, String type, int maxresults) {
        // check the input
            ...
            if ((isid == subqueries.size()) && (isid != 0)) {
                ...
                for (int p = 0; p < subqueries.size(); p++) {
                    if (onetype.equals(testid[p].getTypeId())) {
                        try {
                            result.add("local", testid[p].getId(), 0, (Element) xmltable.readDocument(testid[p]).getRootElement().clone());
                        } catch (Exception e) {
