...
public class MCRQueryServlet extends MCRServlet {
    ...
    private final MCRXMLContainer sort(MCRXMLContainer xmlcont, String lang) {
        ...
        /*
         * maybe here should be a propertie used XPath Expression can be
         * relative to mcr_result
         */
    ...
    private final boolean isInstanceOfLocal(String host,
            HttpServletRequest request) {
        String ServletHost = request.getServerName();
        String RemoteHost = MCR_CONFIG.getString("MCR.remoteaccess_" + host
                + "_host");
        String ServletPath = request.getServletPath().substring(0,
                request.getServletPath().lastIndexOf("/"))
                + "/MCRQueryServlet";
        String RemotePath = MCR_CONFIG.getString("MCR.remoteaccess_" + host
                + "_query_servlet");
        int ServletPort = request.getServerPort();
        int RemotePort = Integer.parseInt(MCR_CONFIG
                .getString("MCR.remoteaccess_" + host + "_port"));
        return ((RemoteHost.equals(ServletHost))
                && (RemotePath.equals(ServletPath)) && (ServletPort == RemotePort)) ? true
                : false;
    }
        ...
        if (att_host != null && (hosts == null || hosts.length == 0)) {
            ...
            for (int i = 0; i < hosts.length; i++) {
                if (!hosts[i].equals("local"))
                    ...
                    // needed
                    hosts[i] = (isInstanceOfLocal(hosts[i], request)) ? "local"
                            : hosts[i];
    ...
    private final void putResult(String host, String type, String query,
