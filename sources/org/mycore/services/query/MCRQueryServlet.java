...
public class MCRQueryServlet extends MCRServlet {
	...
	private static final String MCRStdSorter =
		"org.mycore.common.xml.MCRXMLSorter";
	...
	public void init() throws MCRConfigurationException {
		...
		conf = super.config;
		collector =
			(MCRQueryCollector) this.getServletContext().getAttribute(
				"QueryCollector");
		if (collector == null)
			collector =
				new MCRQueryCollector(
					conf.getInt("MCR.Collector_Thread_num", 2),
					conf.getInt("MCR.Agent_Thread_num", 6));
	...
	public void doGetPost(MCRServletJob job)
		...
		if (!validateCacheSession(request)) {
			...
			StringBuffer msg =
				new StringBuffer("Requested session is invalid, maybe it was timed out!\n");
			msg
				.append("requested session was: ")
				.append(request.getRequestedSessionId())
				.append("!\n")
				.append("actual session is: ")
				.append(sId)
				.append("!");
			generateErrorPage(
				request,
		...
		String style =
			parameters.getProperty("Style", mode + "-" + layout + "-" + lang);
		...
		if (cachedFlag) {
			...
			if (offset == 0 && size == 0) {
				try {
					offset =
						Integer.parseInt(
							jdom.getRootElement().getAttributeValue("offset"));
					size =
						Integer.parseInt(
							jdom.getRootElement().getAttributeValue("size"));
					logger.debug(
						"Found info about last position in resultlist!");
				} catch (Exception e) {
					logger.warn(
						"Failing to determine preset values of resultlist size and offset!");
					offset = 0;
					size = 0;
				}
			}
		...
		try {
			if (mode.equals("ResultList") && !style.equals("xml")) {
				request.setAttribute(
					MCRLayoutServlet.JDOM_ATTR,
					cutJDOM(jdom, offset, size));
			} else
				request.setAttribute(MCRLayoutServlet.JDOM_ATTR, jdom);
			request.setAttribute("XSL.Style", style);
			RequestDispatcher rd =
	 ...
	 * 								searched Document.
	 */
	private final String getBrowseElementID(
		...
		List elements =
			tempDoc.getRootElement().getChildren(MCRXMLContainer.TAG_RESULT);
		...
		int status =
			((search
				.getAttributeValue(MCRXMLContainer.ATTR_SUCC)
				.equals("true"))
				? 1
				: 0)
				+ ((search
					.getAttributeValue(MCRXMLContainer.ATTR_PRED)
					.equals("true"))
					? 2
					: 0);
	...
	private final Document getObjectMetaDataByID(
		HttpSession session,
		String ID,
		String host) {
		...
		if (session != null && ID != null && host != null) {
			Document jdom =
				(org.jdom.Document) session.getAttribute("CachedList");
			if (jdom == null)
				return null;
			List elements =
				jdom.getRootElement().getChildren(MCRXMLContainer.TAG_RESULT);
			org.jdom.Element search = null;
			boolean found = false;
			while (!elements.isEmpty()) {
				search = (Element) elements.get(0);
			...
			logger.info(
				"MCRQueryServlet: found Element with ID " + ID + " in cache!");
		}
		return MetaData;
	}
	private final MCRXMLContainer sort(MCRXMLContainer xmlcont, String lang) {
		...
		try {
			sorter =
				(MCRXMLSortInterface) (Class
					.forName(
						conf.getString(
							"MCR.XMLSortInterfaceImpl",
							MCRStdSorter)))
					.newInstance();
		...
		if (customSort) {
			logger.info(
				"MCRQueryServlet: CustomSort enalbed. Sorting inorder: "
					+ inOrder);
			sorter.addSortKey(
				replString(SortKey, MCRSorterConfDelim, lang),
				inOrder);
		} else {
			int keynum =
				Integer.parseInt(
					conf.getString(MCRSorterConfPrefix + ".keys.count", "0"));
			boolean inorder = true;
			for (int key = 1; key <= keynum; key++) {
				// get XPATH Expression and hope it's good, if not exist sort for title
				inorder =
					conf.getBoolean(
						MCRSorterConfPrefix + ".keys." + key + ".inorder",
						true);
				sorter.addSortKey(
					replString(
						conf.getString(
							MCRSorterConfPrefix + ".keys." + key,
							"./*/*/*/title[lang('" + lang + "')]"),
						MCRSorterConfDelim,
						lang),
					inorder);
	...
	private static String replString(String parse, String from, String to) {
		...
		if ((result.charAt(0) == '\"')
			...
			for (int i = result.toString().indexOf(from);
				i != -1;
				i = result.toString().indexOf(from)) {
	...
	private final Document cutJDOM(Document jdom, int offset, int size) {
		...
		int amount = size;
		for (int i = offset;
			((amount > 0) && (i < children.size()) && (i < (offset + size)));
			i++) {
		...
		returns
			.getRootElement()
			.setAttribute("count", "" + children.size())
			.setAttribute("offset", "" + offset)
			.setAttribute("size", "" + size);
		return returns;
	}

	private final boolean isInstanceOfLocal(
		String host,
		HttpServletRequest request) {
		String ServletHost = request.getServerName();
		String RemoteHost =
			conf.getString("MCR.remoteaccess_" + host + "_host");
		String ServletPath =
			request.getServletPath().substring(
				...
				+ "/MCRQueryServlet";
		String RemotePath =
			conf.getString("MCR.remoteaccess_" + host + "_query_servlet");
		int ServletPort = request.getServerPort();
		int RemotePort =
			Integer.parseInt(
				conf.getString("MCR.remoteaccess_" + host + "_port"));
		return (
			(RemoteHost.equals(ServletHost))
				&& (RemotePath.equals(ServletPath))
				&& (ServletPort == RemotePort))
			? true
			: false;
	}

	private final boolean checkInputParameter(
		HttpServletRequest request,
		...
		if (att_host != null && (hosts == null || hosts.length == 0)) {
			...
			logger.info(
				"MCRQueryServlet: Try to map remote request to local one!");
			...
			for (int i = 0; i < hosts.length; i++) {
				if (!hosts[i].equals("local"))
					...
					hosts[i] =
						(isInstanceOfLocal(hosts[i], request))
							? "local"
							: hosts[i];
			...
			if (host.indexOf("local") != host.lastIndexOf("local")) {
				logger.info(
					"MCRQueryServlet: multiple \"local\" will be removed by MCRQueryResult!");
			}
		}
		...
		if (SortKey != null) {
			if (request.getParameter(InOrderParam) != null
				&& request.getParameter(InOrderParam).toLowerCase().equals(
					"false"))
				inOrder = false;
			else
				inOrder = true;
	...
	private final Document queryClassification(HttpServletRequest request) {
		String squence =
			conf.getString(
				"MCR.classifications_search_sequence",
				"remote-local");
		MCRXMLContainer resarray = new MCRXMLContainer();
	...
	private final void reSort(
		...
		try {
			...
			generateErrorPage(
				...
				new MCRException("Import of elements failed due to some reason!"),
				false);
			return;
		}
		if (resarray.size() > 0) {
			...
			logger.fatal(
				"MCRQueryServlet: Error while RE-sorting JDOM:"
	...
	private final void browse(
		...
		logger.info(
			"MCRQueryServlet: sending to myself:"
				+ "?mode="
				+ mode
				+ "&status="
				+ StrStatus
