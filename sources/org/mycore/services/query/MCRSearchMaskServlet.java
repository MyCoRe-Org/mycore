...
public class MCRSearchMaskServlet extends MCRServlet {
	...
	protected static Logger LOGGER = Logger
			.getLogger(MCRSearchMaskServlet.class.getName());
	 ...
	 */
	public static String createQueryString(String layout,
		...
		for (int i = 0; i < len; i++) {
			...
			try {
				...
				throw new MCRException(
						"SearchMaskServlet : The field attribute is "
								+ "not a number.");
			}
			...
			for (int j = 0; j < param.size(); j++) {
				...
				if (k == -1) {
					throw new MCRException(
							"SearchMaskServlet : The query attribute "
									+ "has not the elemnt "
									+ ((String) varia.get(j)));
				}
				...
				while (k != -1) {
					if (tempquery.charAt(k - 1) == '\'') {
						...
						qsb.append(tempquery.substring(start, k)).append(
								((String) param.get(j)));
						start = k + l;
					}
			...
			System.out.println("query:" + query.toString() + " tempquery:"
					+ tempquery);
