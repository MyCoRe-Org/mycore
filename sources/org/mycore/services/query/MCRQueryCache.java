...
/**
 * This class makes use of the MCRCache functionality and implements the
 * MCRQueryInterface if someone need this. configure options:
 * MCR.query_cache_capacitity_class: integer of amount of objects stored in the
 * classification cache MCR.query_cache_capacitity_other: integer of amount of
 * other objects stored in cache MCR.query_cache_time: time a "other" object is
 * valid in cache in minutes MCR.query_cache_time_class: time a classification
 * stays valid in cache
 * 
...
public class MCRQueryCache {
    ...
    public synchronized static void init() {
        ...
        queryint = (MCRQueryInterface) config.getInstanceOf("MCR.persistence_" + config.getString("MCR.XMLStore.Type").toLowerCase() + "_merger_name");
        ...
        logger.debug("MCRQueryCache initial capacity: " + queryCache[0].getCapacity() + " & " + queryCache[1].getCapacity());
        logger.debug("MCRQueryCache update time: " + (timePassed_cl / 1000) + " & " + (timePassed / 1000) + " seconds!");
    }

    /*
     * (non-Javadoc)
     * 
     * @see org.mycore.services.query.MCRQueryInterface#getResultList(java.lang.String,
     *      java.lang.String, int)
     */
    public static MCRXMLContainer getResultList(String query, String type, int maxresults) {
        if (!isInitialized()) {
     ...
     *      java.lang.String, int)
     */
    public static MCRXMLContainer getResultList(String host, String query, String type, int maxresults) {
        if (!isInitialized()) {
            // first we need to initialize this cache
            init();
        }
    ...
    private final static MCRXMLContainer getClass(String host, String query) {
        if (isInitialized()) {
            ...
            cacheObject = queryCache[CACHETYPE_CLASS].getIfUpToDate(key, (System.currentTimeMillis() - timePassed_cl));
            ...
            if (cacheObject != null) {
                ...
                synchronized (jdom) {
                    if (!host.equals("local")) {
                        synchronized (comm) {
                            comm = (MCRRemoteAccessInterface) config.getInstanceOf("MCR.remoteaccess_" + host + "_query_class");
                        ...
                        if (jdom != null) {
                            returns.add(host, jdom.getRootElement().getAttributeValue("ID"), 1, (Element) jdom.getRootElement().clone());
                        }
                    } // was not local
    ...
    private final static MCRXMLContainer getOther(String host, String query, String type, int maxresults) {
        if (isInitialized()) {
            MCRXMLContainer returns = new MCRXMLContainer();
            Object cacheObject;
            String key = host + "$" + type + "$" + query;
            cacheObject = queryCache[CACHETYPE_OTHER].getIfUpToDate(key, (System.currentTimeMillis() - timePassed));
    ...
    /**
     * clean the cache this is used by the java garbage collector
     */
    ...
    /**
     * clean the cache this is just does the same as finally()
     * 
     */
    public void close() {
        finalize();
