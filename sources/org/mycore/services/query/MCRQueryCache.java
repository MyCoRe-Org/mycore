...
public class MCRQueryCache {
    ...
    public synchronized static void init() {
        ...
        queryint = (MCRQueryInterface) config.getInstanceOf("MCR.persistence_"
        ...
        timePassed *= 1000;
        timePassed_cl = config.getLong("MCR.query_cache_time_class",
                5 * 60 * 60);
        timePassed_cl *= 1000;
        ...
        logger.debug("MCRQueryCache update time: " + (timePassed_cl / 1000)
     ...
     */
    public static MCRXMLContainer getResultList(String query, String type,
            int maxresults) {
        if (!isInitialized()) {
     ...
     *      java.lang.String, int)
     */
    public static MCRXMLContainer getResultList(String host, String query,
            String type, int maxresults) {
        if (!isInitialized()) {
            //first we need to initialize this cache
            init();
        }
    ...
    private final static MCRXMLContainer getClass(String host, String query) {
        if (isInitialized()) {
            ...
            cacheObject = queryCache[CACHETYPE_CLASS].getIfUpToDate(key,
                    (System.currentTimeMillis() - timePassed_cl));
            if (cacheObject != null) {
                    ...
                    if (!host.equals("local"))
                        synchronized (comm) {
                            comm = (MCRRemoteAccessInterface) config
                                    .getInstanceOf("MCR.remoteaccess_" + host
                                            + "_query_class");
                            returns.importElements(comm.requestQuery(host,
                                    "class", query));
                        }//was local
                    else {
                        synchronized (cl) {
                        ...
                        if (jdom != null) {
                            returns.add(host, jdom.getRootElement()
                                    .getAttributeValue("ID"), 1, (Element) jdom
    ...
    private final static MCRXMLContainer getOther(String host, String query,
            String type, int maxresults) {
        if (isInitialized()) {
            MCRXMLContainer returns = new MCRXMLContainer();
            Object cacheObject;
            String key = host + "$" + type + "$" + query;
            cacheObject = queryCache[CACHETYPE_OTHER].getIfUpToDate(key,
                ...
                if (!host.equals("local"))
                    synchronized (comm) {
                        comm = (MCRRemoteAccessInterface) config
                                .getInstanceOf("MCR.remoteaccess_" + host
                                        + "_query_class");
                        returns.importElements(comm.requestQuery(host, type,
                                query));
                    }
                else
                    synchronized (queryint) {
                        returns.importElements(queryint.getResultList(query,
                                type, maxresults));
                    }
    ...
    public void pullStatus() {
        logger.info(isInitialized() ? "MCRQueryCache initialized"
                : "MCRQueryCache not initialized");
