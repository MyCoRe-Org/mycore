...
public class MCROAIQueryService implements MCROAIQuery {
	...
	public List listSets(String instance) {
		String classificationId = config.getString(STR_OAI_SETSCHEME + "."
				+ instance, "");
	 ...
	 */
	private String[] getHeader(MCRObject object, String objectId,
	...
	private Collection doQuery(String query, String querytype) {
		...
		try {
			...
			qra.importElements(MCRQueryCache.getResultList("local", query,
					querytype, MCRConfiguration.instance().getInt(
							"MCR.query_max_results", 10)));
	...
	private List listRecordsOrIdentifiers(String[] set, String[] from,
		...
		try {
			String restrictionClassification = config
