...
public class MCROAIProvider extends MCRServlet {
	...
	static {
		Properties props = CONFIG
				.getProperties("MCR.oai.repositoryidentifier.");
		oaiConfig = new HashMap();
		for (Enumeration en = props.propertyNames(); en.hasMoreElements();) {
			String propName = (String) en.nextElement();
			String instance = propName
					.substring("MCR.oai.repositoryidentifier.".length());
		...
		oaiResumptionTokenTimeOut = CONFIG.getInt(
				"MCR.oai.resumptiontoken.timeout", 72);
		maxReturns = CONFIG.getInt("MCR.oai.maxreturns", 10);
		resStore = (MCROAIResumptionTokenStore) CONFIG.getInstanceOf(
	...
	//      DINI specification.
	// CHANGED: You should add new LABELS in your classification for each category
	...
	private static final String STR_FIRST_DATE = "2000-01-01T00:00:00Z";
	...
	private Element listFromResumptionToken(Element list,
			String resumptionToken, String prefix, Namespace ns, String listType)
			throws IOException {
		...
		try {
			...
			List result = resStore.getResumptionTokenHits(resumptionTokenID,
			...
			if (listType.equals("set")) {
				...
				while (iterator.hasNext()) {
					...
					if (setArray[4].length() > 0) {
						eSet.addContent(newElementWithContent("setName", ns,
								setArray[4]));
					} else {
						eSet.addContent(newElementWithContent("setName", ns,
								setArray[2]));
					...
					if ((setArray[5] != null) && (setArray[5].length() > 0)) {
						...
						Element eSetDescription = new Element("setDescription",
								ns);
				...
				while (iterator.hasNext()) {
					...
					if ((maxReturns == 0) || (elementCounter <= maxReturns)) {
						...
						eHeader.addContent(newElementWithContent("identifier",
								ns, identifier[0]));
						eHeader.addContent(newElementWithContent("datestamp",
								ns, identifier[1]));
			...
			if (endObject < resumptionSize) {
				...
				String newResumptionToken = resumptionTokenID + "x"
						+ (tokenNo + 1) + "x" + (resumptionSize - endObject);
	...
	private org.jdom.Document identify(HttpServletRequest request,
		...
		if (badArguments(request, 1)) {
			logger
					.info("Es wurden ï¿½berflï¿½ssige Argumente an die Anfrage ï¿½bergeben. Nach OAI 2.0 erfolgt hier ein Abbruch.");
		...
		eIdentify.addContent(newElementWithContent("adminEmail", ns,
				oaiAdminEmail));
	...
	private org.jdom.Document listMetadataFormats(HttpServletRequest request,
		...
		try {
			...
			logger.fatal("Missing configuration item: " + STR_OAI_QUERYSERVICE
					+ " is missing.");
			return null;
		}
	...
	private org.jdom.Document listSets(HttpServletRequest request,
		...
		if (sets != null) {
			...
			if (docs > 0) {
				...
				resStore.createResumptionToken(sResumptionToken, "set",
						getServletName(), resumptionList);
	...
	private org.jdom.Document listIdentifiers(HttpServletRequest request,
		...
		if (metadataPrefix != null) {
			...
			try {
				prefix = resStore.getPrefix(resumptionToken[0].substring(0,
						resumptionToken[0].indexOf("x")));
		...
		do {
			List result = query.listIdentifiers(set, from, until, prefix,
					instance);
			if (result != null) {
				Iterator iterator = result.iterator();
				while (iterator.hasNext()) {
					...
					if ((maxReturns == 0) || (elementCounter <= maxReturns)) {
						...
						String[] array = getHeader(object, objectId,
								repositoryID, instance);
						Element eHeader = new Element("header", ns);
						eHeader.addContent(newElementWithContent("identifier",
								ns, array[0]));
						eHeader.addContent(newElementWithContent("datestamp",
		...
		if (docs > 0) {
			sResumptionToken = newResumptionToken();
			resStore.createResumptionToken(sResumptionToken, prefix, instance,
					resumptionList);
			sResumptionToken += "x1x" + docs;
		} else {
			sResumptionToken = null;
	...
	private org.jdom.Document listRecords(HttpServletRequest request,
		...
		if (docs > 0) {
			sResumptionToken = newResumptionToken();
			resStore.createResumptionToken(sResumptionToken, prefix,
					getServletName(), resumptionList);
			sResumptionToken += "x1x" + docs;
		} else {
			sResumptionToken = null;
	...
	private String legalOAIIdentifier(String identifier) throws MCRException {
		...
		String repositoryIdentifier = getConfigBean(getServletName())
		...
		String allowed = new String(
				"1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ;/?:@&=+$,-_.!~*ï¿½()");
	...
	private Document doMCRXSLTransformation(HttpServletRequest request,
		...
		String webApplicationBaseURL = request.getRequestURL()
				.substring(0, pos)
				+ contextPath;
...
//		}
	...
	public static String[] getHeader(MCRObject object, String objectId,
		...
		List classifications = Arrays.asList(MCROAIProvider.getConfigBean(
				instance).getClassificationIDs());
