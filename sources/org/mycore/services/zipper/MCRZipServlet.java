...
/**
 * This servlet delivers the contents of MycoreObects 
 * to the client as zip-Files. There are three modes
 * a) if id=mycoreobjectID (delivers the metadata, including all derivates)
 * b) if id=derivateID (delivers all files of the derivate)
 * c) if id=derivateID/directoryPath (delivers all files of a special directory of one derivate)
 * 
...
public class MCRZipServlet extends MCRServlet
{
  ...
  **/
  ...
  /**
   * sendZipped 
   *      adds a single File to the ZipOutputStream,
   * 
   * @param file
   ...
   *      */  
    ...
    /**
     * sendZipped 
     *      adds a whole Directory of an Derivate to the ZipOutputStream
     * 
     * @param directory 
    ...
    /**
     * sendZipped 
     *      zips a File with the Metadata of the Object,
     *     makes a XSL-Transformation with the Metadata
     * 
     * @param jdom 
     *            MycoreObject-ResultContainer as org.jdom.Document
     * @param parameters
     *            Parameters, that can be needed in the transforming XSL-Stylesheet 
     *      */       
    protected void sendZipped(Document jdom, Properties parameters, ZipOutputStream out) throws IOException {
    	...
    	out.putNextEntry( ze );
   	    String stylesheetFullPath = getServletContext()
			.getRealPath("/WEB-INF/stylesheets/" + stylesheet);
    ...
    /**
     * sendDerivate sends all files of a derivate or of
     *      a special folder of the derivate
     * 
     * @param ownerID 
     *            the derivateID that should be zipped
     * @param dirpath
     *            relative path zu a special folder, if given, 
     *            only this folder is zipped
     *      */    
    protected void sendDerivate( String ownerID, String dirpath, 
						      						  HttpServletRequest req, HttpServletResponse res, 
													  ZipOutputStream out)	throws IOException, ServletException {
        ...
        if ( root instanceof MCRFile ) {
        	...
        	if ( (dirpath == null) || (dirpath.equals("")) ) {
        		...
        		if (zipdirectory == null ) {
        			...
        			String msg = "Error: could not found the dir: "
                        + dirpath;
                LOGGER.error(msg);
                return;        			
        		}
    ...
    /**
     * sendObject:  zips all derivates of
     *      a Object and a metadata-xml file, that was built 
     *      via a given xsl-stylesheet
     * 
     * @param jdom
    ...
    protected void sendObject( Document jdom, HttpServletRequest req,
		...
		// zip all derivates
        List li = jdom.getRootElement().getChild("mcr_result")
			.getChild("mycoreobject").getChild("structure")
			.getChild("derobjects").getChildren("derobject");
    ...
    /**
     * buildZipOutputStream
     *      sets the contenttype and name of the zip-file
     * Returns the ZipOutputStream     
     * 
     * @param id
     *            the id of the object or derivate that is zipped, builds the name
     * @param dirpath
    ...
    protected ZipOutputStream buildZipOutputStream( HttpServletResponse res, String id, String dirpath) throws IOException {
    	String filename = (dirpath == null || dirpath.equals("")) ? 
    			id + ".zip" : id + "-" + dirpath.replaceAll("/","-") + ".zip"; 
