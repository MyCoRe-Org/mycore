...
public class TextFilterPluginManager {
	...
	private static final Logger LOGGER = Logger
			.getLogger(TextFilterPluginManager.class);
	...
	public void loadPlugins() {
		...
		try {
			classLoader = new URLClassLoader(getPluginURLs(), Thread
		...
		for (Iterator iter = MCRUtils.getProviders(TextFilterPlugin.class,
				classLoader); iter.hasNext();) {
			filter = (TextFilterPlugin) iter.next();
			LOGGER.info(new StringBuffer("Loading TextFilterPlugin: ").append(
					filter.getName()).append(" v:").append(
					filter.getMajorNumber()).append('.').append(
					filter.getMinorNumber()).toString());
			for (Iterator CtIterator = filter.getSupportedContentTypes()
					.iterator(); CtIterator.hasNext();) {
	...
	public TextFilterPlugin getPlugin(MCRFileContentType ct) {
		return (ct == null) ? null : (TextFilterPlugin) CONTENT_TYPE_PLUGIN_BAG
				.get(ct);
	}
	...
	private final URL[] getPluginURLs() throws MalformedURLException {
		...
		if (pluginDir == null || !pluginDir.isDirectory()) {
			LOGGER.info("PluginDirectory does not exist! "
