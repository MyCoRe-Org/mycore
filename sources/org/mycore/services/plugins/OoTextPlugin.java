...
public class OoTextPlugin implements TextFilterPlugin {
    ...
    public OoTextPlugin() {
        ...
        try {
            ...
            throw new FilterPluginInstantiationException(new StringBuffer(
                    "This Plugin is only tested with Xerces").append(
                    "(http://xml.apache.org/xerces2-j/index.html) and").append(
                    " though requires it to be installed somewhere in").append(
                    " CLASSPATH. Please ensure that a jar file ").append(
                    " containing the class ").append(SAXparser).append(
                    " is listed in a CLASSPATH before running your").append(
                    " brandnew MyCoRe(tm)-Application.\n").append(
                    " I as a developer of cause know that Xerces is").append(
                    " bundled with every MyCoRe(tm) release and thus").append(
                    " you will never read this message.\n").append(
                    " But just in case, I thought it is a good idea to")
                    .append(" implement this message here.").toString());
        }
        if (info == null)
            info = new StringBuffer(
                    "This filter extracts the text out of a OpenOffice.org Text Document")
                    .toString();
        ...
        if (getSupportedContentTypes().contains(ct)) {
            try {
                ...
                throw new FilterPluginTransformException(
                        "Error while parsing OpenOffice document.", e);
            } catch (IOException e) {
                throw new FilterPluginTransformException(
                        "Error while parsing OpenOffice document.", e);
            }
    ...
    private InputStream getXMLStream(InputStream inp) throws IOException {
        ...
        if (ze == null || !ze.getName().equals("content.xml"))
            throw new FilterPluginTransformException(
                    "No content.xml was found in OpenOffice.org document!");
    ...
    private Reader getTextReader(InputStream xml) throws IOException,
            SAXException {
    ...
    private static class StringBufferReader extends Reader {
        ...
        public int read(char[] cbuf, int off, int len) throws IOException {
            if (pos == buf.length())
                ...
                int start = pos + off;
                int charsRead = (buf.length() < (start + len)) ? (buf.length() - start)
                        : len;
