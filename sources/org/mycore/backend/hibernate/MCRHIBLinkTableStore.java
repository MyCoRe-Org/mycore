...
public class MCRHIBLinkTableStore implements MCRLinkTableInterface {
	...
	public final void init(String type) throws MCRPersistenceException {
		...
		if (type == null) {
			throw new MCRPersistenceException(
					"The type of the constructor is null");
		}
		type = type.trim();
		...
		if (MCRLinkTableManager.LINK_TABLE_TYPES.length != 2
			...
			throw new IllegalStateException(
					"if you change MCRLinkTableManager, you have to change MCRHIBLinkTableStore too");
		}
		...
		if ("class".equals(type)) {
			...
			throw new MCRPersistenceException(
					"The type of the constructor doesn't match 'class' or 'href'.");
	...
	public final void delete(String from) {
		...
		List l = session.createQuery(
				"from " + classname + " where MCRFROM = '" + from + "'").list();
		int t;
		for (t = 0; t < l.size(); t++) {
	...
	public final int countTo(String to) {
		...
		Transaction tx = session.beginTransaction();
		List l = session.createQuery(
				"from " + classname + " where MCRTO = '" + to + "'").list();
		tx.commit();
		session.close();
		return l.size();
	}
