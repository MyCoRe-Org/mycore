...
public class MCRHIBFileMetadataStore implements MCRFileMetadataStore {
	...
	public void storeNode(MCRFilesystemNode node)
		...
		if (node instanceof MCRFile) {
			...
			throw new MCRPersistenceException(
					"MCRFilesystemNode must be either MCRFile or MCRDirectory");
	...
	public String retrieveRootNodeID(String ownerID)
		...
		List l = session.createQuery(
				"from MCRFSNODES where OWNER = '" + ownerID + "' and PID=NULL")
				.list();
		if (l.size() < 1) {
	...
	public MCRFilesystemNode retrieveChild(String parentID, String name) {
		...
		List l = session.createQuery(
				"from MCRFSNODES where PID = '" + parentID + "' and NAME = '"
						+ name + "'").list();
		if (l.size() < 1) {
			return null;
		}
	...
	public Vector retrieveChildrenIDs(String parentID)
		...
		List l = session.createQuery(
				"from MCRFSNODES where PID = '" + parentID + "'").list();
		if (l.size() < 1) {
			String msg = "MCRSQLUserStore.retrieveUser(): There is no node with PID = "
					+ parentID;
			throw new MCRException(msg);
		}
		tx.commit();
	...
	public void deleteNode(String ID) throws MCRPersistenceException {
		Session session = getSession();
		Transaction tx = session.beginTransaction();
		List l = session.createQuery("from MCRFSNODES where ID = '" + ID + "'")
				.list();
		int t;
		for (t = 0; t < l.size(); t++) {
	...
	public MCRFilesystemNode buildNode(MCRFSNODES node) {
		...
		return MCRFileMetadataManager.instance().buildNode(node.getType(),
				node.getId(), node.getPid(), node.getOwner(), node.getName(),
				node.getLabel(), node.getSize(), greg, node.getStoreid(),
				node.getStorageid(), node.getFctid(), node.getMd5(),
				node.getNumchdd(), node.getNumchdf(), node.getNumchtd(),
				node.getNumchtf());
	}
