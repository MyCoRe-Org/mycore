...
public class MCRHIBXMLStore implements MCRXMLTableInterface {
	...
	public synchronized final void create(MCRObjectID mcrid, byte[] xml,
		...
		System.out.println("Inserting " + mcrid.getId() + "/" + version + "/"
				+ this.type + " into database");
	...
	public synchronized final void delete(MCRObjectID mcrid, int version)
		...
		Session session = getSession();
		Transaction tx = session.beginTransaction();
		MCRXMLTABLE tab = new MCRXMLTABLE(mcrid.getId(), version, this.type,
				null);
		session.delete(tab); //"from MCRXMLTABLE where id='"+mcrid+"' and
							 // version='"+version+"' and type='"+type+"'");
		tx.commit();
		session.close();
	}
	...
	public final byte[] retrieve(MCRObjectID mcrid, int version)
		...
		List l = session.createCriteria(MCRXMLTABLE.class).add(
				Restrictions.eq("key", pk)).list();
		tx.commit();
		session.close();
		if (l.size() > 0)
	...
	public ArrayList retrieveAllIDs(String type) {
		...
		List l = session.createQuery(
				"from MCRXMLTABLE where MCRTYPE = '" + type + "'").list();
