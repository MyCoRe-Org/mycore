...
public class MCRTableGenerator {
	...
	private static Logger logger = Logger
			.getLogger("org.mycore.backend.hibernate");
	 ...
	 */
	public MCRTableGenerator(String tableName, String sqlName, String Package,
			int intPKColumns) {
		try {
	...
	private DocType createDoctype() {
		if (doctype_url == null) {
			try {
				File docFile = new File(new File(System.getProperties()
				...
				throw new MCRException(
						"couldn't create temporary hibernate docType file", e);
			}
		}
		return new DocType("hibernate-mapping",
				"-//Hibernate/Hibernate Mapping DTD//EN", doctype_url);
	}
	 ...
	 */
	public boolean addColumn(String Name, String Column, Type Type, int Length,
			boolean NotNull, boolean Unique, boolean Index) {
	 ...
	 * @return boolean as indicator for errors
	 */
	public boolean addIDColumn(String Name, String Column, Type Type,
			int Length, String Generator, boolean Index) {
	...
	public String getTableName() {
		try {
			return docOut.getRootElement().getChild("class").getAttributeValue(
					"table");
		} catch (Exception e) {
			System.out.println(e.toString());
			return "";
