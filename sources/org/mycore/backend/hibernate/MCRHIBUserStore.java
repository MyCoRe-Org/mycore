...
public class MCRHIBUserStore implements MCRUserStore {
	...
	public synchronized void createUser(MCRUser newUser) throws MCRException {
		...
		try {
			...
			String updateAllowed = (newUser.isUpdateAllowed()) ? "true"
					: "false";
			...
			tx.rollback();
			throw new MCRException("Hibernate error while creating new user.",
					e);
		} finally {
			session.close();
		}
	...
	public synchronized void deleteUser(String delUserID) throws MCRException {
		...
		try {
			...
			List l = session.createQuery(
					"from MCRGROUPMEMBERS where USERID='" + delUserID + "'")
					.list();
			...
			l = session.createQuery(
					"from MCRUSERS where UID='" + delUserID + "'").list();
			if (l.size() == 1) {
			...
			tx.rollback();
			throw new MCRException("Hibernate error while deleting user '"
					+ delUserID + "'.", e);
		} finally {
			session.close();
		}
	...
	public synchronized boolean existsUser(String userID) throws MCRException {
		...
		Transaction tx = session.beginTransaction();
		List l = session.createQuery(
				"from MCRUSERS where UID = '" + userID + "'").list();
		tx.commit();
		session.close();
		if (l.size() > 0)
			return true;
	...
	public synchronized boolean existsUser(int numID, String userID)
		...
		Transaction tx = session.beginTransaction();
		List l = session.createQuery(
				"from MCRUSERS where NUMID = '" + numID + "' or UID = '"
						+ userID + "'").list();
		tx.commit();
		session.close();
		if (l.size() > 0)
	...
	public synchronized MCRUser retrieveUser(String userID) throws MCRException {
		...
		try {
			...
			if (l.size() == 0) {
				...
				l = session.createQuery(
						"from MCRGROUPMEMBERS where USERID = '" + userID + "'")
						.list();
				...
				if (l.size() < 1) {
					...
					throw new MCRException("User with ID = " + userID
							+ " is no groupmember.");
				}
				...
				try {
					retuser = new MCRUser(numID, userID, creator, created,
							modified, id_enabled, update_allowed, description,
							passwd, primaryGroupID, groups, salutation,
							firstname, lastname, street, city, postalcode,
							country, state, institution, faculty, department,
	...
	public synchronized void updateUser(MCRUser updUser) throws MCRException {
		...
		Transaction tx = session.beginTransaction();
		List l = session.createQuery(
				"from MCRUSERS where UID = '" + updUser.getID() + "'").list();
		MCRUSERS user = new MCRUSERS();
		if (l.size() == 1) {
			user = (MCRUSERS) l.get(0);
	...
	public synchronized boolean existsGroup(String groupID) throws MCRException {
		...
		Transaction tx = session.beginTransaction();
		List l = session.createQuery(
				"from MCRGROUPS where GID = '" + groupID + "'").list();
		tx.commit();
		session.close();
		if (l.size() > 0)
			return true;
	...
	public synchronized void deleteGroup(String delGroupID) throws MCRException {
		...
		try {
			List l = session.createQuery(
					"from MCRGROUPS where GID = '" + delGroupID + "'").list();
			if (l.size() == 1) {
			...
			tx.rollback();
			throw new MCRException("Error while deleting group '" + delGroupID
					+ "'.", e);
		} finally {
			session.close();
		}
	...
	public synchronized ArrayList getGroupIDsWithAdminUser(String userID)
		...
		Transaction tx = session.beginTransaction();
		List l = session.createQuery(
				"from MCRGROUPADMINS where USERID = '" + userID + "'").list();
		tx.commit();
		if (l.size() > 0) {
			for (int i = 0; i < l.size(); i++) {
	...
	public synchronized ArrayList getUserIDsWithPrimaryGroup(String groupID)
		...
		Transaction tx = session.beginTransaction();
		List l = session.createQuery(
				"from MCRUSERS where PRIMGROUP = '" + groupID + "'").list();
		tx.commit();
		if (l.size() > 0) {
			for (int i = 0; i < l.size(); i++) {
	...
	public synchronized void updateGroup(MCRGroup group) throws MCRException {
		...
		try {
			...
			List l = session.createQuery(
					"from MCRGROUPS where GID = '" + group.getID() + "'")
					.list();
			...
			l = session.createQuery(
					"from MCRGROUPADMINS where GID='" + group.getID() + "'")
					.list();
			for (int i = 0; i < l.size(); i++) {
			...
			for (int i = 0; i < oldAdminUserIDs.size(); i++) {
				if (!newAdminUserIDs.contains(oldAdminUserIDs.get(i))) {
					int deletedEntities = session.createQuery(
							"delete MCRGROUPADMINS " + "where GID = '"
									+ group.getID() + "'" + " and GROUPID = '"
			...
			for (int i = 0; i < oldUserIDs.size(); i++) {
				if (!newUserIDs.contains(oldUserIDs.get(i))) {
					int deletedEntities = session.createQuery(
							"delete MCRGROUPMEMBERS where GID = '"
									+ group.getID() + "' " + "and USERID ='"
									+ (String) oldUserIDs.get(i) + "'")
							.executeUpdate();
				}
			}
	...
	public synchronized MCRGroup retrieveGroup(String groupID)
		...
		try {
			...
			if (l.size() == 0) {
				...
				l = session.createQuery(
						"from MCRPRIVSLOOKUP where GID = '" + groupID
								+ "' and NAME is not null").list();
				...
				l = session.createQuery(
						"from MCRGROUPADMINS where GID = '" + groupID + "'")
						.list();
				...
				l = session.createQuery(
						"from MCRGROUPMEMBERS where GID = '" + groupID + "'")
						.list();
				...
				l = session.createQuery(
						"from MCRGROUPMEMBERS where GROUPID = '" + groupID
								+ "'").list();
				ArrayList groupIDs = new ArrayList();
				if (l.size() > 0) {
					for (int i = 0; i < l.size(); i++) {
						MCRGROUPMEMBERS grpmembers = (MCRGROUPMEMBERS) l.get(i);
						if (grpmembers.getGid() != null
								&& !grpmembers.getGid().equals("")) {
	...
	public synchronized boolean existsPrivilege(String privName) {
		...
		Transaction tx = session.beginTransaction();
		List l = session.createQuery(
				"from MCRPRIVSM where NAME = '" + privName + "'").list();
		tx.commit();
		session.close();
		if (l.size() > 0)
			return true;
	...
	public ArrayList retrievePrivilegeSet() throws MCRException {
		...
		try {
			...
			for (int i = 0; i < l.size(); i++) {
				...
				thePrivilege = new MCRPrivilege(privs.getName(), privs
						.getDescription());
