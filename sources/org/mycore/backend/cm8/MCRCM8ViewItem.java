...
public class MCRCM8ViewItem implements DKConstantICM {
    ...
    public static void main(String argv[]) throws DKException, Exception {
        ...
        if (argv[0].equals("doc")) {
            if (argv[1].equals("id")) {
                ...
                itemtype = conf.getString("MCR.persistence_cm8_"
                        + mcrid.getTypeId());
                prefix = conf.getString("MCR.persistence_cm8_"
                        + mcrid.getTypeId() + "_prefix");
                query = "/" + itemtype + "[@" + prefix + "ID=\""
                        + mcrid.getId() + "\"]";
                //query =
        ...
        if (argv[0].equals("ifs")) {
            ...
            if (argv[1].equals("nse")) {
                query = "/" + itemtype + "[contains-text (@TIEREF,\"\'"
                        + argv[2] + "\'\")=1]";
            }
            // show all items
        ...
        System.out.println("| All items in " + itemtype + ", server "
                + database);
        ...
        while (iter.more()) {
            System.out.println("|");
            Object ddo = iter.next(); // Move pointer to next element and obtain
                                      // that object.
            processDDO(ddo, "");
        }
    ...
    private static void processDDO(Object obj, String pre) throws Exception {
        System.out.println(pre + "|--+-- Class name: "
        ...
        if (ddo instanceof DKLobICM) {
            ...
            System.out.println(pre + "+  +-- Size: " + lob.getSize()
                    + " Bytes, Mime-Type: " + lob.getMimeType());
            String url[] = lob.getContentURLs(DK_CM_RETRIEVE, DK_CM_CHECKOUT,
        ...
        for (short i = 1; i <= ddo.dataCount(); i++) {
            ...
            if (ddo.getDataNameSpace(i).equals("CHILD")) {
                ...
                System.out.println(pre + "|  |   Cardinality: "
                        + col.cardinality());
            ...
            if (ddo.getDataName(i).equals("DKParts")) {
                DKSequentialCollection col = (DKSequentialCollection) ddo
                        .getData(i);
                System.out.println(pre + "|  |   Cardinality: "
                        + col.cardinality());
                dkIterator iter = col.createIterator();
