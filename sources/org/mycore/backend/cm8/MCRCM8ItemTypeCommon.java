...
final class MCRCM8ItemTypeCommon implements DKConstantICM {
	...
	static final DKTextIndexDefICM getTextDefinition() {
		...
		int text_commitcount = conf.getInt(
				"MCR.persistence_cm8_textsearch_commitcount", 1);
		mcr_item_text_index.setCommitCount(text_commitcount);
		mcr_item_text_index
				.setFormat(DKTextIndexDefICM.TEXT_INDEX_DOC_FORMAT_TEXT);
		int text_ccsid = conf.getInt("MCR.persistence_cm8_textsearch_ccsid",
				850);
		mcr_item_text_index.setIndexCCSID(text_ccsid);
		String text_indexdir = conf.getString(
				"MCR.persistence_cm8_textsearch_indexdir",
				"/home/icmadmin/index");
		mcr_item_text_index.setIndexDir(text_indexdir);
		String text_lang = conf.getString(
				"MCR.persistence_cm8_textsearch_lang", "DE");
		mcr_item_text_index.setIndexLangCode(text_lang);
		int text_minchanges = conf.getInt(
				"MCR.persistence_cm8_textsearch_minchanges", 1);
		mcr_item_text_index.setMinChanges(text_minchanges);
		String text_updatefreq = conf.getString(
				"MCR.persistence_cm8_textsearch_updatefreq", "");
		mcr_item_text_index.setUpdateFrequency(text_updatefreq);
		String text_workingdir = conf.getString(
				"MCR.persistence_cm8_textsearch_workingdir",
				"/home/icmadmin/work");
		mcr_item_text_index.setWorkingDir(text_workingdir);
		logger.debug("CM8 TextSearch - CommitCount = "
				+ mcr_item_text_index.getCommitCount());
		logger.debug("CM8 TextSearch - Format = "
				+ mcr_item_text_index.getFormat());
		logger.debug("CM8 TextSearch - CCSID = "
				+ mcr_item_text_index.getIndexCCSID());
		logger.debug("CM8 TextSearch - IndexDir = "
				+ mcr_item_text_index.getIndexDir());
		logger.debug("CM8 TextSearch - Lang = "
				+ mcr_item_text_index.getIndexLangCode());
		logger.debug("CM8 TextSearch - MinChanges = "
				+ mcr_item_text_index.getMinChanges());
		logger.debug("CM8 TextSearch - UpdateFreq = "
				+ mcr_item_text_index.getUpdateFrequency());
		logger.debug("CM8 TextSearch - WorkingDir = "
	 ...
	 */
	static final boolean createAttributeVarChar(DKDatastoreICM connection,
			String name, int len, boolean search)
			throws Exception {
	 ...
	 * @return If the attribute exists, false was returned, else true.
	 */
	static final boolean createAttributeDate(DKDatastoreICM connection,
			String name) throws Exception {
		if (attributeExists((DKDatastoreDefICM)connection.datastoreDef(), name)) {
			LOGGER.warn("CM8 Datastore attribute already exists: " + name);
			return false;
	 ...
	 * @return If the attribute exists, false was returned, else true.
	 */
	static final boolean createAttributeDouble(DKDatastoreICM connection,
			String name) throws Exception {
		if (attributeExists((DKDatastoreDefICM)connection.datastoreDef(), name)) {
			LOGGER.warn("CM8 Datastore attribute already exists: " + name);
			return false;
	 ...
	 * @return If the attribute exists, false was returned, else true.
	 */
	static final boolean createAttributeInteger(DKDatastoreICM connection,
			String name) throws Exception {
		if (attributeExists((DKDatastoreDefICM)connection.datastoreDef(), name)) {
			LOGGER.warn("CM8 Datastore attribute already exists: " + name);
			return false;
	 ...
	 * @return If the attribute exists, false was returned, else true.
	 */
	static final boolean createAttributeBlob(DKDatastoreICM connection,
			String name, int len, boolean search)
			throws Exception {
		if (attributeExists((DKDatastoreDefICM)connection.datastoreDef(), name)) {
			LOGGER.warn("CM8 Datastore attribute already exists: " + name);
	 ...
	 * @return If the attribute exists, false was returned, else true.
	 */
	static final boolean createAttributeClob(DKDatastoreICM connection,
			String name, int len, boolean search)
			throws Exception {
		if (attributeExists((DKDatastoreDefICM)connection.datastoreDef(), name)) {
			LOGGER.warn("CM8 Datastore attribute already exists: " + name);
	 ...
	 *  
	 */
	static final boolean attributeExists(DKDatastoreDefICM deficm, String name)
