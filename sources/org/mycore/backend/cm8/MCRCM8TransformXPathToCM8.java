...
public class MCRCM8TransformXPathToCM8 implements MCRMetaSearchInterface,
    ...
    protected static Logger logger = Logger
            .getLogger(MCRCM8TransformXPathToCM8.class.getName());
    ...
    public final HashSet getResultIDs(String root, String query, String type) {
        ...
        while (i < l) {
            ...
            if (j == -1) {
                try {
                    cond.append(' ').append(
                            traceOneCondition(query.substring(i, l),
                                    itemtypeprefix));
            ...
            if (ss.equals(" and ")) {
                ...
                if (ss.equals(" or ")) {
                    ...
                    try {
                        cond.append(' ').append(
                                traceOneCondition(ss, itemtypeprefix));
                    } catch (MCRException me) {
                        cond.append(' ');
                    }
                }
        ...
        try {
            ...
            parms[0] = new DKNVPair(DK_CM_PARM_MAX_RESULTS, new Integer(
                    maxresults).toString());
            ...
            DKResults rsc = (DKResults) connection.evaluate(newquery,
            ...
            while (iter.more()) {
                ...
                resitem.retrieve();
                dataId = resitem.dataId(DK_CM_NAMESPACE_ATTR, itemtypeprefix
                        + "ID");
                id = (String) resitem.getData(dataId);
                idmeta.add(new MCRObjectID(id));
            }
        } catch (Exception e) {
            throw new MCRPersistenceException("CM8 Search error."
                    + e.getMessage());
        } finally {
        ...
        for (Iterator it = idmeta.iterator(); it.hasNext();) {
            logger.debug("IDMETA = "
                    + ((String) ((MCRObjectID) it.next()).getId()));
        }
        return idmeta;
    }
    ...
    private final String traceOneCondition(String condstr, String itemtypeprefix) {
        ...
        if (i != -1) {
            ...
            if (j == -1) {
                throw new MCRPersistenceException(
                        "Error while analyze the query string.");
            }
            klammer = true;
        ...
        while ((tippelauf != -1) && (tippelzu != -1)) {
            ...
            value[counter] = new String(cond.substring(tippelauf + 1, tippelzu)
                    .trim());
            boolean opset = false;
            if (!opset) {
            ...
            while (j < k) {
                ...
                if (l == -1) {
                    ...
                    if (nt.length() == 0) {
                        ...
                        sbpath.append(tag[counter].substring(j, tag[counter]
                                .length()));
                    }
                    if (sbpath.length() != 0) {
            ...
            while (j < k) {
                ...
                if (l == -1) {
                    sbpath.append(itemtypeprefix).append(
                            pathout[counter].substring(j, k));
                    pathout[counter] = sbpath.toString();
                    break;
                }
                sbpath.append(itemtypeprefix).append(
                        pathout[counter].substring(j, l + 1));
                j = l + 1;
            }
            ...
            while (j < k) {
                ...
                if (l == -1) {
                    sbpath.append(itemtypeprefix).append(
                            pathin[counter].substring(j, k));
                    pathin[counter] = sbpath.toString();
                    break;
                }
                sbpath.append(itemtypeprefix).append(
                        pathin[counter].substring(j, l + 1));
                j = l + 1;
            }
            ...
            if (tag[counter].equals("text()")) {
                ...
                if (pathin[counter].length() != 0) {
                    ...
                    while (j < k) {
                        ...
                        if (l == -1) {
                            tag[counter] = "@"
                                    + pathin[counter].substring(j + 2, k);
                            break;
                        }
                        j = l + 1;
                    }
                    ...
                    while (j < k) {
                        ...
                        if (l == -1) {
                            tag[counter] = "@"
                                    + pathout[counter].substring(j + 2, k);
                            break;
                        }
                        j = l + 1;
                    }
        ...
        for (i = 0; i < counter; i++) {
            ...
            if (!tag[i].equals("*")) {
                tag[i] = "@" + itemtypeprefix
                        + tag[i].substring(1, tag[i].length());
            }
            // create for CONTAINS
            if (op[i].equals("contains")) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    if (tag[i].equals("*")) {
                        ...
                        sbout.append(tag[i]).append(",\"\'").append(
                                st.nextToken()).append("\'\")=1");
                    }
            ...
            // is value[0] a date
            try {
                GregorianCalendar date = MCRUtils
                        .covertDateToGregorianCalendar(value[i]);
                long number = 0;
                ...
                logger.debug("Date " + value[i] + " as number = "
