  ...
  return idmeta;
	...
	private final String traceOneCondition(
		...
		while ((tippelauf != -1) && (tippelzu != -1)) {
			...
			value[counter] =
				new String(cond.substring(tippelauf + 1, tippelzu).trim());
			boolean opset = false;
			if (!opset) {
		...
		for (i = 0; i < counter; i++) {
			...
			if (op[i].equals("contains")) {
				...
				while (st.hasMoreTokens()) {
					...
					if (tag[i].equals("*")) {
						...
						sbout.append(tag[i]).append(",\"\'").append(st.nextToken()).append(
							"\'\")=1");
					}
			...
			try {
				GregorianCalendar date =
					MCRUtils.covertDateToGregorianCalendar(value[i]);
				long number = 0;
				if (date.get(Calendar.ERA) == GregorianCalendar.AD) {
					number =
						(4000 + date.get(Calendar.YEAR)) * 10000
				...
				logger.debug(
					"Date " + value[i] + " as number = " + Long.toString(number));
				value[i] = Long.toString(number);
				sbout.append(' ').append(op[i]).append(' ').append(value[i]).append(
					bool[i]);
				continue;
			} catch (Exception e) {
			}
