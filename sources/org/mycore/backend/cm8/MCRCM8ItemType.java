...
final class MCRCM8ItemType implements DKConstantICM {
     ...
     */
    static void create(String mcr_type, org.jdom.Document mcr_conf) throws MCRConfigurationException, MCRPersistenceException {
        ...
        try {
            ...
            try {
                ...
                MCRCM8ItemTypeCommon.createAttributeVarChar(connection, mcr_item_type_prefix + "ID", MCRObjectID.MAX_LENGTH, false);

                // create the Attribute for MCR_Label
                MCRCM8ItemTypeCommon.createAttributeVarChar(connection, mcr_item_type_prefix + "label", MCRObject.MAX_LABEL_LENGTH, false);

                // create the Attribut for the TS byte array
                MCRCM8ItemTypeCommon.createAttributeClob(connection, mcr_item_type_prefix + "ts", 100 * 1024, true);

                // create the default attribute type
                MCRCM8ItemTypeCommon.createAttributeVarChar(connection, mcr_item_type_prefix + "type", MCRMetaDefault.DEFAULT_TYPE_LENGTH, false);

                // create the default attribute lang
                MCRCM8ItemTypeCommon.createAttributeVarChar(connection, mcr_item_type_prefix + "lang", MCRMetaDefault.DEFAULT_LANG_LENGTH, false);

                // check for the root itemtype
                try {
                    DKItemTypeDefICM isitemTypeDef = (DKItemTypeDefICM) dsDefICM.retrieveEntity(mcr_item_type_name);

                    if (isitemTypeDef != null) {
                        logger.error("CM8 Datastore Creation itemtype " + mcr_item_type_name + " exist.");
                ...
                DKAttrDefICM attr = (DKAttrDefICM) dsDefICM.retrieveAttr(mcr_item_type_prefix + "ID");
...
//                    DKComponentTypeDefICM item_structure = new DKComponentTypeDefICM(connection);
...
//                        String parasearch = (String) mcr_tag.getAttribute("parasearch").getValue();
...
//                            stb.append(META_PACKAGE_NAME).append("MCRCM8").append(classname.substring(3, classname.length()));
//                            logger.debug("CM8 Datastore Creation: " + tagname + " with class " + stb.toString());
...
//                                DKComponentTypeDefICM item_subtag = ((MCRCM8MetaInterface) obj).createItemType(mcr_subtag, connection, dsDefICM, mcr_item_type_prefix, mcr_item_text_index, "false");
...
//                        String textsearch = (String) mcr_tag.getAttribute("textsearch").getValue();
...
//                                DKComponentTypeDefICM item_subtag = ((MCRCM8MetaInterface) obj).createItemType(mcr_subtag, connection, dsDefICM, mcr_item_type_prefix, mcr_item_text_index, textsearch);
//                                item_tag.addSubEntity(item_subtag);
//                            } catch (ClassNotFoundException e) {
//                                throw new MCRException(classname + " ClassNotFoundException");
//                            } catch (IllegalAccessException e) {
...
//                    DKComponentTypeDefICM item_derivate = new DKComponentTypeDefICM(connection);
