...
public final class MCRCM8SearchStore implements MCRObjectSearchStoreInterface {
     ...
     */
    public final void create(MCRBase obj) throws MCRConfigurationException,
        ...
        String itemtypename = MCRConfiguration.instance().getString(
                sb.toString());
        String itemtypeprefix = MCRConfiguration.instance().getString(
                sb + "_prefix");
        // set up data to item
        ...
        try {
            ...
            boolean test = false;
            try {
                if (new MCRCM8Item(mcr_id.getId(), connection, itemtypename,
                        itemtypeprefix) != null)
                    test = true;
            ...
            for (int i = mcr_tc_counter; i < mcr_tc.getSize(); i++) {
                ...
                if ((mcr_tc.getNameElement(i).equals("metadata"))
                    ...
                    xmlpath[MCRTypedContent.TYPE_MASTERTAG] = itemtypeprefix
                            + "metadata";
                    lastpath = MCRTypedContent.TYPE_MASTERTAG;
                    item.setChild(connection, itemtypename, xmlpath[lastpath],
                            "/", "/" + xmlpath[lastpath] + "/");
                    item.setAttribute("/" + xmlpath[lastpath] + "/",
                            itemtypeprefix + "lang", mcr_tc
                                    .getValueElement(i + 1));
                    i++;
                    continue;
                }
                ...
                if (mcr_tc.getTypeElement(i) > MCRTypedContent.TYPE_MASTERTAG) {
                    xmlpath[mcr_tc.getTypeElement(i)] = new String(
                            itemtypeprefix + mcr_tc.getNameElement(i));
                    ...
                    item.setChild(connection, xmlpath[lastpath - 1],
                            xmlpath[lastpath], sb.toString(), sb.append(
                                    xmlpath[lastpath]).append('/').toString());
                    continue;
                }
                ...
                case MCRTypedContent.FORMAT_DATE:
                    GregorianCalendar cal = (GregorianCalendar) mcr_tc
                            .getValueElement(i);
                    int number = 0;
                    ...
                    // begin for debug
                    Calendar calendar = (GregorianCalendar) mcr_tc
                            .getValueElement(i);
                ...
                case MCRTypedContent.FORMAT_LINK:
                    valueobject = mcr_tc.getValueElement(i);
                    elname = itemtypeprefix + "xlink"
                            + elname.substring(2, elname.length());
                    logger.debug("Attribute : " + sb + "  " + elname + "  "
                            + (String) mcr_tc.getValueElement(i));
                    break;
     ...
     *                if a persistence problem is occured
     */
    public final void update(MCRBase obj) throws MCRConfigurationException,
            MCRPersistenceException {
