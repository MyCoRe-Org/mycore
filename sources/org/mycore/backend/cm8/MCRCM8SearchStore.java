...
public final class MCRCM8SearchStore implements MCRObjectSearchStoreInterface {
    ...
    /**
     * The methode create an object in the search store. The index class is
     * determinated by the type of the object ID. This <b>must </b> correspond
     * with the lower case configuration name. <br>
    ...
    public final void create(MCRBase obj) throws MCRConfigurationException, MCRPersistenceException {
        ...
        try {
            ...
            for (int i = mcr_tc_counter; i < mcr_tc.getSize(); i++) {
                // tag is 'metadata'
                if ((mcr_tc.getNameElement(i).equals("metadata")) && (mcr_tc.getTypeElement(i) == MCRTypedContent.TYPE_MASTERTAG)) {
                    xmlpath[MCRTypedContent.TYPE_MASTERTAG] = itemtypeprefix + "metadata";
                    lastpath = MCRTypedContent.TYPE_MASTERTAG;
                    item.setChild(connection, itemtypename, xmlpath[lastpath], "/", "/" + xmlpath[lastpath] + "/");
                    item.setAttribute("/" + xmlpath[lastpath] + "/", itemtypeprefix + "lang", mcr_tc.getValueElement(i + 1));
                    i++;
                ...
                // tag is 'structure'
                if ((mcr_tc.getNameElement(i).equals("structure")) && (mcr_tc.getTypeElement(i) == MCRTypedContent.TYPE_MASTERTAG)) {
                ...
                // tag is 'service'
                if ((mcr_tc.getNameElement(i).equals("service")) && (mcr_tc.getTypeElement(i) == MCRTypedContent.TYPE_MASTERTAG)) {
                ...
                // tag is 'derivate'
                if ((mcr_tc.getNameElement(i).equals("derivate")) && (mcr_tc.getTypeElement(i) == MCRTypedContent.TYPE_MASTERTAG)) {
                ...
                if (mcr_tc.getTypeElement(i) > MCRTypedContent.TYPE_MASTERTAG) {
                    xmlpath[mcr_tc.getTypeElement(i)] = new String(itemtypeprefix + mcr_tc.getNameElement(i));
                    ...
                    item.setChild(connection, xmlpath[lastpath - 1], xmlpath[lastpath], sb.toString(), sb.append(xmlpath[lastpath]).append('/').toString());
                ...
                case MCRTypedContent.FORMAT_STRING:
                    ...
                    logger.debug("Attribute : " + sb + "  " + elname + "  " + (String) mcr_tc.getValueElement(i));
                ...
                case MCRTypedContent.FORMAT_DATE:
                    ...
                    if (cal.get(Calendar.ERA) == GregorianCalendar.AD) {
                        number = ((4000 + cal.get(Calendar.YEAR)) * 10000) + (cal.get(Calendar.MONTH) * 100) + cal.get(Calendar.DAY_OF_MONTH);
                    } else {
                        number = ((4000 - cal.get(Calendar.YEAR)) * 10000) + (cal.get(Calendar.MONTH) * 100) + cal.get(Calendar.DAY_OF_MONTH);
                    }
                ...
                case MCRTypedContent.FORMAT_NUMBER:
                    valueobject = mcr_tc.getValueElement(i);
                    logger.debug("Attribute : " + sb + "  " + elname + "  " + ((Double) mcr_tc.getValueElement(i)).toString());
    ...
    /**
     * The methode delete an object from the data store. The index class is
     * determinated by the type of the object ID. This <b>must </b> correspond
     * with the lower case configuration name. <br>
     * As example: Document --> MCR.persistence_cm8_document
     * 
     ...
     *                if a persistence problem is occured
     */
    public final void delete(MCRObjectID mcr_id) throws MCRConfigurationException, MCRPersistenceException {
        Logger logger = MCRCM8ConnectionPool.getLogger();
    ...
    /**
     * The methode update an object in the data store. The index class is
     * determinated by the type of the object ID. This <b>must </b> correspond
     * with the lower case configuration name. <br>
     * As example: Document --> MCR.persistence_cm8_document
     * 
