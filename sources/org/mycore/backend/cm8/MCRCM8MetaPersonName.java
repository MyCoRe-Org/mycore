...
public class MCRCM8MetaPersonName implements DKConstantICM, MCRCM8MetaInterface {
    ...
    public DKComponentTypeDefICM createItemType(org.jdom.Element element,
        ...
        try {
            ...
            MCRCM8ItemTypeCommon.createAttributeVarChar(connection, name, len,
                    ts);
            ...
            // add the person child component fullname
            ...
            it.setName(name);
            it.setDeleteRule(DK_ICM_DELETE_RULE_CASCADE);
            if (!MCRCM8ItemTypeCommon.createAttributeVarChar(connection, name,
                    len, ts)) {
                logger.warn("CM8 Datastore Creation attribute " + name
                        + " already exists.");
            }
            attr = (DKAttrDefICM) dsDefICM.retrieveAttr(name);
            attr.setNullable(true);
            ...
            // add the person child component prefix
            name = prefix + "prefix";
            it = new DKComponentTypeDefICM(connection);
            it.setName(name);
            it.setDeleteRule(DK_ICM_DELETE_RULE_CASCADE);
