...
public class MCRJDOMTransformXPathToJDOM implements MCRMetaSearchInterface {
	...
	protected static Logger logger = Logger
			.getLogger(MCRJDOMTransformXPathToJDOM.class.getName());
	...
	public final HashSet getResultIDs(String root, String query, String type) {
		...
		logger.debug("" + objects.size() + " objects of type " + type
				+ " available");
		org.jdom.Document stylesheet = store.getStylesheet(xsltquery);

		javax.xml.transform.Transformer trans = null;
		try {
			trans = transfakt.newTransformer(new org.jdom.transform.JDOMSource(
					stylesheet));
		} catch (Exception ex) {
		...
		while (ids.hasMoreElements()) {
			...
			try {
				trans
						.transform(new org.jdom.transform.JDOMSource(obj),
								jdomres);
			} catch (TransformerException ex) {
				String msg = "Error executing XSL query";
				throw new MCRException(msg, ex);
			}
			String result = jdomres.getDocument().getRootElement()
					.getTextTrim();
	...
	private final String getXSLTArgument(String query) {
		...
		while (i < l) {
			...
			if (j == -1) {
				try {
					cond.append(' ').append(
							traceOneCondition(newquery.substring(i, l)));
	...
	private final String traceOneCondition(String condstr) {
		...
		for (i = 0; i < counter; i++) {
			...
			if (test != null) {
				value[i] = test;
				sbout
						.append("java:org.mycore.backend.jdom.MCRXalanExtensions.compareDates(");
				if (pathin[i].length() != 0) {
					sbout.append(pathin[i]).append('/');
				}
				sbout.append(newtag).append(",\'").append(value[i]).append(
						"\',\'").append(op[i]).append("\') ").append(bool[i]);
				continue;
			}
			// numerical
			if (op[i].equals("<") || op[i].equals(">") || op[i].equals("<=")
				...
				sbout.append(newtag).append(' ').append(op[i]).append(' ')
						.append(value[i]).append(' ').append(bool[i]);
				continue;
			}
			// general
			if (pathin[i].length() != 0) {
