...
public class MCRJDOMTransformXPathToJDOM implements MCRMetaSearchInterface
{
  ...
  /**
   * This method start the Query over the memory search store for one
   * object type and returns the query result as HashSet of MCRObjectIDs.
   *
   * @param root
   *            the query root
   * @param query
  ...
  public final HashSet getResultIDs( String root, String query, String type )
  {
    ...
    while( ids.hasMoreElements() )
    {
      ...
      if( "true".equals( result ) ) idmeta.add( id );
    }
  ...
  private final String traceOneCondition( String condstr )
  {
    ...
    if( i != -1 )
    {
      ...
      if( j == -1 ) { throw new MCRPersistenceException(
    ...
    for( i = 0; i < counter; i++ )
    {
      ...
      if( op[i].equals( "contains" ) || op[i].equals( "like" ) )
      {
        ...
        sbout.append( newtag ).append( ",\'" ).append(
            MCRUtils.replaceString( value[i], "*", "" ) ).append( "\')" );
        sbout.append( bool[i] );
        continue;
      }
      // date
