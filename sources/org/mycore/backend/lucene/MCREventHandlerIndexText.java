 ...
 * TextFilterPluginManager is used. If MRCFile can be converted in text it will
 * be indexed.
 * 
...
public class MCREventHandlerIndexText extends MCREventHandlerBase {
	...
	private static Logger logger = Logger
			.getLogger(MCREventHandlerIndexText.class);
	...
	/**
	 * Handles file created events. This implementation builds fulltext index of
	 * file content with lucene.
	 * 
	 * @param evt
	...
	protected Document buildLuceneDocument(MCRFile file) {
		...
		Field derivateID = new Field("DerivateID", file.getOwnerID(), true,
				true, false);
		...
		try {
			BufferedReader in = new BufferedReader(PLUGIN_MANAGER.transform(
					file.getContentType(), file.getContentAsInputStream()));
			/*
	...
	protected void handleFileUpdated(MCREvent evt, MCRFile file) {
		logger
				.debug("This default handler implementation does nothing (UPDATE TEXT)");
