...
public class LuceneCStoreQueryParser extends QueryParser {
    ...
    public void setGroupingValue(String string) {
        ...
        logger.debug(new StringBuffer("Set ").append(GROUPING_FIELD).append(" to ").append(string).append(" for next query...").toString());
    }
    ...
    protected Query getBooleanQuery(Vector clauses) throws ParseException {
        ...
        for (int i = 0; i < clauses.size(); i++) {
            ...
            combiner = new BooleanClause(WS_PARSER.getFieldQuery(GROUPING_FIELD, groupingValue), true, false);
    ...
    public BooleanQuery[] getBooleanQueries(String query) throws ParseException {
        ...
        for (int i = 0; i < clauses.length; i++) {
            if ((clauses[i].prohibited == true) || ((clauses.length > 1) && (clauses[i].required == true))) {
                throw new ParseException("Queries should be OR linked: " + clauses[i].prohibited + ":" + clauses[i].required + "\n" + clauses[i].query.toString(field));
            }
