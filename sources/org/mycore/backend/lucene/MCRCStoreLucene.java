 ...
 */
public class MCRCStoreLucene extends MCRCStoreLocalFilesystem implements
        MCRTextSearchInterface {
    ...
    public String[] getDerivateIDs(String docTextQuery) {
        ...
        try {
            while (it.hasNext()) {
                ...
                if (ok) {
                    ...
                    if (derivateID != null) {
                        ...
                        LOGGER
                                .warn("Found Document containes no Field \"DerivateID\":"
                                        + doc);
                    }
                }
                //				else{
            ...
            throw new MCRPersistenceException("IOException while query:"
                    + docTextQuery, e);
        }
        return returns;
    }
    ...
    public void init(String storeID) {
        ...
        try {
            ...
            LOGGER.error(
                    "Error while getting last modified info from IndexDir", e);
        }
    }
    ...
    protected String doStoreContent(MCRFileReader file,
        ...
        if (!PLUGIN_MANAGER.isSupported(file.getContentType())) {
            throw new MCRPersistenceException(
                    new StringBuffer(file.getContentTypeID())
                            .append(
                                    " is not supported by any TextFilterPlugin detected")
                            .append(" by the TextFilterPluginManager.\n")
                            .append(
                                    "Make sure you have a Plugin installed in the proper directory:")
                            .append(
                                    CONF.getString("MCR.PluginDirectory",
                                            "(not configured yet)"))
                            .append(
                                    "\nIf you don't have the right Plugin ready, reasign \"")
                            .append(file.getContentTypeID()).append(
                                    "\" to another ContentStore.\n").append(
                                    "Read the manual on how to do this!")
                            .toString());
        }
        Document doc = null;
        ...
        if (returns == null || returns.length() == 0)
            throw new MCRPersistenceException("Failed to store file "
                    + file.getID() + " to local file system!");
    ...
    private Hits[] getHitsForDerivate(String derivateID, String queryText) {
        ...
        LuceneCStoreQueryParser parser = new LuceneCStoreQueryParser("content",
                analyzer);
        ...
        try {
            ...
            StringBuffer msg = new StringBuffer("Error while querying (")
                    .append(queryText).append(
                            ") over Files matching DerivateID=").append(
                            derivateID).append("!");
    ...
    private HashSet getUniqueFieldValues(String fieldName, String query) {
        HashSet collector = new HashSet();
        if (fieldName == null || query == null || fieldName.length() == 0
                || query.length() == 0)
            return collector;
        int size = 0;
        ...
        try {
            Hits hits = INDEX_SEARCHER.search(QueryParser.parse(biggestSub,
                    "content", getAnalyzer()));
            String[] values;
            for (int i = 0; i < hits.length(); i++) {
                values = hits.doc(i).getValues(fieldName);
                if (values == null) {
                    LOGGER.warn("Found a document but " + fieldName
                            + " was not stored in!");
                } else {
            ...
            StringBuffer msg = new StringBuffer(
                    "Error while fetching unique values of field ").append(
                    fieldName).append("!");
            throw new MCRPersistenceException(msg.toString(), e);
        } catch (IOException e) {
            StringBuffer msg = new StringBuffer(
    ...
    private void indexDocument(Document doc) throws IOException {
        INDEX_SEARCHER.close();
        loadIndexWriter();
        LOGGER.debug("Create index for storageID="
    ...
    private synchronized void loadIndexReader() {
        try {
            ...
            throw new MCRPersistenceException("Cannot read index in "
                    + INDEX_DIR.getAbsolutePath() + File.pathSeparatorChar
                    + INDEX_DIR.getName(), e);
        }
    }
    ...
    private synchronized void loadIndexWriter() {
        ...
        try {
            ...
            throw new MCRPersistenceException("Cannot create index in "
                    + INDEX_DIR.getAbsolutePath(), e);
        }
