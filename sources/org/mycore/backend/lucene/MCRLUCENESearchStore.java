 ...
 */
public final class MCRLUCENESearchStore implements MCRObjectSearchStoreInterface
{
  static final private Logger logger = Logger.getLogger(MCRLUCENESearchStore.class.getName());
  ...
  {
    logger.debug("MCRLUCENESearchStore create: MCRObjectID    : " + obj.getId().getId());
    logger.debug("MCRLUCENEPersistence create: MCRLabel       : " + obj.getLabel());
...
/*
    ...
    { 
      ...
      outputter.output(new org.jdom.Document(root), System.out);
    } catch (Exception ex)
    {
      ex.printStackTrace();
    }
*/
    ...
    {
      throw new MCRPersistenceException("Error Creating search entry in lucene " + ex.getMessage(),
          ex);
    }
  }
  ...
  /**
   * Updates the searchable content in the database. Currently this is the same
   * like delete and then a new create. Should be made with XUpdate in the
   * future.
   * 
   * @param obj
   *          the MCRObject to put in the search store
  ...
  {
    logger.debug("MCRLUCENEPersistence delete: MCRObjectID    : " + mcr_id.getId());
    logger.info("MCRLUCENEPersistence delete: MCRObjectID    : " + mcr_id.getId());
    try
    {
   ...
   * @param root object to be to be transformed
   * 
