...
public class MCRServletCommunication implements MCRRemoteAccessInterface {
    ...
    private final boolean readConnectionData(String hostAlias) {
        ...
        if (realhost.length() == 0) {
            logger.error("Connection data for host " + hostAlias
                    + " not found.");
            return false;
        }
        protocol = config.getString("MCR.remoteaccess_" + hostAlias
                + "_protocol", "");
        ...
        if (!protocol.equals("http")) {
            logger.error("Connection protocol for host " + hostAlias
                    + " is not HTTP.");
            return false;
        }
        ...
        if (port == 0) {
            logger
                    .error("Connection port for host " + hostAlias
                            + " is false.");
            return false;
        }
        return true;
    ...
    public MCRXMLContainer requestQuery(String hostAlias, String reqtype,
        ...
        if (location.length() == 0) {
            logger.error("Connection location for host " + hostAlias
                    + " not found.");
            return result;
        }
        URL currentURL;
     ...
     *             IOException
     */
    private int getConnectionStatus(PostMethod method) throws URIException,
            IOException {
        int statusCode = -1;
    ...
    public final BufferedInputStream requestIFS(String hostAlias, String path)
        ...
        if (path.length() == 0) {
            logger
                    .error("Connection path for host " + hostAlias
                            + " is empty.");
            return in;
        }
        ...
        try {
            ...
            if (getConnectionStatus(method) == -1) {
                ...
                headercontext = method.getResponseHeader("Content-Type")
                        .getValue();
                hasifsdata = true;
