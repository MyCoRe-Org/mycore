/**
 * $RCSfile$ $Revision$ $Date: 2005/04/27
 * 08:22:17 $
 * 
...
public class MCRSQLFileMetadataStore implements MCRFileMetadataStore {
    ...
    private void createTable() {
        MCRSQLConnection.justDoUpdate(new MCRSQLStatement(table).addColumn(
                "ID CHAR(16) NOT NULL PRIMARY KEY").addColumn("PID CHAR(16)")
                .addColumn("TYPE CHAR(1) NOT NULL").addColumn(
                        "OWNER VARCHAR("
                                + Integer.toString(MCRObjectID.MAX_LENGTH)
                                + ") NOT NULL").addColumn(
                        "NAME VARCHAR(250) NOT NULL").addColumn(
                        "LABEL VARCHAR(250)").addColumn("SIZE BIGINT NOT NULL")
                .addColumn("DATE TIMESTAMP NOT NULL").addColumn(
                        "STOREID VARCHAR(32)").addColumn(
                        "STORAGEID VARCHAR(250)")
                .addColumn("FCTID VARCHAR(32)").addColumn("MD5 CHAR(32)")
    ...
    public void storeNode(MCRFilesystemNode node)
        ...
        if (node instanceof MCRFile) {
            ...
            TYPE = "D";
            NUMCHDD = dir.getNumChildren(MCRDirectory.DIRECTORIES,
                    MCRDirectory.HERE);
            NUMCHDF = dir.getNumChildren(MCRDirectory.FILES, MCRDirectory.HERE);
            NUMCHTD = dir.getNumChildren(MCRDirectory.DIRECTORIES,
                    MCRDirectory.TOTAL);
            NUMCHTF = dir
                    .getNumChildren(MCRDirectory.FILES, MCRDirectory.TOTAL);
        }
    ...
    public MCRFilesystemNode retrieveNode(String ID)
        ...
        MCRSQLRowReader reader = MCRSQLConnection
                .justDoQuery(new MCRSQLStatement(table).setCondition("ID", ID)
                        .toSelectStatement());
    ...
    public MCRFilesystemNode buildNodeFromReader(MCRSQLRowReader reader) {
        ...
        return MCRFileMetadataManager.instance().buildNode(objectType, ID,
                parentID, ownerID, name, label, size, date, storeID, storageID,
                fctID, md5, numchdd, numchdf, numchtd, numchtf);
    }

    public String retrieveRootNodeID(String ownerID)
            throws MCRPersistenceException {
        MCRSQLRowReader reader = MCRSQLConnection
                .justDoQuery(new MCRSQLStatement(table).setCondition("PID",
                        null).setCondition("OWNER", ownerID)
                        .toSelectStatement());
    ...
    public MCRFilesystemNode retrieveChild(String parentID, String name) {
        MCRSQLRowReader reader = MCRSQLConnection
                .justDoQuery(new MCRSQLStatement(table).setCondition("PID",
                        parentID).setCondition("NAME", name)
                        .toSelectStatement());
