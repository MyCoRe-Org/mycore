...
public class MCRSQLLinkTableStore implements MCRLinkTableInterface {
	...
	// logger
	static Logger logger = Logger.getLogger(MCRSQLLinkTableStore.class
			.getName());
	...
	public final void init(String type) throws MCRPersistenceException {
		...
		mytype = type;
		// set configuration
		tableName = config.getString("MCR.linktable_store_sql_table_" + type,
				"MCRLINKTABLE");
		...
		try {
			if (mytype.equals("class")) {
				c.doUpdate(new MCRSQLStatement(tableName).addColumn(
						"MCRFROM VARCHAR(" + Integer.toString(lengthObjectID)
								+ ") NOT NULL").addColumn(
						"MCRTO VARCHAR("
								+ Integer.toString(lengthClassID
										+ lengthCategID + 2) + ") NOT NULL")
						.addColumn("PRIMARY KEY (MCRFROM,MCRTO)")
						.toCreateTableStatement());
				c.doUpdate(new MCRSQLStatement(tableName).addColumn("MCRFROM")
						.addColumn("MCRTO").toIndexStatement());
			} else {
				c
						.doUpdate(new MCRSQLStatement(tableName).addColumn(
								"MCRFROM VARCHAR("
										+ Integer.toString(lengthObjectID)
										+ ") NOT NULL").addColumn(
								"MCRTO VARCHAR("
										+ Integer.toString(lengthObjectID)
										+ ") NOT NULL").addColumn(
								"PRIMARY KEY (MCRFROM,MCRTO)")
								.toCreateTableStatement());
				c.doUpdate(new MCRSQLStatement(tableName).addColumn("MCRFROM")
						.addColumn("MCRTO").toIndexStatement());
			}
	...
	public final void create(String from, String to) {
		if ((from == null) || ((from = from.trim()).length() == 0)) {
			throw new MCRPersistenceException(
					"The from value is null or empty.");
		}
		...
		try {
			MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableName)
					.setValue("MCRFROM", from).setValue("MCRTO", to)
					.toInsertStatement());
		} catch (Exception e) {
	...
	public final void delete(String from) {
		...
		MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableName)
				.setCondition("MCRFROM", from).toDeleteStatement());
	}
