...
public class MCRSQLClassificationStore implements MCRClassificationInterface {
    ...
    private final void createClass() {
        ...
        try {
            ...
            c.doUpdate(new MCRSQLStatement(tableClass).addColumn(
                    "ID VARCHAR(" + Integer.toString(lengthClassID)
    ...
    private final void createCateg() {
        ...
        try {
            ...
            c.doUpdate(new MCRSQLStatement(tableCateg).addColumn(
                    "ID VARCHAR(" + Integer.toString(lengthCategID)
                            + ") NOT NULL").addColumn(
                    "CLID VARCHAR(" + Integer.toString(lengthClassID)
                            + ") NOT NULL").addColumn(
                    "PID VARCHAR(" + Integer.toString(lengthCategID) + ")")
                    .addColumn(
                            "URL VARCHAR(" + Integer.toString(lengthURL) + ")")
                    .addColumn("PRIMARY KEY ( CLID, ID )")
                    .toCreateTableStatement());
            c.doUpdate(new MCRSQLStatement(tableCateg).addColumn("CLID")
    ...
    private final void createClassLabel() {
        ...
        try {
            // the classification table
            c
                    .doUpdate(new MCRSQLStatement(tableClassLabel)
                            .addColumn(
                                    "ID VARCHAR("
                                            + Integer.toString(lengthClassID)
                                            + ") NOT NULL")
                            .addColumn(
                                    "LANG VARCHAR("
                                            + Integer.toString(lengthLang)
                                            + ") NOT NULL")
                            .addColumn(
                                    "TEXT VARCHAR("
                                            + Integer.toString(lengthText)
                                            + ")")
                            .addColumn(
                                    "MCRDESC VARCHAR("
                                            + Integer
                                                    .toString(lengthDescription)
                                            + ")").addColumn(
                                    "PRIMARY KEY ( ID, LANG )")
                            .toCreateTableStatement());
            c.doUpdate(new MCRSQLStatement(tableClassLabel).addColumn("ID")
                    .addColumn("LANG").toIndexStatement());
        } finally {
            c.release();
        }
    }
    ...
    private final void createCategLabel() {
        ...
        try {
            // the category table
            c
                    .doUpdate(new MCRSQLStatement(tableCategLabel)
                            .addColumn(
                                    "ID VARCHAR("
                                            + Integer.toString(lengthCategID)
                                            + ") NOT NULL")
                            .addColumn(
                                    "CLID VARCHAR("
                                            + Integer.toString(lengthClassID)
                            ...
                            .addColumn(
                                    "MCRDESC VARCHAR("
                                            + Integer
                                                    .toString(lengthDescription)
                                            + ")").addColumn(
                                    "PRIMARY KEY ( CLID, ID, LANG )")
                            .toCreateTableStatement());
            c.doUpdate(new MCRSQLStatement(tableCategLabel).addColumn("CLID")
    ...
    public final void createClassificationItem(
        ...
        MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableClass).setValue(
                "ID", classification.getID()).toInsertStatement());
        for (int i = 0; i < classification.getSize(); i++) {
            MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableClassLabel)
                    .setValue("ID", classification.getID()).setValue("LANG",
                            classification.getLang(i)).setValue("TEXT",
                            classification.getText(i)).setValue("MCRDESC",
    ...
    public void deleteClassificationItem(String ID) {
        MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableClass)
                .setCondition("ID", ID).toDeleteStatement());
        MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableClassLabel)
                .setCondition("ID", ID).toDeleteStatement());
        MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableCateg)
                .setCondition("CLID", ID).toDeleteStatement());
    ...
    public final MCRClassificationItem retrieveClassificationItem(String ID) {
        MCRSQLRowReader reader = MCRSQLConnection
                .justDoQuery(new MCRSQLStatement(tableClass).setCondition("ID",
                        ID).toSelectStatement());
        if (!reader.next())
            return null;
        MCRClassificationItem c = new MCRClassificationItem(ID);
        reader = MCRSQLConnection.justDoQuery(new MCRSQLStatement(
                tableClassLabel).setCondition("ID", ID).toSelectStatement());
        while (reader.next()) {
    ...
    public final boolean classificationItemExists(String ID) {
        return MCRSQLConnection.justCheckExists(new MCRSQLStatement(tableClass)
                .setCondition("ID", ID).toRowSelector());
    }
    ...
    public final void createCategoryItem(MCRCategoryItem category) {
        try {
            MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableCateg)
                    .setValue("ID", category.getID()).setValue("CLID",
                            category.getClassificationID()).setValue("PID",
                            category.getParentID()).setValue("URL",
                            category.getURL()).toInsertStatement());
        } catch (Exception e) {
            MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableCateg)
                    .setValue("ID", category.getID()).setValue("CLID",
                            category.getClassificationID()).setValue("PID",
                            category.getParentID()).toInsertStatement());
        }
        for (int i = 0; i < category.getSize(); i++) {
            MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableCategLabel)
                    .setValue("ID", category.getID()).setValue("CLID",
                            category.getClassificationID()).setValue("LANG",
                            category.getLang(i)).setValue("TEXT",
                            category.getText(i)).setValue("MCRDESC",
                            category.getDescription(i)).toInsertStatement());
        }
    }
    ...
    public final void deleteCategoryItem(String CLID, String ID) {
        MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableCateg)
                .setCondition("CLID", CLID).setCondition("ID", ID)
                .toDeleteStatement());
        MCRSQLConnection.justDoUpdate(new MCRSQLStatement(tableCategLabel)
                .setCondition("CLID", CLID).setCondition("ID", ID)
                .toDeleteStatement());
    ...
    public final MCRCategoryItem retrieveCategoryItem(String CLID, String ID) {
        MCRSQLRowReader reader = MCRSQLConnection
                .justDoQuery(new MCRSQLStatement(tableCateg).setCondition("ID",
                        ID).setCondition("CLID", CLID).toSelectStatement());
        if (!reader.next())
            return null;
        ...
        c.setURL(URL);
        reader = MCRSQLConnection.justDoQuery(new MCRSQLStatement(
                tableCategLabel).setCondition("ID", ID).setCondition("CLID",
                CLID).toSelectStatement());
        if (!reader.next())
            return null;
        do {
            String lang = reader.getString("LANG");
    ...
    public MCRCategoryItem retrieveCategoryItemForLabelText(String CLID,
            String labeltext) {
        MCRSQLRowReader reader = MCRSQLConnection
                .justDoQuery(new MCRSQLStatement(tableCategLabel).setCondition(
                        "TEXT", labeltext).setCondition("CLID", CLID)
                        .toSelectStatement());
        if (!reader.next())
            return null;
        ...
        c.addData(lang, text, desc);
        reader = MCRSQLConnection.justDoQuery(new MCRSQLStatement(tableCateg)
                .setCondition("ID", ID).setCondition("CLID", CLID)
                .toSelectStatement());
        if (!reader.next())
            return null;
        String URL = "";
    ...
    public final ArrayList retrieveChildren(String CLID, String PID) {
        ...
        MCRSQLRowReader reader = MCRSQLConnection
                .justDoQuery(new MCRSQLStatement(tableCateg).setCondition(
                        "PID", PID).setCondition("CLID", CLID)
                        .toSelectStatement());
        logger.debug("Generiertes Statement: "
                + new MCRSQLStatement(tableCateg).setCondition("PID", PID)
                        .setCondition("CLID", CLID).toSelectStatement());
        while (reader.next()) {
            String ID = reader.getString("ID");
            String URL = "";
            try {
        ...
        for (int i = 0; i < children.size(); i++) {
            reader = MCRSQLConnection.justDoQuery(new MCRSQLStatement(
                    tableCategLabel).setCondition("ID",
                    ((MCRCategoryItem) children.get(i)).getID()).setCondition(
                    "CLID", CLID).toSelectStatement());
            while (reader.next()) {
                String lang = reader.getString("LANG");
                String text = reader.getString("TEXT");
                String desc = reader.getString("MCRDESC");
    ...
    public final int retrieveNumberOfChildren(String CLID, String PID) {
        ...
        return MCRSQLConnection.justCountRows(new MCRSQLStatement(tableCateg)
                .setCondition("PID", PID).setCondition("CLID", CLID)
                .toRowSelector());
    ...
    public final String[] getAllClassificationID() {
        ...
        MCRSQLRowReader reader = MCRSQLConnection
                .justDoQuery(new MCRSQLStatement(tableClass).addColumn("ID")
                //  .setCondition( "ID", "*" )
                        .toSelectStatement());
