 ...
 * MCRFile objects on a filesystem supported by Apache Jakarta Commons VFS. 
 * The connection parameters are configured in mycore.properties:
 *
 * <code>
...
public class MCRStoreVFS extends MCRContentStore {
	...
	protected String doStoreContent(MCRFileReader file,
		...
		FileObject targetObject = fsManager.resolveFile(baseDir, 
				storageId.toString());
	...
	protected void doDeleteContent(String storageId) throws Exception {
		...
		targetObject.delete();
		while (!parent.getName().getPathDecoded().equals(baseDir.getName().getPathDecoded())) {
			targetObject = parent;
	...
	public void init(String storeId) {
		...
		try {
			...
			StringBuffer baseDirName = new StringBuffer(method).
					append("://");
			...
			if (!baseDir.isWriteable()) {
				String msg = "Content store base directory must be writable: " + baseDirectory;
