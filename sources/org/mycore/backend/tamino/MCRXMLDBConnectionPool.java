...
public class MCRXMLDBConnectionPool
  ...
  /** The logger */
  private static Logger logger=Logger.getLogger("org.mycore.backend.taminoXMLDB");

  private static String conf_prefix = "MCR.persistence_taminoxmldb_";
  ...
  protected MCRXMLDBConnectionPool() throws MCRPersistenceException
    {
     ...
     logger.info( "Building connection pool singleton to XML:DB..." );
     driver     = config.getString( conf_prefix + "driver" );
     logger.info("MCRXMLDBConnectionPool MCR.persistence_taminoxmldb_driver      : " + driver);
     connString = config.getString( conf_prefix + "database_url" , "");
     logger.info("MCRXMLDBConnectionPool MCR.persistence_taminoxmldb_database_url: " + connString);
 //    String DATABASE_URI1="http"+connString.substring(12);
 //    logger.info("MCRXMLDBConnectionPool MCR.persistence_taminoxmldb_database_url:"+DATABASE_URI1);
     try
     {
  ...
  protected Collection buildConnection( String collection )
    {
     ...
     catch( Exception ex )
     {
      ...
      logger.info( "Could not connect to XML:DB...catch String msg:"+msg  );
 ...
 */
  ...
  /**
   * The method return the logger for org.mycore.backend.taminoxmldb.
   *
   * @return the logger.
   **/
