...
public class MCRXMLDBQuery implements MCRQueryInterface {
	...
	maxres = config.getInt( "MCR.query_max_results", MAX_RESULTS );
        ...
        database   = config.getString( "MCR.persistence_taminoxmldb_database" , "");
        logger.info("MCRXMLDBQuery MCR.persistence_taminoxmldb_database    : " + database);
    ...
    private String handleQueryString( String query, String type ) {
        ...
        if ( database.equals( "exist" ) )
         ...
         if ( database.equals( "tamino" ) )    /////////////// Lili 120803
            logger.debug("MCRXMLDBQuery handlequerstring   (tamino Changed)  : " + query);
          query = handleQueryStringTamino( query, type );    /////////
        logger.debug("Fnished MCRXMLDBQuery handlequerstring   (tamino Changed)  : " + query);
	return query;
    }
