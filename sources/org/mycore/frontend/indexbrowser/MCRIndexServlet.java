...
public class MCRIndexServlet extends MCRServlet {
	protected void doGetPost(MCRServletJob job) throws Exception {
		...
		// Execute SQL Query
		MCRSQLConnection mcrConn = MCRSQLConnectionPool.instance()
				.getConnection();
		try {
			...
			try {
				...
				if (numSelectedRows <= ic.maxPerPage) {
					...
					int stepSize = calculateStepSize(numSelectedRows,
							ic.maxPerPage);
	...
	private List buildDelimList(int from, int to, int steps,
			java.sql.ResultSet rs) throws Exception {
		...
		for (int i = from; i <= to; i++) {
			...
			while ((i < to) && rs.next()
					&& value.equals(rs.getString("idxvalue")))
				i++;
	...
	private void buildPrefixDifference(List delims) {
		for (int i = 0; i < delims.size(); i++) {
			...
			MCRRangeDelim prev = (MCRRangeDelim) (delims
					.get(Math.max(0, i - 1)));
			MCRRangeDelim next = (MCRRangeDelim) (delims.get(Math.min(i + 1,
					delims.size() - 1)));
	...
	private void sendXML(MCRServletJob job, Element root, String style)
			throws Exception {
	...
	private synchronized MCRIndexConfiguration getConfiguration(String ID) {
		MCRIndexConfiguration ic = (MCRIndexConfiguration) (configurations
				.get(ID));
		if (ic == null) {
