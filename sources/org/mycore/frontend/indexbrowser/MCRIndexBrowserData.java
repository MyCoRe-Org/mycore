...
public class MCRIndexBrowserData {
    ...
    private int createLinkedListfromSearch() {
	    ...
	    if ( mcrHit != null ) {
            ...
            for (int i = 0; i< numRows ; i++) {            	     
            	...
            	// they should be set with an searchfield with type="identifier", to get the exact String
    ...
    private int fillLinkedListWithMetadata( int from, int to) {
    	if ( from == 0 ) from=1;
    	// now we take only the objects from metadata for the dataobjects that we will shown in detail
    	// this occurres if we have only a small count of results, or we show only a part 
