...
public class MCRUserCommands {
	 ...
	 **/
	public static void deleteGroup(MCRSession session, String groupID)
		throws Exception {
		init();
	 ...
	 **/
	public static void deleteUser(MCRSession session, String userID)
		throws Exception {
		init();
	 ...
	 * @param session the MCRSession object
	 **/
	public static void listAllPrivileges(MCRSession session)
		throws MCRException {
		try {
			init();
			...
			throw new MCRException(
				"Error while command saveAllGroupsToFile()",
				e);
		}
	}
	...
	public static void saveAllGroupsToFile(MCRSession session, String filename)
		...
		try {
			org.jdom.Document jdomDoc =
				MCRUserMgr.instance().getAllGroups(session);
	 ...
	 */
	public static void saveAllPrivilegesToFile(
		MCRSession session,
		String filename)
		throws MCRException {
		try {
			org.jdom.Document jdomDoc =
				MCRUserMgr.instance().getAllPrivilegesAsJDOMDocument(session);
			FileWriter outFile = new FileWriter(new File(filename));
			saveToXMLFile(jdomDoc, outFile);
		} catch (Exception e) {
			throw new MCRException(
				"Error while command saveAllPrivilegesToFile()",
				e);
		}
	}
	...
	public static void saveAllUsersToFile(MCRSession session, String filename)
		...
		try {
			org.jdom.Document jdomDoc =
				MCRUserMgr.instance().getAllUsers(session);
			FileWriter outFile = new FileWriter(new File(filename));
			saveToXMLFile(jdomDoc, outFile);
		} catch (Exception e) {
			throw new MCRException(
				"Error while command saveAllUsersToFile()",
				e);
		}
	}
	...
	public static void saveGroupToFile(
		...
		try {
			MCRGroup group =
				MCRUserMgr.instance().retrieveGroup(session, groupID);
	...
	public static void setLock(MCRSession session) throws MCRException {
		...
		logger.info(
			"Write access to the user component persistent database now is"
				+ " denied.");
	}
	...
	public static final void showUser(MCRSession session, String userID)
		throws MCRException {
		MCRUser user =
			MCRUserMgr.instance().retrieveUser(session, userID, true);
		org.jdom.Document jdomDoc = user.toJDOMDocument();
		showAsXML(jdomDoc);
	}
	 ...
	 * @param filename the filename of the user data input
	 **/
	public static final void createUserFromFile(
		MCRSession session,
		String filename) {
		init();
	 ...
	 * @param filename the filename of the user data input
	 **/
	public static final void createGroupFromFile(
		MCRSession session,
		String filename) {
		init();
		if (!checkFilename(filename))
	 ...
	 * @param filename the filename of the user data input
	 **/
	public static final void updateUserFromFile(
		MCRSession session,
		String filename) {
		init();
		if (!checkFilename(filename))
	 ...
	 * @param filename the filename of the user data input
	 **/
	public static final void updateGroupFromFile(
		MCRSession session,
		String filename) {
		init();
		if (!checkFilename(filename))
	 ...
	 **/
	public static final void updatePrivilegesFromFile(
		MCRSession session,
		String filename)
		throws MCRException {
		init();
		...
		try {
			...
			for (int i = 0; i < listelm.size(); i++) {
				MCRPrivilege p =
					new MCRPrivilege((org.jdom.Element) listelm.get(i));
				if (p.isValid())
					list.add(p);
			}
			...
			throw new MCRException(
				"Error while update privileges form file.",
				e);
		}
	}
	 ...
	 */
	private static final void saveToXMLFile(
		org.jdom.Document jdomDoc,
		FileWriter outFile) {
		// get encoding
		config = MCRConfiguration.instance();
		...
		XMLOutputter outputter =
			new XMLOutputter(
				Format.getPrettyFormat().setEncoding(mcr_encoding));
		try {
