...
final class MCRQueryCommands extends MCRAbstractCommands {
	...
	private static final Logger LOGGER = Logger
			.getLogger(MCRQueryCommands.class);
	...
	public static void query(String host, String type, String query) {
		...
		if (type.equals("class")) // classifications
		{
			...
			synchronized (resarray) {
				if (squence.equalsIgnoreCase("local-remote")) {
					COLLECTOR.collectQueryResults("local", type, query,
							resarray);
					try {
						resarray.wait();
					...
					if (resarray.size() == 0) {
						COLLECTOR.collectQueryResults(host, type, query,
								resarray);
						try {
							resarray.wait();
						} catch (InterruptedException ignored) {
		...
		InputStream in = MCRQueryCommands.class.getResourceAsStream("/"
				+ xslfile);
		if (in == null)
			throw new MCRConfigurationException("Can't read stylesheet file "
					+ xslfile);
