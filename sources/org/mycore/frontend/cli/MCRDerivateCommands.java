...
public class MCRDerivateCommands extends MCRAbstractCommands {
	...
	/** The logger */
	private static Logger LOGGER = Logger.getLogger(MCRDerivateCommands.class
			.getName());
	...
	private static boolean processFromFile(String file, boolean update) {
		...
		try {
			...
			if (mycore_obj.getDerivate().getInternals() != null) {
				String path = mycore_obj.getDerivate().getInternals()
						.getSourcePath();
	...
	public static void save(String ID, String dirname) {
		...
		try {
			...
			InputStream in = MCRQueryCommands.class.getResourceAsStream("/"
					+ xslfile);
			if (in != null) {
				...
				trans.transform(new org.jdom.transform.JDOMSource(MCRXMLHelper
						.parseXML(xml, false)), sr);
			} else {
				out.write(xml);
				out.flush();
			}
		...
		try {
			...
			MCRFileImportExport.exportFiles(obj.receiveDirectoryFromIFS(id
					.toString()), dir);
		} catch (MCRException ex) {
			LOGGER.error(ex.getMessage());
		...
		LOGGER.info("Derivate " + id.toString() + " stored under " + dirname
				+ SLASH + id.toString() + " and " + filename + ".");
		LOGGER.info("");
	}
	...
	public static void save(String fromID, String toID, String dirname) {
		...
		try {
			for (int i = fid.getNumberAsInteger(); i < tid.getNumberAsInteger() + 1; i++) {
				...
				if (trans != null) {
					StreamResult sr = new StreamResult(out);
					trans.transform(new org.jdom.transform.JDOMSource(
							MCRXMLHelper.parseXML(xml, false)), sr);
				} else {
					out.write(xml);
					out.flush();
				}
				LOGGER.info("Object " + nid.toString() + " stored under "
						+ filename + ".");
				// store the derivate file under dirname
				try {
					...
					MCRFileImportExport.exportFiles(obj
							.receiveDirectoryFromIFS(nid.toString()), dir);
				} catch (MCRException ex) {
					LOGGER.error(ex.getMessage());
					LOGGER.error("Exception while store to object in "
							+ dirname + SLASH + nid.toString());
					LOGGER.error("");
					return;
				}
				LOGGER.info("Derivate " + nid.toString() + " stored under "
						+ dirname + SLASH + nid.toString() + " and " + filename
						+ ".");
				k++;
			}
		} catch (Exception ex) {
