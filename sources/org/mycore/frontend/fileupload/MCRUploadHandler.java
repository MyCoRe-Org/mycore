...
public abstract class MCRUploadHandler
{
   ...
   * @param numFiles the number of files that the applet will upload 
   **/
  public abstract void startUpload( int numFiles ) 
    throws Exception;

  /**
   * Before the applet sends each file, this method is called to ask
   * if this file should be uploaded and will be accepted by the server.
   * The default implementation always returns true (always upload file),
   * but subclasses should overwrite this method to decide whether the
   * file's content must be uploaded. Decision can be based on the MD5
   * checksum that the applet calculated on the client side, so unchanged
   * files do not have to be uploaded again. 
   * 
   * @param path the path and filename of the file
   * @param checksum the MD5 checksum computed at the client applet side
   * @return true, if the file should be uploaded, false if the file should be skipped
   * @throws Exception
   **/
  public boolean acceptFile( String path, String checksum ) 
    throws Exception 
  {	return true; }
   ...
   * 
   * @param path the path and filename of the file
   * @param in the inputstream to read the content of the file from
   * @throws Exception
   **/
  ...
  /**
   * When the applet finished uploading all files, this method is called
   * so that the UploadHandler subclass can finish work and commit all saved
   * files.
   * 
   * @throws Exception
   **/
  ...
  /**
   * When the applet is closed after uploading all files, the servlet
   * calls this method automatically to unregister this upload handler
   * from the UploadHandlerManager.
   **/
