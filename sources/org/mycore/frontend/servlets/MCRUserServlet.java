...
public class MCRUserServlet extends MCRServlet
{
  ...
  /**
   * This method handles the "ChangePwd" (change password) mode. The change password
   * dialog of the presentation layer must provide three passwords in the http request:
   * The new password, an repetition of the new password and (for security reasons)
   * the old password again. This method checks if the old password is correct and if
   * both new passwords are equal. If so, the password is changed and the control flow
   * is routed to the presentation of possible task for the current user. If not,
  ...
  /**
   * This method handles the "CreatePwdDialog" mode. It is nothing more than choosing
  ...
  /**
   * This method handles the "Select" mode. Depending on the privileges of the current
   * user a list of possible tasks is collected as an XML representation and forwarded
   * to the LayoutServlet.
   *
   * @param  job The MCRServletJob instance
   * @throws IOException for java I/O errors.
   * @throws ServletException for errors from the servlet engine.
  ...
  protected void selectTask(MCRServletJob job)
    throws IOException, ServletException
  {
    // For the moment only tasks possible for all users are presented. But this is work
    // in progress. In the future the list of privileges for the current user will be
    // checked here and in case he or she has additional privileges this will be
  ...
  /**
   * This method handles the "ShowUser" mode. The current user is retrieved from the
   * user manager and its XML representation is forwarded to the LayoutServlet.
   *
   * @param  job The MCRServletJob instance
   * @throws IOException for java I/O errors.
   * @throws ServletException for errors from the servlet engine.
