/**
 * $RCSfile$ $Revision$ $Date: 2005/04/20
 * 07:29:54 $
 * 
...
public class MCREditorDefReader {
     ...
     */
    protected static MCRResolvedInclude resolveInclude(String uri,
            String idref, boolean cacheable) {
        if (idref == null)
            idref = "";
        ...
        if (cached != null) {
            MCREditorServlet.logger
                    .debug("Editor resolved include from cache: " + key);
        ...
        MCREditorServlet.logger.debug("Editor resolved include " + key
                + " is cacheable? " + cacheable);
        if (cacheable)
    ...
    protected static boolean resolveIncludes(Element container) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            if (child.getName().equals("include")) {
                ...
                boolean includeCacheable = !"false".equals(child
                        .getAttributeValue("cacheable"));
                allCacheable = allCacheable && includeCacheable;

                children.remove(child);
                MCRResolvedInclude ri = resolveInclude(uri, ref,
                        includeCacheable);
