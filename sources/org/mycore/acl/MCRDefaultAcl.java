...
public final class MCRDefaultAcl implements MCRAcl, MCRAclGuarded {
     ...
     */
    public boolean containsPrincipal(MCRPrincipal principal) throws MCRAccessException, MCRInvalidPrincipalException {
    ...
    public boolean isAccessPermitted(MCRUser user, MCROwnedObject object, MCRPermission permission) {
        ...
        if (permissionRing == null) {
            ...
            return permissionRing.isPermissionGranted(permission); // the
                                                                    // status
        }
    ...
    private Map getMap(MCRPrincipal principal) throws MCRInvalidPrincipalException {
        if (principal instanceof MCRAclCategory) {
            ...
            throw new MCRInvalidPrincipalException(MCRAclMessages.UNSUPPORTED_PRINCIPAL_MESSAGE);
        }
    }
    ...
    // ------------------------------------------------------------------------------
    private void put(MCRPrincipal principal, MCRPermissionRing permissionRing) throws MCRInvalidPrincipalException {
        // Categories USERS and GROUPS are not allowed as entries in ACLs, only
        // their
        // individual members
        if ((principal == MCRAclCategory.USERS) || (principal == MCRAclCategory.GROUPS)) {
