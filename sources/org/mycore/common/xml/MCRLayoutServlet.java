...
public class MCRLayoutServlet extends MCRServlet {
	...
	protected String parseDocumentType(InputStream in) {
		...
		try {
			...
			String msg =
				"Could not build a SAX Parser for processing XML input";
		...
		DefaultHandler handler = new DefaultHandler() {
			public void startElement(
				...
				throws SAXException {
				logger.debug(
					"MCRLayoutServlet detected root element = " + qName);
			...
			public void startDTD(String name, String publicId, String systemId)
				throws SAXException {
				logger.debug(
					"MCRLayoutServlet detected DOCTYPE declaration = " + name);
		...
		try {
			...
			if (!forcedInterrupt.equals(ex.getMessage())) {
				String msg =
					"Error while detecting XML document type from input source";
	...
	public void doGetPost(MCRServletJob job)
		...
		if ((styleFile == null)
			...
			try {
				...
				logger.error(
					"IO Error while XSL transforming XML Document",
					ex);
			} catch (MCRException ex) {
				if (errorPage)
					throw new MCRException(
						"Error while genrating error page!",
						ex);
				else
					generateErrorPage(
						request,
	...
	public static Properties buildXSLParameters(HttpServletRequest request) {
		...
		logger.debug(
			"LayoutServlet XSL.MCRSessionID="
	 ...
	 **/
	protected String buildStylesheetName(
		String style,
		String docType,
		String type) {
	...
	protected Templates buildCompiledStylesheet(File file) {
		...
		if (stylesheet == null) {
			try {
				...
				logger.debug(
					"MCRLayoutServlet compiled stylesheet " + file.getName());
			} catch (TransformerConfigurationException exc) {
				String msg =
					"Error while compiling XSL stylesheet "
						+ file.getName()
						+ ": "
			...
			logger.debug(
				"MCRLayoutServlet using cached stylesheet " + file.getName());
		}
	...
	protected Transformer buildTransformer(Templates stylesheet) {
		try {
			...
			String msg =
				"Error while building XSL transformer: "
					+ exc.getMessageAndLocation();
			throw new MCRConfigurationException(msg, exc);
		}
	}
	 ...
	 **/
	protected void setXSLParameters(
	...
	protected void transform(
		...
		logger.debug(
			"MCRLayoutServlet starts to output " + ct + "; charset=" + enc);
		...
		try {
			...
			String msg =
				"Error while transforming XML using XSL stylesheet: "
