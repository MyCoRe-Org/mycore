...
public class MCRXMLContainer implements MCRSortable {
    ...
    private static final Logger LOGGER = Logger.getLogger(MCRXMLContainer.class);
    ...
    private void init() {
        ...
        defaultEncoding = config.getString("MCR.metadata_default_encoding", "UTF-8");
        host = new ArrayList();
        mcrId = new ArrayList();
        rank = new ArrayList();
        xml = new ArrayList();
     ...
     */
    public final void add(String in_host, String in_id, int in_rank, org.jdom.Element in_xml) {
        int index;
        ...
        if (index > 0) {
            status.set((index - 1), new Integer(((Integer) status.get(index - 1)).intValue() + 1));
            in_status = 2;
        }
     ...
     */
    public final void add(String in_host, String in_id, int in_rank, byte[] in_xml) throws JDOMException, IOException {
    ...
    public final byte[] exportAllToByteArray() throws IOException {
        ...
        XMLOutputter op = new XMLOutputter(Format.getRawFormat().setEncoding(defaultEncoding));
        op.output(doc, bos);
        bos.close();
    ...
    public final byte[] exportElementToByteArray(int index) throws IOException {
        ...
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        BufferedOutputStream bos = new BufferedOutputStream(os);
        XMLOutputter op = new XMLOutputter(Format.getCompactFormat().setEncoding(defaultEncoding));
        op.output(doc, bos);
        bos.close();
     ...
     */
    public final void importElements(InputStream in) throws MCRException, JDOMException, IOException {
     ...
     *                cant read the byte array as XML
     */
    public final void importElements(org.jdom.Document jdom) throws MCRException, org.jdom.JDOMException {
        org.jdom.Element root = jdom.getRootElement();
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            add(curElem.getAttributeValue(ATTR_HOST), curElem.getAttributeValue(ATTR_ID), irank, (Element) curElem.getChildren().get(0));
        }
    }
     ...
     * 
     * @param order
     *            The new order of the elements. Each entry of the array
     *            references the object (by index) of the element that should be
     *            at this position in the new object.
     */
    public synchronized void permutate(int[] order) throws MCRException {
        /* sanity check */
        if ((order.length != this.host.size()) || (order.length != this.mcrId.size()) || (order.length != this.rank.size()) || (order.length != this.xml.size()) || (order.length != this.status.size())) {
