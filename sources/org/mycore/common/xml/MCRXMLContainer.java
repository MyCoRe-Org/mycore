...
public final void add(
	...
	public final org.jdom.Document exportElementToDocument(int index) {
		...
		if ((index >= 0) && (index <= rank.size())) {
			...
			res.setAttribute(
				ATTR_PRED,
				(((((getStatus(index) >> 1)) % 2) == 1) ? "true" : "false"));
			res.setAttribute(
				ATTR_SUCC,
	 ...
	 * empty body was returned.
	 **/
	public final byte[] exportElementToByteArray(int index)
		throws IOException {
	...
	private final void resetStatus() {
		if (size() != status.size()) {
			...
			for (int i = 0; i < size(); i++) {
				...
				status.add(new Integer(in_status));
				logger.info(
					"MCRXMLContainer: refreshing status : " + in_status);
			}
		}
	}
