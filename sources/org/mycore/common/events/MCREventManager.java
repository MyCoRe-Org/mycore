...
/**
 * Acts as a multiplexer to forward events that are created to all
 * registered event handlers, in the order that is configured in mycore properties.
 * For information how to configure, see MCREventHandler javadocs.
 * 
 * @see MCREventHandler
 * 
...
public class MCREventManager
{
  ...
  {
    if( instance == null ) instance = new MCREventManager();
    return instance;
  }
  ...
  /**
   * This method is called by the component that created the event and 
   * acts as a multiplexer that invokes all registered event handlers
   * doHandleEvent methods.
   * 
  ...
  public void handleEvent( MCREvent evt ) throws MCRException
  {
    for( int i = 0; i < handlers.size(); i++ )
    {
      ...
      logger.debug( "EventManager calling handler " + eh.getClass().getName() );
