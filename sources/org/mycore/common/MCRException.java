...
public class MCRException extends RuntimeException
{
  ...
  /**
   * Creates a new MCRException with an error message and a reference 
   * to an exception thrown by an underlying system. Normally, this exception will be
   * the cause why we would throw an MCRException, e. g. when something
   * in the datastore goes wrong.
   *
   * @param message the error message for this exception
  ...
  {
    // Use counter to prevent a recursion between getStackTrace() and toString()
    toStringInvocationCounter++;
    if( toStringInvocationCounter > 1 ) return super.toString();
