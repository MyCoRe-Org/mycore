 ...
 * Each MCRCache has a certain capacity, the maximum
 * number of objects the cache will hold. When the cache is full
 * and another object is put into the cache, the cache will 
 * discard the least recently used object to get place for the
 * new object. The cache will always hold the most recently used
 * objects by updating its internal structure whenever an object
 * is get from the cache or put into the cache. The cache also
 * provides methods for getting the current cache hit rate and 
 * fill rate. Like in a hashtable, an MCRCache uses a unique key
 * for each object.
 *
...
public class MCRCache
{
  ...
  /**
   * Returns the capacity of this cache. This is the maximum number of
   * objects this cache will hold at a time.
   * 
  ...
  /**
   * Changes the capacity of this cache. This is the maximum number
   * of objects that will be cached at a time. If the new capacity
   * is smaller than the current number of objects in the cache,
   * the least recently used objects will be removed from the cache.
   * 
   * @param capacity the maximum number of objects this cache will hold
   **/
  public synchronized void setCapacity( int capacity )
  {
    MCRArgumentChecker.ensureIsTrue( capacity >= 0, 
      "cache capacity must be >= 0" );
    
    while( size > capacity ) remove( lru.key );
  ...
  /**
   * Returns the fill rate of this cache. This is the current 
   * number of objects in the cache diveded by its capacity.
   * 
  ...
  /**
   * Returns the hit rate of this cache. This is the number of successful
   * hits divided by the total number of get requests so far. Using this
   * ratio can help finding the appropriate cache capacity.
   * 
