...
public class MCRUtils {
    ...
    public static final GregorianCalendar covertDateToGregorianCalendar(
        ...
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
        try {
            calendar.setTime(formatter.parse(indate.substring(start, indate
                    .length())));
            if (!era) {
        ...
        if (!test) {
            for (int i = 0; i < MCRDefaults.SUPPORTED_LANG.length; i++) {
                DateFormat df = getDateFormat(MCRDefaults.SUPPORTED_LANG[i]);
                try {
                    calendar.setTime(df.parse(indate.substring(start, indate
                            .length())));
                    if (!era) {
                        calendar.set(Calendar.ERA, GregorianCalendar.BC);
                    }
        ...
        try {
            ...
            while (true) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(MCRUtils.class.getName() + ".copyStream(): "
    ...
    public static boolean copyReader(Reader source, Writer target) {
        ...
        try {
            ...
            while (true) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(MCRUtils.class.getName() + ".copyReader(): "
     ...
     */
    public static final HashSet mergeHashSets(HashSet set1, HashSet set2,
            char operation) {
        ...
        case COMMAND_XOR:
            ...
            break;
        default:
            throw new IllegalArgumentException("operation not permited: "
                    + operation);
        }
        return merged;
    }
     ...
     *  
     */
    public static final int readBlocking(InputStream in, byte b[], int off,
     ...
     *  
     */
    public static final int readBlocking(Reader in, char c[], int off, int len)
            throws IOException {
        int totalCharsRead = 0;

        while (totalCharsRead < len) {
            int charsRead = in.read(c, off + totalCharsRead, len
                    - totalCharsRead);
            if (charsRead < 0) {
                break;
            }
    ...
    public static final Iterator getProviders(Class service, ClassLoader loader) {
        ...
        while (services.hasMoreElements()) {
            ...
            try {
                ...
                for (StringBuffer className = new StringBuffer().append(reader
                        .readLine()); (className.length() != 4 && className
                        .toString().indexOf("null") == -1); className.delete(0,
                        className.length()).append(reader.readLine())) {
                    ...
                    if (spacePos != -1)
                        className = className.delete(spacePos, className
                                .length());
                    //trim String
                    st = 0;
                    ...
                    if (className.length() > 0) {
                        ...
                        try {
                            classInstance = Class.forName(className.toString(),
                                    true, loader).newInstance();
                            if (service.isInstance(classInstance))
                                classMap.put(className.toString(),
                                        classInstance);
                            else {
                                classInstance = null;
                                LOGGER.error(className.toString()
                                        + " does not implement "
                                        + service.getName()
                                        + "! Class instance will not be used.");
                            }
                        } catch (ClassNotFoundException e) {
                            LOGGER.error("Service: cannot find class: "
                                    + className);
                        } catch (InstantiationException e) {
                            LOGGER.error("Service: cannot instantiate: "
                                    + className);
                        } catch (IllegalAccessException e) {
                            LOGGER.error("Service: illegal access to: "
                                    + className);
                        } catch (NoClassDefFoundError e) {
                            LOGGER.error("Service: " + e + " for " + className);
                        } catch (Exception e) {
                            LOGGER.error("Service: exception for: " + className
                                    + " " + e);
                        }
                    }
                }
    ...
    public static final void saveJDOM(Document jdom, File xml)
            throws IOException {
    ...
    public static ArrayList getAllFileNames(File basedir) {
        ...
        for (int i = 0; i < stage.length; i++) {
            ...
            if (stage[i].isDirectory()) {
                out
                        .addAll(getAllFileNames(stage[i], stage[i].getName()
                                + SLASH));
            }
        }
        return out;
    ...
    public static ArrayList getAllFileNames(File basedir, String path) {
        ...
        for (int i = 0; i < stage.length; i++) {
            ...
            if (stage[i].isDirectory()) {
                out.addAll(getAllFileNames(stage[i], path + stage[i].getName()
                        + SLASH));
            }
        }
        return out;
    ...
    public static ArrayList getAllDirectoryNames(File basedir) {
        ...
        for (int i = 0; i < stage.length; i++) {
            if (stage[i].isDirectory()) {
                out.add(stage[i].getName());
                out.addAll(getAllDirectoryNames(stage[i], stage[i].getName()
                        + SLASH));
            }
        }
        return out;
    ...
    public static ArrayList getAllDirectoryNames(File basedir, String path) {
        ...
        for (int i = 0; i < stage.length; i++) {
            if (stage[i].isDirectory()) {
                out.add(path + stage[i].getName());
                out.addAll(getAllDirectoryNames(stage[i], path
                        + stage[i].getName() + SLASH));
            }
        }
        return out;
    }
