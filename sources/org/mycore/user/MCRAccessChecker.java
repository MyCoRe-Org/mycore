...
/**
 * This class holds a lot of static check methods to test some access values in
 * the current instance. The methods of this class can be used by the access
 * controll of commandline or servlets.
 * 
...
public class MCRAccessChecker {
    ...
    /**
     * The method check the given IP against an IP address area as peer of IP
     * part and subnet mask.
     * 
     * @param ip
     *            the IP they should be checked
     * @param subnetIP
     *            the IP address area
     * @param subnetMask
     *            the IP subnet mask of the IP address area
     * @return true if the IP is in the net address range, else false
     */
    public static final boolean isInetAddressInSubnet(String ip, String subnetIP, String subnetMask) throws UnknownHostException {
        if (LOGGER.isDebugEnabled()) {
            StringBuffer buf = new StringBuffer();
            buf.append(MCRAccessChecker.class.getName()).append("\nIP: ").append(ip).append("\nsubnetIP: ").append(subnetIP).append("\nsubnetMask: ").append(subnetMask);
        ...
        if (LOGGER.isDebugEnabled()) {
            StringBuffer buf = new StringBuffer();
            buf.append(MCRAccessChecker.class.getName()).append("\nIPAddr: ").append(ipAddr.getHostAddress()).append("\nsubnetIPAddr: ").append(ipSubAddr.getHostAddress()).append("\nsubnetMaskAddr: ").append(subnetMaskAddr.getHostAddress());
            LOGGER.debug(buf.toString());
        }

        int length = (ipAddr.getAddress().length + ipSubAddr.getAddress().length + subnetMaskAddr.getAddress().length) / 3;
     ...
     *            or subgroup like one of the list
     */
    public static final boolean isUserLikeListedUsers(String user, HashSet list) {
        ...
        if (!hasUserThePrivilege("editor", user)) {
            return false;
        }
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            String primaryGroupID = MCRUserMgr.instance().getPrimaryGroupIDOfUser((String) it.next());
