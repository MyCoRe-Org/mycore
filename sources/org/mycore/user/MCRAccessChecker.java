...
public class MCRAccessChecker {
	 ...
	 */
	public static final boolean isInetAddressInSubnet(String ip,
			String subnetIP, String subnetMask) throws UnknownHostException {
		...
		if ((subnetMask == null)
				|| ((subnetMask = subnetMask.trim()).length() == 0)) {
			return false;
		}
		...
		if (LOGGER.isDebugEnabled()) {
			StringBuffer buf = new StringBuffer();
			buf.append(MCRAccessChecker.class.getName()).append("\nIPAddr: ")
					.append(ipAddr.getHostAddress()).append("\nsubnetIPAddr: ")
					.append(ipSubAddr.getHostAddress()).append(
							"\nsubnetMaskAddr: ").append(
							subnetMaskAddr.getHostAddress());
			LOGGER.debug(buf.toString());
		}
			...
			for (int i = 0; i < 4; i++) {
				...
				if (LOGGER.isDebugEnabled()) {
					...
					buf.append(MCRAccessChecker.class.getName())
							.append("\ni: ").append(i).append("\nsubnet1[i]: ")
							.append(((int) subnet1[i]) & 0xff).append(
									"\nsubnet2[i]: ").append(
									((int) subnet2[i]) & 0xff);
					LOGGER.debug(buf.toString());
				}
	 ...
	 */
	public static final boolean hasUserThePrivilege(String privilege,
			String userid) {
		if ((privilege == null)
				|| ((privilege = privilege.trim()).length() == 0)) {
			return false;
		}
