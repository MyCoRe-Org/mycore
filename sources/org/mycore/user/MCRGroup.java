...
public class MCRGroup extends MCRUserObject implements MCRPrincipal {
	...
	public MCRGroup(String ID, String creator, Timestamp creationDate,
			...
			ArrayList groupIDs, ArrayList privileges) throws MCRException,
			Exception {
	...
	public MCRGroup(org.jdom.Element elm) {
		...
		if (privilegeElement != null) {
			...
			for (int j = 0; j < privilegeIDList.size(); j++) {
				org.jdom.Element privilegeID = (org.jdom.Element) privilegeIDList
						.get(j);
	...
	public final ArrayList getAllPrivileges() {
		...
		for (int i = 0; i < groupIDs.size(); i++) {
			MCRGroup nextGroup = MCRUserMgr.instance().retrieveGroup(
	...
	public boolean hasGroupMember(MCRGroup group) {
		if ((admGroupIDs.contains(group.getID()))
				|| (mbrGroupIDs.contains(group.getID())))
			return true;
		return false;
	}
	...
	public static boolean isImplicitMemberOf(MCRGroup group, String matchID)
			throws MCRException {
		MCRGroup matchGroup = MCRUserMgr.instance()
				.retrieveGroup(matchID, true);
		...
		for (int i = 0; i < matchMemberGroupIDs.size(); i++) {
			if (MCRGroup.isImplicitMemberOf(group, (String) matchMemberGroupIDs
					.get(i)))
				return true;
		}
		return false;
	...
	public boolean isValid() throws MCRException {
		ArrayList requiredGroupAttributes = MCRUserPolicy.instance()
	...
	public org.jdom.Element toJDOMElement() {
		org.jdom.Element group = new org.jdom.Element("group").setAttribute(
				"ID", ID);
		...
		org.jdom.Element CreationDate = new org.jdom.Element(
				"group.creation_date").setText(super.creationDate.toString());
		org.jdom.Element ModifiedDate = new org.jdom.Element(
				"group.last_modified").setText(super.modifiedDate.toString());
		...
		for (int i = 0; i < mbrGroupIDs.size(); i++) {
			org.jdom.Element mbrGroupID = new org.jdom.Element(
					"members.groupID").setText((String) mbrGroupIDs.get(i));
		...
		group.addContent(Creator).addContent(CreationDate).addContent(
				...
				.addContent(members).addContent(groups).addContent(Privileges);
		return group;
	}
	...
	public final boolean modificationIsAllowed() throws MCRException {
		...
		throw new MCRException("The current user " + currentUserID
				+ " has no right to modify the group " + this.ID + ".");
	}
}
