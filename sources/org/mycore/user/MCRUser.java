...
public class MCRUser extends MCRUserObject implements MCRPrincipal {
	 ...
	 */
	public MCRUser(int numID, String ID, String creator,
			Timestamp creationDate, Timestamp modifiedDate, boolean idEnabled,
			boolean updateAllowed, String description, String passwd,
			String primaryGroupID, ArrayList groupIDs, String salutation,
			String firstname, String lastname, String street, String city,
			String postalcode, String country, String state,
			String institution, String faculty, String department,
		...
		this.userContact = new MCRUserContact(salutation, firstname, lastname,
				street, city, postalcode, country, state, institution, faculty,
	...
	public MCRUser(org.jdom.Element elm) {
		...
		this.idEnabled = (elm.getAttributeValue("id_enabled").equals("true")) ? true
				: false;
		this.updateAllowed = (elm.getAttributeValue("update_allowed")
		...
		if (userGroupElement != null) {
			...
			for (int j = 0; j < groupIDList.size(); j++) {
				org.jdom.Element groupID = (org.jdom.Element) groupIDList
						.get(j);
	...
	public final ArrayList getAllGroupIDs() {
		...
		try {
			for (int i = 0; i < groupIDs.size(); i++) {
				ArrayList implicitGroupIDs = MCRUserMgr
						.instance()
	...
	/**
	 * This method returns a normalized ArrayList of all privileges of this
	 * user.
	 * 
	...
	public final ArrayList getPrivileges() {
		...
		try {
			...
			for (int i = 0; i < groupIDs.size(); i++) {
				g = MCRUserMgr.instance().retrieveGroup(
						(String) groupIDs.get(i), false);
				ar.addAll(g.getAllPrivileges());
			}
		} catch (MCRException ex) {
		}
		...
		for (int i = 0; i < n.size(); i++) {
			logger.debug("Privileg = " + (String) ar.get(i));
		}
		return n;
	}
	 ...
	 * privileges.
	 * 
	 * @return returns true if the given privilege is in one of the list of
	 *         privileges of one of the groups
	 */
	...
	public boolean isValid() {
		ArrayList requiredUserAttributes = MCRUserPolicy.instance()
	...
	public org.jdom.Element toJDOMElement() throws MCRException {
		org.jdom.Element user = new org.jdom.Element("user").setAttribute(
				"numID", Integer.toString(numID)).setAttribute("ID", ID)
				.setAttribute("id_enabled", (idEnabled) ? "true" : "false")
				.setAttribute("update_allowed",
						(updateAllowed) ? "true" : "false");
		org.jdom.Element Creator = new org.jdom.Element("user.creator")
				.setText(super.creator);
		org.jdom.Element CreationDate = new org.jdom.Element(
				"user.creation_date").setText(super.creationDate.toString());
		org.jdom.Element ModifiedDate = new org.jdom.Element(
				"user.last_modified").setText(super.modifiedDate.toString());
		...
		org.jdom.Element Primarygroup = new org.jdom.Element(
				"user.primary_group").setText(primaryGroupID);
		...
		user.addContent(Creator).addContent(CreationDate).addContent(
				ModifiedDate).addContent(Passwd).addContent(Description)
				.addContent(Primarygroup).addContent(
						userContact.toJDOMElement()).addContent(Groups);
		return user;
	}
	...
	public final boolean modificationIsAllowed() throws MCRException {
		...
		if (currentUser.hasPrivilege("user administrator")) {
			return true;
		} else if (this.ID.equals(currentUserID)
				|| this.creator.equals(currentUserID)) {
			return true;
		} else if (primaryGroup.getAdminUserIDs().contains(currentUserID)) {
			return true;
		} else { // check if the current user is (direct, not implicit) member
			// of one of the admGroups
			ArrayList admGroupIDs = primaryGroup.getAdminGroupIDs();
			for (int i = 0; i < admGroupIDs.size(); i++) {
				MCRGroup currentGroup = MCRUserMgr.instance().retrieveGroup(
						(String) admGroupIDs.get(i), false);
				if (currentGroup.getMemberUserIDs().contains(
						mcrSession.getCurrentUserID())) {
					return true;
				}
			}
		}
		throw new MCRException("The current user " + currentUserID
				+ " has no right to modify the user " + this.ID + ".");
	}
}