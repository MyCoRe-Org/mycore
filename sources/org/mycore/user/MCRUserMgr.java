...
public class MCRUserMgr {
	...
	private MCRUserMgr() throws MCRException {
		...
		useEncryption = config.getBoolean("MCR.users_use_password_encryption",
				false);

		try {
			mcrUserStore = (MCRUserStore) Class.forName(userStoreName)
					.newInstance();
		} catch (Exception e) {
	...
	public final void checkConsistency() throws MCRException {
		...
		if ((!admin.hasPrivilege("list all users"))
			...
			throw new MCRException(
					"The session does not have the privilege 'list all users'!");
		}
		if (!admin.hasPrivilege("user administrator")) {
			locked = false;
			throw new MCRException(
					"The session does not have the privilege 'user administrator'!");
		}
		...
		for (int i = 0; i < allUserIDs.size(); i++) {
			...
			for (int j = 0; j < currentGroupIDs.size(); j++) {
				if (!mcrUserStore.existsGroup((String) currentGroupIDs.get(j))) {
					logger.error("user : '" + currentUser.getID()
							+ "' error: unknown group '"
							+ (String) currentGroupIDs.get(j) + "'!");
				}
			}
			MCRGroup primaryGroup = retrieveGroup(currentUser
					.getPrimaryGroupID(), true);
			ArrayList mbrUserIDs = primaryGroup.getMemberUserIDs();
			if (!mbrUserIDs.contains(currentUser.getID())) {
				logger.error("user : '" + currentUser.getID()
						+ "' error: is not member of" + " primary group '"
		...
		for (int i = 0; i < allGroupIDs.size(); i++) {
			...
			for (int j = 0; j < mbrGroupIDs.size(); j++) {
				if (!mcrUserStore.existsGroup((String) mbrGroupIDs.get(j))) {
					logger.error("group: '" + currentGroup.getID()
							+ "' error: unknown member" + " group '"
							+ (String) mbrGroupIDs.get(j) + "'!");
			...
			// check if all privileges set for the groups exist in the privilege
			// set
	 ...
	 */
	public final synchronized void createGroup(MCRGroup group)
			throws MCRException {
		if (locked) {
			throw new MCRException(
		...
		if ((!admin.hasPrivilege("create group"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The current user does not have the privilege to create a group!");
		}
		...
		if (mcrUserStore.existsGroup(group.getID())) {
			throw new MCRException("The group '" + group.getID()
					+ "' already exists!");
		}
		...
		for (int j = 0; j < admUserIDs.size(); j++) {
			if (!mcrUserStore.existsUser((String) admUserIDs.get(j))) {
				throw new MCRException(
						"MCRUserMgr.createGroup(): unknown admin userID: "
								+ (String) admUserIDs.get(j));
			}
		}
		...
		for (int j = 0; j < admGroupIDs.size(); j++) {
			...
			if (!mcrUserStore.existsGroup((String) admGroupIDs.get(j))) {
				throw new MCRException(
						"MCRUserMgr.createGroup(): unknown admin groupID: "
								+ (String) admGroupIDs.get(j));
			}
		}
		...
		for (int j = 0; j < groupIDs.size(); j++) {
			if (!mcrUserStore.existsGroup((String) groupIDs.get(j))) {
				throw new MCRException(
						"MCRUserMgr.createGroup(): unknown groupID: "
								+ (String) groupIDs.get(j));
			}
			...
			MCRGroup linkedGroup = this.retrieveGroup((String) groupIDs.get(j),
					true);
		...
		try {
			...
			for (int i = 0; i < groupIDs.size(); i++) {
				MCRGroup membergroup = retrieveGroup((String) groupIDs.get(i),
						true);
	...
	public final synchronized void createUser(MCRUser user) throws MCRException {
		...
		if ((!admin.hasPrivilege("create user"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The current user does not have the privilege to create a user!");
		}
		...
		if (mcrUserStore.existsUser(user.getNumID(), user.getID())) {
			throw new MCRException("The user '" + user.getID()
					+ "' or numerical ID '" + user.getNumID()
					+ "' already exists!");
		}
		...
		if (!mcrUserStore.existsGroup(user.getPrimaryGroupID())) {
			throw new MCRException("The primary group of the user '"
		...
		for (int j = 0; j < groupIDs.size(); j++) {
			if (!mcrUserStore.existsGroup((String) groupIDs.get(j))) {
				throw new MCRException("The user '" + user.getID()
						+ "' is linked to the unknown group '"
						+ groupIDs.get(j) + "'.");
			}
		...
		try {
			...
			for (int i = 0; i < groupIDs.size(); i++) {
				MCRGroup otherGroup = retrieveGroup((String) groupIDs.get(i),
						true);
	 ...
	 */
	public final synchronized void deleteGroup(String groupID)
			throws MCRException {
		if (locked) {
			throw new MCRException(
					"The user component is locked. At the moment write"
							+ " access is denied.");
		}
		...
		if ((!admin.hasPrivilege("delete group"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The current user does not have the privilege to delete a group!");
		}
		...
		// updated.
		ArrayList primUserIDs = mcrUserStore
				.getUserIDsWithPrimaryGroup(groupID);
		if (primUserIDs.size() > 0) {
			throw new MCRException("Group '" + groupID
					+ "' can't be deleted since there" + " are users with '"
					+ groupID + "' as their primary group. First update or"
					+ " delete the users!");
		}
		try {
			...
			for (int i = 0; i < delGroup.getMemberGroupIDs().size(); i++) {
				...
				MCRGroup ugroup = retrieveGroup((String) delGroup
						.getMemberGroupIDs().get(i));
			...
			for (int i = 0; i < delGroup.getMemberUserIDs().size(); i++) {
				...
				MCRUser uuser = retrieveUser((String) delGroup
						.getMemberUserIDs().get(i), false);
			...
			for (int i = 0; i < delGroup.getGroupIDs().size(); i++) {
				...
				MCRGroup ggroup = retrieveGroup((String) delGroup.getGroupIDs()
						.get(i));
	...
	public final synchronized void deleteUser(String userID)
		...
		if ((!admin.hasPrivilege("delete user"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The current user does not have the privilege to delete a user!");
		}
		...
		if (!user.isUpdateAllowed()) {
			throw new MCRException("Delete for user '" + userID
					+ "' is not allowed!");
		}
		try {
			// We have to notify the groups where this user is an administrative
			// user
			ArrayList adminGroups = mcrUserStore
					.getGroupIDsWithAdminUser(userID);
			...
			for (int i = 0; i < user.getGroupIDs().size(); i++) {
				MCRGroup currentGroup = retrieveGroup((String) user
						.getGroupIDs().get(i));
	...
	public final synchronized ArrayList getAllGroupIDs() throws MCRException {
		...
		if ((!admin.hasPrivilege("list all users"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The current user does not have the privilege to list all users!");
		}
	 ...
	 */
	public final synchronized ArrayList getAllImplicitGroupIDsOfGroup(
		...
		for (int i = 0; i < allGroupIDs.size(); i++) {
			if (MCRGroup.isImplicitMemberOf(currentGroup, (String) allGroupIDs
					.get(i)))
	 ...
	 */
	public final synchronized org.jdom.Document getAllGroups()
			throws MCRException {
		// Check the privileges
		MCRSession session = MCRSessionMgr.getCurrentSession();
		MCRUser admin = retrieveUser(session.getCurrentUserID(), false);
		...
		ArrayList allGroupIDs = mcrUserStore.getAllGroupIDs();
		for (int i = 0; i < allGroupIDs.size(); i++) {
			currentGroup = mcrUserStore.retrieveGroup((String) allGroupIDs
					.get(i));
	...
	/**
	 * This method gets all privileges from the persistent datastore and returns
	 * them as a ArrayList of strings.
	 * 
	...
	public final synchronized org.jdom.Document getAllPrivilegesAsJDOMDocument()
		...
		if ((!admin.hasPrivilege("list all privileges"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The current user does not have the privilege to list all privileges!");
		}
	 ...
	 */
	public final synchronized org.jdom.Document getAllUsers()
			throws MCRException {
		// Check the privileges
		MCRSession session = MCRSessionMgr.getCurrentSession();
		MCRUser admin = retrieveUser(session.getCurrentUserID(), false);
	...
	/**
	 * The access control subsystem needs to know the group whose members have
	 * the privilege to change the owner of an object in the repository.
	 * 
	 * @return Group of users with the privilege to change the owner of an
	 *         object in the repository.
	 */
	public final MCRGroup getOwnerAdministrationGroup() {
		// loop over all groups in the system to find the one with the privilege
		// to
	 ...
	 */
	public final synchronized void importUserObject(MCRUserObject obj)
			throws MCRException {
		if (locked) {
			throw new MCRException(
					"The user component is locked. At the moment write access is denied.");
	 ...
	 *            the creator
	 */
	public final synchronized void initializeGroup(MCRGroup group,
			String creator) throws MCRException {
		...
		if (!group.isValid()) {
			throw new MCRException(
					"The data of the provided group object is not valid.");
		}
		if (mcrUserStore.existsGroup(group.getID())) {
			throw new MCRException("The group '" + group.getID()
					+ "' already exists!");
	 ...
	 * 
	 * @param privileges
	 *            The privileges ArrayList which should be created
	 */
	...
	public final synchronized void initializeUser(MCRUser user, String creator)
		...
		if (!user.isValid()) {
			throw new MCRException(
					"The data of the provided user object is not valid.");
		}
		if (mcrUserStore.existsUser(user.getNumID(), user.getID())) {
			throw new MCRException("The user '" + user.getID()
					+ "' or numerical ID '" + user.getNumID()
	 ...
	 *                if group object is not known
	 */
	protected synchronized MCRGroup retrieveGroup(String groupID,
			boolean bFromDataStore) throws MCRException {
		...
		if (reqGroup == null) { // We do not have this group in the cache.
			...
			if (reqGroup == null)
				throw new MCRException(
						"MCRUserMgr.retrieveGroup(): Unknown group '" + groupID
								+ "'!");
	 ...
	 *                if user object is not known
	 */
	protected synchronized MCRUser retrieveUser(String userID,
			boolean bFromDataStore) throws MCRException {
	...
	public final synchronized void setLock(boolean locked) {
		...
		if (!admin.hasPrivilege("user administrator")) {
			throw new MCRException(
					"The current user does not have the privilege 'user administrator'!");
		}
		this.locked = locked;
	}
	 ...
	 *            the user password
	 */
	public final void setPassword(String userID, String password)
			throws MCRException {
		...
		MCRUser user = retrieveUser(userID, false);
		if (!user.isUpdateAllowed()) {
			throw new MCRException("The update for the user " + userID
					+ " is not allowed!");
		}
		boolean test = false;
		...
		if (!test) {
			throw new MCRException(
					"You have no rights to change the users password!");
		}
	 ...
	 */
	public final synchronized void updateGroup(MCRGroup updGroup)
			throws MCRException {
		if (locked) {
			throw new MCRException(
					"The user component is locked. At the moment write access is denied.");
		...
		if ((!admin.hasPrivilege("modify group"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The current user does not have the privilege to modify this group!");
		}
		...
		if (!mcrUserStore.existsGroup(groupID)) {
			throw new MCRException("You tried to update the unknown group '"
					+ groupID + "'.");
		}
		try {
			...
			// At first we check that admins or members or privileges which may
			// be added to this
			...
			for (int i = 0; i < updGroup.getAdminUserIDs().size(); i++) {
				...
				if (!mcrUserStore.existsUser(userID))
					throw new MCRException(
							"You tried to add the unknown admin user '"
			...
			for (int i = 0; i < updGroup.getAdminGroupIDs().size(); i++) {
				...
				if (!mcrUserStore.existsGroup(gid))
					throw new MCRException(
							"You tried to add the unknown admin group '" + gid
			...
			for (int i = 0; i < updGroup.getMemberUserIDs().size(); i++) {
				...
				if (!mcrUserStore.existsUser(userID))
					throw new MCRException(
							"You tried to add the unknown member user '"
									+ userID + "' to the group '" + groupID
									+ "'.");
			}
			...
			for (int i = 0; i < updGroup.getMemberGroupIDs().size(); i++) {
				...
				if (!mcrUserStore.existsGroup(gid))
					throw new MCRException(
							"You tried to add the unknown member group '" + gid
									+ "' to the group '" + groupID + "'.");
			}

			// We do not look for newly added groupIDs in this group because
			// this will
			// be done later (in update()). Therefore we now check if the
			// privileges set for
			...
			for (int i = 0; i < oldGroup.getMemberUserIDs().size(); i++) {
				if (!updGroup.getMemberUserIDs().contains(
						oldGroup.getMemberUserIDs().get(i))) {
					userCache.remove(oldGroup.getMemberUserIDs().get(i));
				}
			}
			for (int i = 0; i < oldGroup.getMemberGroupIDs().size(); i++) {
				if (!updGroup.getMemberGroupIDs().contains(
						oldGroup.getMemberGroupIDs().get(i))) {
					groupCache.remove(oldGroup.getMemberGroupIDs().get(i));
				}
			}
			for (int i = 0; i < updGroup.getMemberUserIDs().size(); i++) {
				if (!oldGroup.getMemberUserIDs().contains(
						updGroup.getMemberUserIDs().get(i))) {
					userCache.remove(updGroup.getMemberUserIDs().get(i));
				}
			}
			for (int i = 0; i < updGroup.getMemberGroupIDs().size(); i++) {
				if (!oldGroup.getMemberGroupIDs().contains(
						updGroup.getMemberGroupIDs().get(i))) {
					groupCache.remove(updGroup.getMemberGroupIDs().get(i));
				}
			}

		} catch (MCRException ex) {
			throw new MCRException("Error while updating group "
					+ updGroup.getID(), ex);
		}
	}
	 ...
	 */
	public final synchronized void updatePrivileges(ArrayList updPriv)
			throws MCRException {
		if (locked) {
			throw new MCRException(
					"The user component is locked. At the moment write access is denied.");
		...
		if ((!admin.hasPrivilege("modify privileges"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The does not have the privilege to modify privileges!");
		}
		...
		try {
			for (int i = 0; i < updPriv.size(); i++) {
				if (!((MCRPrivilege) updPriv.get(i)).isValid()) {
					test = false;
				}
			}
		} catch (Exception e) {
	 ...
	 */
	public final synchronized void updateUser(MCRUser updUser)
			throws MCRException {
		if (locked) {
			throw new MCRException(
					"The user component is locked. At the moment write access is denied.");
		...
		if ((!admin.hasPrivilege("modify user"))
				&& (!admin.hasPrivilege("user administrator"))) {
			throw new MCRException(
					"The current user does not have the privilege to modify this user!");
		}
		...
		if (!mcrUserStore.existsUser(updUser.getID())) {
			throw new MCRException("You tried to update the unknown user '"
					+ updUser.getID() + "'.");
		}
		try {
			...
			if (!updUser.getPrimaryGroupID()
					.equals(oldUser.getPrimaryGroupID())) {
				MCRGroup testGroup = retrieveGroup(updUser.getPrimaryGroupID(),
						false);
			...
			throw new MCRException("Error while updating user "
					+ updUser.getID() + ", the user has been deleted.");
		}
	}

	/**
	 * This private helper method checks if the privileges defined for a given
	 * group really exist in the privilege set. It is used by createGroup() and
	 * updateGroup(). If a privilege does not exist an MCRException is thrown.
	 */
		...
		if (privs != null) {
			for (int i = 0; i < privs.size(); i++) {
				...
				if (!mcrUserStore.existsPrivilege(privName)) {
					throw new MCRException("Create/update of group '"
							+ group.getID() + "' failed: unknown privilege: "
							+ privName);
				}
			}
		}
	...
	private final void update(MCRUserObject updObject, MCRUserObject oldObject)
		...
		for (int i = 0; i < updObject.getGroupIDs().size(); i++) {
			...
			if (!mcrUserStore.existsGroup(gid)) {
				...
				msg.append(
						(updObject instanceof MCRUser) ? "user '" : "group '")
						.append(ID).append("' with the unknown group '")
						.append(gid).append("'.");
				throw new MCRException(msg.toString());
			}
			MCRGroup linkedGroup = retrieveGroup(gid);
			linkedGroup.modificationIsAllowed(); // If the modification is not
		...
		for (int i = 0; i < updObject.getGroupIDs().size(); i++) {
			MCRGroup updGroup = retrieveGroup((String) updObject.getGroupIDs()
					.get(i));
		...
		for (int i = 0; i < oldObject.getGroupIDs().size(); i++) {
			MCRGroup updGroup = retrieveGroup((String) oldObject.getGroupIDs()
					.get(i));
			if (!updObject.getGroupIDs().contains(updGroup.getID())) { // The
				// object
				// is no
				// longer
				// member
