...
public class MCRUserMgr {
            ...
            if (privs != null) {
                for (int j = 0; j < privs.size(); j++) {
                    if (!mcrUserStore.existsPrivilege((String) privs.get(j))) {
                        logger.error("group: '" + currentGroup.getID() + "' error: unknown privilege '" + (String) privs.get(j) + "'");
                    }
                }
            }
        }
     ...
     */
    public final synchronized org.jdom.Document getAllPrivilegesAsJDOMDocument() throws MCRException {
        // Check the privileges
        MCRSession session = MCRSessionMgr.getCurrentSession();
        MCRUser admin = retrieveUser(session.getCurrentUserID(), false);
        ...
        if (privs != null) {
            for (int i = 0; i < privs.size(); i++) {
                ...
                if (!mcrUserStore.existsPrivilege(privName)) {
                    throw new MCRException("Create/update of group '" + group.getID() + "' failed: unknown privilege: " + privName);
                }
            }
        }
    }
