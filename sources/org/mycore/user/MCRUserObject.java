...
abstract class MCRUserObject {
    protected static Logger logger = Logger.getLogger(MCRUserObject.class
            .getName());
    ...
    public MCRUserObject() {
        ID = "";
        creator = "";
        creationDate = new Timestamp(new GregorianCalendar().getTime()
                .getTime());
        modifiedDate = new Timestamp(new GregorianCalendar().getTime()
                .getTime());
        description = "";
    ...
    protected final static String trim(String s, int len) {
        ...
        if (sn.length() > len) {
            logger.warn("The string \'" + sn + "\' is too long (max. "
