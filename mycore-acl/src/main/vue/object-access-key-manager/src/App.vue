...
const fetch = async (): Promise<void> => {
  ...
  if (objectId) {
    ...
    const accessKeyInformation:
      AccessKeyInformation = await getAccessKeys(objectId, 'read', offset, perPage);
...
const handleRemoveAccessKey = async (index: number): Promise<void> => {
  ...
  if (objectId) {
    ...
    if (accessKey.id) {
      const fixedValue = accessKey.id.length > 30 ? `${accessKey.id.slice(0, 27)}...` : accessKey.value;
...
onMounted(async () => {
  if (objectId) {
    try {
      ...
      await fetch();
      configStore.manageReadAccesskeys = true;
    } catch (error) {
      handleError(error);
    }
  } else {
