...
export const getAccessKeys = async (
  ...
  Promise<AccessKeyInformation> => extractResponse(await instance.get<AccessKeyDto[]>(API_URL, {
    params: {
      reference, type: permission, offset, limit,
    },
  }));
...
export const createAccessKey = async (accessKey: AccessKeyDto): Promise<string> => (await instance
  .post(API_URL, accessKey, {
    headers: {
      'Content-Type': 'application/json',
    },
  }) as AxiosResponse).headers.location.split('/').pop() as string;
export const patchAccessKey = async (id: string, accessKey: AccessKeyDto): Promise<void> => {
