<template>
  <BaseModal v-if="accessKey" :show="showModal"
    :title="$t('component.acl.accesskey.frontend.label.viewAccessKey')"
...
const handleUpdateAccessKey = async () => {
  if (objectId && props.accessKey?.value && !busy.value) {
    busy.value = true;
    try {
      ...
      await patchAccessKey(objectId, props.accessKey.value, accessKeyDto);
      const updatedAccessKey = await getAccessKey(objectId, props.accessKey.value);
      busy.value = false;
