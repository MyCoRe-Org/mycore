...
export const fetchConfig = async (webApplicationBaseURL: string) => (await fetch(`${webApplicationBaseURL}access-key-config.json`))
  .json();
...
export const urlEncode = (value: string) => btoa(value).replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_');
