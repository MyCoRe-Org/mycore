...
<xsl:template match="metsmods">
  ...
  <object 
    codebase = "{$applet_mm.microsoft.plugin}"
    width    = "{$applet_mm.width}" 
    height   = "{$applet_mm.height}" >
    ...
    <noembed> <xsl:value-of select="$applet_mm.nojava" /> </noembed>

    <comment> <!-- for netscape -->
      <xsl:element name="embed">
        <xsl:attribute name="type">          <xsl:value-of select="$applet_mm.mime"/>            </xsl:attribute> 
        <xsl:attribute name="codebase">      <xsl:value-of select="$applet_mm.codebase"/>        </xsl:attribute> 
        <xsl:attribute name="code">          <xsl:value-of select="$applet_mm.class"/>           </xsl:attribute>
        <xsl:attribute name="cache_option">  <xsl:value-of select="$applet_mm.cache"/>           </xsl:attribute> 
        <xsl:attribute name="archive">       <xsl:value-of select="$applet_mm.archives"/>        </xsl:attribute> 
        <xsl:attribute name="cache_archive"> <xsl:value-of select="$applet_mm.archives"/>        </xsl:attribute> 
        <xsl:attribute name="width">         <xsl:value-of select="$applet_mm.width"/>           </xsl:attribute> 
        <xsl:attribute name="height">        <xsl:value-of select="$applet_mm.height"/>          </xsl:attribute> 
        <xsl:attribute name="pluginspage">   <xsl:value-of select="$applet_mm.netscape.plugin"/> </xsl:attribute> 
        <xsl:attribute name="metsfile">      <xsl:value-of select="$applet_mm.metsfile"/>        </xsl:attribute>
