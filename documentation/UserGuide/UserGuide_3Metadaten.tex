...
\begin{lstlisting}[caption=XML-Syntax eines Metadaten-Objektes,label=lst:xml_syntax_metadatenobjekt]
...
Für \mcridentifier{xsi:noNamespaceSchemaLocation} ist das entsprechende XMLSchema-File des Metadaten-Types anzugeben (z. B. \mcrfile{Document.xsd})\\[2ex]
...
Im XML-Knoten \mcridentifier{structure} sind alle Informationen über die Beziehung des Metadaten-Objektes zu anderen Objekten abgelegt. Es werden derzeit die folgenden XML-Daten unter diesem Knoten abgelegt. Die Tag-Namen \mcridentifier{parents/parent}, \mcridentifier{children/child} und \mcridentifier{derobjects/derobject} sind reserviert und {\bf dürfen NICHT anderweitig verwendet werden!} Alle Sub-Knoten haben einen Aufbau wie für MCRMetaLinkID beschieben.\\[2ex]
In \mcridentifier{parents} wird ein Link zu einem Eltern-Objekt gespeichert, sofern das referentierende Objekt Eltern hat. Ob dies der Fall ist, bestimmt die Anwendung. Das Tag dient der getsaltung von Vererbungsbäumen und kann durch den Anweder festgelegt werden. Siehe auch 'Programmers Guide', Abschnitt Vererbung. Die Werte für \mcridentifier{xlink:title} und \mcridentifier{xlink:label} werden beim Laden der Daten automatisch ergänzt.\\[2ex]
...
Das selbe gilt auch für den XML-Unterknoten \mcridentifier{derobjects}. In diesem Bereich werden alle Verweise auf die an das Metadaten-Objekt angehangenen digitalen Objekte gespeichert. Jeder Eintrag verweist mittels einer Referenz auf ein Datenobjekt vom Typ \mcridentifier{mycorederivate}, wie es im nachfolgenden Abschnitt \mcridentifier{'IFS und Content Store'} näher erläutert ist.
...
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaAddress}
Der Basistype \mcridentifier{MCRMetaAddress} beinhaltet eine Liste von postalischen Anschriften in der Ausprägung eines XML-Abschnittes. Dabei wird berücksichtigt, dass die Anschrift in verschiedenen Sprachen und in international gängigen Formen gespeichert werden soll. Die einzelnen Subtags sind dabei selbsterklärend. Die Angaben zu \mcridentifier{type} und \mcridentifier{xml:lang} sind optional, ebenso die unter \mcridentifier{subtag} liegenden Tags, jedoch muss mindestens eines ausgefüllt sein. Alle Werte wenden als Text betrachtet. Das optionale Attribut \mcridentifier{textsearch} hat keinen Effekt bei diesem Typ.\\[2ex]

\bottomcaption{}
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaAddress,label=lst:xml_syntax_MCRMetaAddress]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel des Metadaten-Basistypes MCRMetaAddress, label=lst:beispiel_basistyp_mcraddress]
...
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaBoolean}
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaBoolean,label=lst:xml_syntax_basistyp_mcrboolean]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel des Metadaten-Basistypes MCRMetaBoolean, label=lst:beispiel_basistyp_mcrboolean]
...
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaClass}
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaClass,label=lst:xml_syntax_basistyp_mcrclass]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel des Metadaten-Basistypes MCRMetaClass,label=lst:beispiel_basistyp_mcrclass]
...
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaCorporation}
Der Basistyp \mcridentifier{MCRMetaCorporation} beinhaltet eine Liste von Namen einer Firma oder Einrichtung. Dabei soll berücksichtigt werden, dass die Name in verschiedenen Sprachen und in international gängigen Formen gespeichert werden sollen.  Über das Attribut \mcridentifier{type} ist eine Differenzierung der verschiedenen Namen, z. B. Abteilungen, Institute, Kurznamen usw.) möglich. \mcridentifier{name} beinhaltet den vollständigen Namen, \mcridentifier{nickname} das Pseudonym, \mcridentifier{parent} den Namen der übergeordneten Einrichtung und \mcridentifier{property} den rechtlichen Stand, z. B. GmbH. Das optionale Attribut \mcridentifier{textsearch} hat keinen Effekt bei diesem Typ.\\[2ex]

\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaCorporation,label=lst:xml_syntax_basistyp_corporation]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel des Metadaten-Basistypes MCRMetaCorporation, label=lst:beispiel_basistyp_mcrcorporation]
...
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaDate}
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaDate,label=lst:xml_syntax_basistyp_mcrdate]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel des Metadaten-Basistypes MCRMetaDate,label=lst:beispiel_basistyp_mcrdate]
...
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaLangText}
Der Basistyp \mcridentifier{MCRMetaLangText} dient der Speicherung einer Liste von Textabschnitten mit zugehöriger Sprachangabe. Das Attribut \mcridentifier{textsearch} bewirkt, dass alle Text-Values in einen gemeinsamen Textindex des Metadaten-Objektes abgelegt werden.\\[2ex]

\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaLangText,label=lst:xml_syntax_basistyp_mcrlangtext]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel des Metadaten-Basistypes MCRMetaLangText,label=lst:beispiel_basistyp_mcrlangtext]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaLink,label=lst:xml_syntax_basistyp_mcrlink]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel des Metadaten-Basistypes MCRMetaLink,label=lst:beispiel_basistyp_mcrlink]
...
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaLinkID}
Der Basistyp \mcridentifier{MCRMetaLinkID} entspricht im Aufbau dem von \mcridentifier{MCRMetaLink}. Der einzige Unterschied ist, dass die Attribute \mcridentifier{xlink:href}, \mcridentifier{xlink:from} und \mcridentifier{xlink:to} nur mit \mcridentifier{MCRObjectID}'s belegt werden dürfen.
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaNumber}
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaNumber,label=lst:xml_syntax_basistyp_mcrnumber]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel des Metadaten-Basistypes MCRMetaNumber,label=lst:beispiel_basistyp_mcrnumber]
...
%
%
\subsubsection{XML-Syntax des Metadaten-Types MCRMetaPerson}
Der Basistyp \mcridentifier{MCRMetaPerson} beinhaltet eine Liste von Namen für natürliche Personen. Dabei wird berücksichtigt, dass die namen in verschiedenen Sprachen und international gängigen Formen auftreten können. Das Attribut \mcridentifier{type} dient der Differenzierung der verschiedenen Namen einer Person, z. B. Geburtsname, Sysnonym, Kosename usw. \mcridentifier{firstname} repräsentiert den/die Vornamen, \mcridentifier{callname} den Rufnamen, \mcridentifier{surename} den Familiennamen, \mcridentifier{academic} den akademischen  und \mcridentifier{peerage} den Adelstitel. Das optionale Attribut \mcridentifier{textsearch} hat den Effekt, dass alle textlichen Werte des Namens in das allgemeine Textsuch-Feld des Metadaten-Objektes übernommen werden..\\[2ex]

\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=XML-Syntax des Metadaten-Basistypes MCRMetaPerson,label=lst:xml_syntax_basistyp_mcrperson]
...
\lstset{language=XML,fancyvrb=true,frame=btlr,breaklines,prebreak={\space\MyHookSign}}
\begin{lstlisting}[caption=Beispiel-Syntax des Metadaten-Basistypes MCRMetaPerson,label=lst:beispiel_basistyp_mcrperson]
 <tag class="MCRMetaPerson" heritable="true" parasearch="false">
  <subtag type="geburtsname" xml:lang="de">
   ...
   <surename>Schnell<surename>
  </subtag>
  <subtag type="familienname" xml:lang="de">
   <firstname>Lisa Marie</firstname>
   <callname>Lisa</callname>
   <surename>Schmidt<surename>
...
%
%
\subsubsection{XML-Syntax des service XML-Knotens}
Für die Einrichtung eines Workflow und um die wartung großer Datenmengen zu vereinfachen, wurde noch der XML-Abschnitt \mcridentifier{service} in das Metadaten-Objekt integriert.  Hier sind Informationen wie Datumsangaben und Flags für die Verarbeitung im Batch-Betrieb enthalten. {\bf Achtung, die Tag-Namen sind fest vorgegeben und dürfen nicht anderweitig verwendet werden!} \\[2ex]
