...
\section{Hinweise zur Installation freier Datenbanken, XML:DB's, Text-Suchmaschinen und Web--Komponenten}
%
%
...
mit {\tt setDefaultJava} als root die Links richtig setzten. 

Nun prüfen sie bitte bei Verwendung von JDK 1.4.x noch, mit welcher 
XALAN-Version sie arbeiten. Diese muss gleich der von MyCoRe mitgelieferten 
Version sein, damit Inkonstitenzen vermieden werden! {\bf Ist dies nicht der
Fall, beachten Sie bitte die Hinweise unter Punkt 2.2.} Der Test geschieht mit 

{\tt java org.apache.xalan.xslt.EnvironmentCheck}
...
\item {\bf mysql-lcc-3.23.52-83} oder höher
\end{itemize}
...
\subsubsection{Die Nutzung der freien Text-Suchmaschine Lucene}
\label{sec:lucene_installation}

Um die frei verfügbare Text-Suchmaschine für die Volltextsuche Lucene des Apache
Jakarta Projektes (\url{http://jakarta.apache.org/lucene/docs/index.html}) 
zu benutzen, bedarf es keiner zusätzlichen Installationen.
Derzeit ist die Version Lucene 1.3 Final Released aktuell. 
Die erforderliche \mcrfile{lucene-1.3-final.jar} Datei wird unter dem 
MyCoRe-Kern im /mycorefile{lib} Verzeichnis mitgeliefert. Zur Nutzung ist
lediglich die weiter hinten beschriebene Konfiguration in der MyCore-Anwendung
nötig.
...
Mit \mcrcommand{rcapache2 [start; stop; restart]} als root kann der Server 
gestartet gestoppt bzw. neugestartet werden und über \mcrcommand{rcapache2 
[status]} wird der aktuelle Staus abgefragt.
