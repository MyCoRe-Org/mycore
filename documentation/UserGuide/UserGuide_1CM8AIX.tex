...
\item Stop all WebSphere applications with \mrccommand{/usr/WebSphere/AppServer/bin/stopServer ...}\footnote{like icmrm or mycoresample}
\item Start the install wizzard with \mrccommand{install\_root/updateWizard.sh} or as command line tool with \mrccommand{install\_root/updateSilent.sh}
\begin{itemize}
...
\begin{enumerate}
\item Legen Sie bitte die nachfolgenden Benutzer an. {\bf icmadmin} und {\bf rmadmin} benötigen nicht viel Platz. Für den Benutzer {\bf mcradmin} sollte ausreichend Platz eingeplant werden, da hier später die MyCoRe-Anwendungen installiert werden.. Achten Sie darauf, dass man sich in die Accounts einloggen kann, z. B. via Telnet! Im IBM Handbuch wir ein Accout {\bf icmconct} angegeben, dies entspricht in der Funktionalität unserem {\bf mcradmin}. Sicherheitshalber können sie es mit anlegen.
\begin{itemize}
...
\end{itemize}
\item Fügen sie den Nutzer {\bf root} der Gruppe {\bf db2grp1} hinzu.
\item Ergänzen Sie das File \mcrfile{/etc/environment} um folgende Zeilen:
\begin{verbatim}
#
...
\end{verbatim}
\item Ergänzen Sie das File \mcrfile{.profile} für die User {\bf root}, {\bf icmadmin}, {\bf rmadmin} und {\bf mcradmin} mit den folgenden Zeilen:
\begin{verbatim}
...
\end{verbatim}
\item Das File \mcrfile{/home/db2inst1/sqllib/profile.env} sollte folgende Einträge aufweisen:
\begin{verbatim}
...
DB2AUTOSTART='YES'
\end{verbatim}
\item Nun sollte noch einmal die DB2 und NSE gestoppt und neu gestartet werden.\footnote{Es ist sinnvoll nach jedem Reboot des Systems vor dem Start des CM diese Schritte durchzuführen um einen sicheren Ausgangspunkt zu haben!}
\begin{itemize}
...
\ttem \mcrcommand{mkdir </home/rmadmin/staging>}
...
\begin{itemize}
\item \mcrcommand{su - root}
\ttem \mcrcommand{mkdir </home/rmadmin/storage>}
...
\begin{itemize}
\item WebSphere home $\rightarrow$ /mcrfile{/usrWebSphere/AppServer}
...
\item Test the Resource Manager in a web browser $\rightarrow$ \mcrcommand{https://<your full qualified hostname>/icmrm/ICMResourceManager}\footnote{The response is : No order found to prcess, that is okay.}
\end{enumerate}
%
{\bf Installation of the FixPack 2}
\begin{enumerate}
