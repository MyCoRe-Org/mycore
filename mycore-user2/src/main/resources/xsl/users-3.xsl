...
>
  ...
  <xsl:variable name="PageID">
    <xsl:choose>
      <xsl:when test="mcrurl:get-param($RequestURL, 'XSL.Style') ='subselect'">
        <xsl:value-of select="'select-user'"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:variable name="PageTitle">
    <xsl:choose>
      <xsl:when test="mcrurl:get-param($RequestURL, 'XSL.Style') ='subselect'">
        <xsl:value-of select="mcri18n:translate('component.user2.admin.userSelect.title')"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="mcri18n:translate('component.user2.admin.users.title')"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
        ...
        <xsl:choose>
                ...
                <thead>
                  <tr>
                    ...
                    <xsl:choose>
                      <xsl:when test="@num">
                        <th scope="col">
                          <xsl:value-of select="mcri18n:translate('component.user2.admin.user.origin')"/>
                        </th>
                        <th scope="col">
                      ...
                      <xsl:otherwise>
                        <th scope="col">
                          <xsl:value-of select="mcri18n:translate('component.user2.admin.user.description')"/>
                        </th>
                      </xsl:otherwise>
                    </xsl:choose>
                  </tr>
          ...
          <xsl:when test="string-length(@num) = 0">
            ...
            <div class="alert alert-danger">
              <xsl:value-of select="mcri18n:translate('component.user2.admin.search.error.noRight')"/>
            </div>
          </xsl:when>
          <xsl:when test="@num = 0">
            <br/>
            <div class="alert alert-warning">
              <xsl:value-of select="mcri18n:translate('component.user2.admin.search.error.noUserFound')"/>
            </div>
          </xsl:when>
          <xsl:when test="number(@num) &gt; number(@max)">
            ...
            <div class="alert alert-warning">
              <xsl:value-of
                      select="mcri18n:translate-with-params('component.user2.admin.search.error.tooManyUsers', concat(@num,';',@max))"/>
            </div>
          </xsl:when>
        </xsl:choose>
      </div>
