...
public class MCRIIIFScaleParser {
    ...
    private record Dimension(Integer width, Integer height) {
        ...
        static Dimension build(String scale) {
            ...
            try {
                Integer w = wBuilder.length() == 0 ? null : Integer.parseInt(wBuilder.toString());
                Integer h = hBuilder.length() == 0 ? null : Integer.parseInt(hBuilder.toString());
                return new Dimension(w, h);
            } catch (NumberFormatException e) {
                throw new IllegalArgumentException(wBuilder.toString() + " or " + hBuilder.toString()
                    + " ist not a valid scale value!", e);
            }
        }
