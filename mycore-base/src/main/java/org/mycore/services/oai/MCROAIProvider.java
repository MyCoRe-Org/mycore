...
public class MCROAIProvider extends MCRServlet {
    ...
    private org.jdom.Document identify(HttpServletRequest request, org.jdom.Document header) {
        ...
        try {
            ...
            if (!oaiProviderFriends.equals("") && friendsAr.length > 0) {
                ...
                for (String element : friendsAr) {
                    eFriends.addContent(newElementWithContent("baseURL", frns, "http://" + element));
                }
    ...
    private org.jdom.Document listMetadataFormats(HttpServletRequest request, org.jdom.Document header) {
        ...
        while (propertiesNames.hasMoreElements()) {
            ...
            eMetadataFormat.addContent(newElementWithContent("schema", ns, MCRConfiguration.instance().getString(
                    STR_OAI_METADATA_SCHEMA + "." + metadataPrefix)));
            eMetadataFormat.addContent(newElementWithContent("metadataNamespace", ns, MCRConfiguration.instance().getString(
     ...
     */
    public static String[] getHeader(MCRBase sourceObject, String objectId, String repositoryId, String instance) {
        ...
        if (sourceObject instanceof MCRObject) {
            ...
            for (int j = 0; j < mcrObject.getMetadata().size(); j++) {
                if (mcrObject.getMetadata().getMetadataElement(j).getClassName().equals("MCRMetaClassification")) {
