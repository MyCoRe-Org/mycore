...
/**
 * A generic class to interact with metadata stored in a {@link MCRNewMetadataStore}-based store.
 * 
 * A MCRNewMetadata instance represents an object that is (to be) stored in the linked store.
 * It has an immutable set of information about its associated store, its name and its ID.
 * @author bt1cn
 *
 */
public class MCRNewMetadata {
    ...
    public MCRNewMetadata(MCRNewMetadataStore store, int id) {
        ...
        this.id = MCRObjectID.getInstance(MCRObjectID.formatID(store.getStoreID(), id));
    }
    ...
    public void create(MCRContent content) throws MCRPersistenceException {
        try {
            ...
            throw new MCRPersistenceException(
                "Failed to create " + id.toString() + " in revision " + revision + " in store!",
                e);
        }
    }
    ...
    public MCRContent read() throws MCRPersistenceException {
        try {
            ...
            throw new MCRPersistenceException(
                "Failed to read " + id.toString() + " in revision " + revision + " from store!",
                e);
        }
    }
    ...
    public void update(MCRContent content) throws MCRPersistenceException {
        try {
            ...
            throw new MCRPersistenceException(
                "Failed to update " + id.toString() + " in revision " + revision + " in store!",
                e);
        }
    }
