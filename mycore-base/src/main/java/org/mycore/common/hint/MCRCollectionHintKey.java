...
public class MCRCollectionHintKey<T, C extends Collection<T>> extends MCRHintKey<C> {
    ...
    public MCRCollectionHintKey(Class<?> hintValueClass, Class<?> namespaceClass, String name,
        ...
        super(hintValueClass, namespaceClass, name, formatter, checks(checks, elementChecks));
    }

    private static <T, C extends Collection<T>> List<Consumer<C>> checks(List<Consumer<C>> checks,
        List<Consumer<T>> elementChecks) {

        Objects.requireNonNull(checks, "Checks must not be null");
        checks.forEach(check -> Objects.requireNonNull(check, "Check must not be null"));
        ...
        List<Consumer<C>> allChecks = new ArrayList<>(checks.size() + 1);
        allChecks.addAll(checks);
        allChecks.add(collection -> elementsCheck(collection, allElementChecks));

        return allChecks;

    }

    private static <T, C extends Collection<T>> void elementsCheck(C collection, List<Consumer<T>> elementChecks) {
        collection.forEach(element -> elementChecks.forEach(elementCheck -> elementCheck.accept(element)));
    }
