...
public class MCRConfiguration {
    ...
    private void debug() {
        ...
        if (configurationDirectory != null && configurationDirectory.isDirectory()) {
            tmp = getCurrentProperties();
            File resolvedPropertiesFile = new File(configurationDirectory, "mycore.resolved.properties");
            try (FileOutputStream fout = new FileOutputStream(resolvedPropertiesFile)) {
                tmp.store(fout, comments + "\nDo NOT edit this file!");
            } catch (IOException e) {
                logWarn("Could not store resolved properties to " + resolvedPropertiesFile.getAbsolutePath(), e);
            }
        }
        if (!isLog4JEnabled()) {
        ...
        if (logger.isDebugEnabled()) {
            try (StringWriter sw = new StringWriter(); PrintWriter out = new PrintWriter(sw)) {
                tmp = tmp == null ? getCurrentProperties() : tmp;
