...
var AccessTable = function(){
	...
	return {
		...
		add: 	function(accessID, accessPool, rule, prepend){
			...
			tr.append("<td class='acle2-access-id acle2-table-access-entry-td' title='" +  accessID + "'>" + accessID + "<i class='fas fa-pencil-alt acle2-icon acle2-button-edit' title='" + geti18n("ACLE.title.edit") + "'></i>" + "</td>");
			tr.append("<td class='acle2-access-pool acle2-table-access-entry-td' title='" +  accessPool + "'>" + accessPool  + "<i class='fas fa-pencil-alt acle2-icon acle2-button-edit' title='" + geti18n("ACLE.title.edit") + "'></i>" + "</td>");
			var td = $("<td class='acle2-access-rule-parent'></td>");
			ruleSelectorInstance.append(rule, td);
			tr.append(td);
			tr.append('<td><i class="fas fa-filter acle2-icon acle2-button-filter-access" title="' + geti18n("ACLE.title.filter") + '"></i></td>');
			if(prepend == true){
		...
		edit: function(entry, accessID, accessPool, rule) {
			entry.find(".acle2-access-id").html(accessID + "<i class='fas fa-pencil-alt acle2-icon acle2-button-edit' title='" + geti18n("ACLE.title.edit") + "'></i>");
