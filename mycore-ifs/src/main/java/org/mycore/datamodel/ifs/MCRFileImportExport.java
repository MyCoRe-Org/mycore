...
/**
 * Imports or exports complete directory trees with all contained files and subdirectories between the 
 * local hosts filesystem and the internal MCRDirectory structures.
 * 
...
public class MCRFileImportExport {
    ...
    /**
     * Imports the contents of a local file or directory into a newly created MCRDirectory that is owned by the given
     * owner ID. The new MCRDirectory will have the same name as the owner ID.
     * If the local object is a file, a MCRFile with the same name will be created or updated in that MCRDirectory.
     * If the local object is a directory, all contained subdirectories and files will be imported into the newly
     * created MCRDirectory. That means that after finishing this method, the complete directory structure will have
     * been imported and mapped from the local filesystem's structure. The method checks the contents of each
     * local file to be imported. If the file's content has not changed for existing files, the internal MCRFile
     * will not be updated. If there is any exception while importing the local contents, the system will try to undo
     * this operation by completely deleting all content that was imported so far.
     * 
     * @param local
     *            the local file or directory to be imported
     * @param ownerID
     *            the ID of the logical owner of the content that will be stored
     * @return a new MCRDirectory that will contain all imported files and directories
     * as instances of MCRFilesystemNode children.
     */
    ...
    /**
     * Imports the contents of a local file or directory into an existing MCRDirectory
     * that is owned by the given owner ID. The new MCRDirectory will have the same name as the owner ID.
     * If the local object is a file, a MCRFile with the same name will be created or updated in that MCRDirectory.
     * If the local object is a directory, all contained subdirectories and files will be imported into the newly
     * created MCRDirectory. That means that after finishing this method, the complete directory structure will
     * have been imported and mapped from the local filesystem's structure. The method checks the contents of each
     * local file to be imported. If the file's content has not changed for existing files, the internal MCRFile
     * will not be updated. If there is any exception while importing the local contents,
     * the system will stop with the last state and break the work.
     * 
     * @param local
     *            the local file or directory to be imported
     * @param ownerID
     ...
     * If the local object is a file, a MCRFile with the same name will be created or updated in the given MCRDirectory.
     * If the local object is a directory, all contained subdirectories and files will be imported into the
     * given MCRDirectory. That means that after finishing this method, the complete directory structure will have been
     * imported and mapped from the local filesystem's structure.
     * The method checks the contents of each local file to be imported.
     * If the file's content has not changed for existing files, the internal MCRFile will not be updated.
     * If an internal directory is updated from a local directory, new files will be added,
     * existing files will be updated if necessary, but files that already exist in the given MCRDirectory
     * but not in the local filesystem will be kept and will not be deleted.
     * 
     * @param local
     ...
     * including all subdirectories and stored files.
     * If the local object is a file, the parent directory of that file will be used for exporting.
     * 
     * @param local
