...
public class MCRDefaultTileFileProvider implements  MCRTileFileProvider {
    ...
    public Path getTiledFile(String identifier) throws MCRIIIFImageImpl.ImageNotFoundException, MCRAccessException {
        String[] splittedIdentifier = identifier.split("/", 2);

        if (splittedIdentifier.length < 2) {
            throw new MCRIIIFImageImpl.ImageNotFoundException(identifier);
        }
