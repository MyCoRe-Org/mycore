...
PanoJS.prototype.isRefreshed = true;//stores if the viewer represents the current state of data from the model
/**
...
//	}
	...
	//remember the current motion so it's accessable from updateScreen
	this.motion = motion;
	//if no refresh is already pending create one
	if (this.isRefreshed) {
		var that = this;
...
PanoJS.prototype.updateModel = function() {
	...
	if((xViewerBorder > 0 || yViewerBorder > 0) || (motion.x == 0 && motion.y == 0)){
		...
		var motionX = - (this.x + this.motion.x);
		var motionY = - (this.y + this.motion.y);

		for (var c = 0; c < this.tiles.length; c++) {
			for (var r = 0; r < this.tiles[c].length; r++) {
				var tile = this.tiles[c][r];
				tile.posx = (tile.xIndex * this.tileSize) + motionX;
				tile.posy = (tile.yIndex * this.tileSize) + motionY;
				...
				if (tile.posy > this.height) {
					...
					while (tile.posy < -this.tileSize) {
						tile.yIndex += this.tiles[c].length;
						tile.posy = (tile.yIndex * this.tileSize) + motionX;
					}
...
//			this.y += motion.y;
//		}
		jQuery(this.viewer).trigger("move.viewer",{'x': this.x + motion.x, 'y': this.y + motion.y});
	}
};
 ...
 */
PanoJS.prototype.recenter = function(coords, absolute) {
	if (absolute) {
		coords.x += this.x;
		coords.y += this.y;
	}
