...
EventUtils.addEventListener = function(target, type, callback, captures) {
	...
	if (result)	{
		EventUtils_Events[EventUtils_Events.length] = [target, type, callback, wrapper];
	}
	return result;
}
...
EventUtils.removeEventListener = function(target, type, callback, captures) {
	...
	var selection = EventUtils.findEvents(target, type, callback);
	if (selection) {
		index = 0;
		while (index < selection.length) {
			var item = EventUtils_Events[selection[index]];
			...
			EventUtils_Events.splice(selection[index], 1);
...
EventUtils.findEvents = function(target, type, callback) {
	// to allow differences in function call
	if (type) type = type.toLowerCase();
	
	var i = EventUtils_Events.length;
	if (!i) return;
	...
	while (i >= 1) {
		 var item = EventUtils_Events[i - 1];
...
EventUtils.objEquals = function(obj1, obj2) {
	if (obj1 == obj2) {
		return true;
...
/*
EventUtils.findEvent = function(target, type, callback) {
	// to allow differences in function call
